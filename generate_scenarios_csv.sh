...
SPAM_FLAG=0 # Default to not run allcrops spam process
...
if [ $# -eq 0 ]; then
  echo "Usage: $0 [-c] [-s] [scenario_config_file_location]"
  exit 1
fi
...
# Only run the allcrops spam script if the flag is present
if [ $SPAM_FLAG -eq 1 ]; then
  cd basics_15jun22/sge_Mink3daily/prerun_scripts/
  ./process_allcrops_spam_datasets.sh
  ./import_remaining_landarea.sh
  cd -
fi
