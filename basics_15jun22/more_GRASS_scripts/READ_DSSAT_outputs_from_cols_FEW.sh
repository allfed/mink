...
#g.mlist rast pat=${base_file_name}_* | wc
  ...
  ##### prepare the geography
      ...
      range_string=`r.in.xyz input=${inputs_directory}${geog_correct_base}_geog.txt output=deleteme_just_looking_${quasi_random_code} x=4 y=3 z=1 fs="	" -s --q --o 2>>deleteme_junk_${quasi_random_code}.txt`
...
#     header_input=${inputs_directory}${geog_correct_base}_header \
    ...
    # ok, my plan is to extract just the column we care about, along with the latitude and longitude
      ...
      r.in.xyz input=deleteme_full_thing_for_import_${quasi_random_code}.txt output=${base_file_name}_${clean_column_name} x=2 y=1 z=3 fs="	" --o --q
