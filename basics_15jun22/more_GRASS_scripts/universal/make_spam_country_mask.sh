...
if [ 0 = 1 ]; then
  echo "using spam2005_30may14 country vector..."
  country_vector=spam2005_country_code_vector@spam2005_30may14
else
# this one was at 1-degree resolution because i forgot the g.region command! grrrrr....
#  echo "using spam2005_v3 country vector... (from oct/nov 2016)"
#  country_vector=spam2005_v3_country_code_vector@spam2005_v3
  echo "using spam2005_v3r1 country vector... (from march 2017; fixed 03oct17)"
  country_vector=spam2005_v3r1_country_code_vector@spam2005_v3r1
fi
...
v.extract input=$country_vector output=deleteme_vectors_we_want list=$cat_list --o --q $invert_flag 2>&1 | grep -v overwritten | grep -v "already exists"
...
if [ $n_countries -ne $n_found ]; then
  ...
  for found_country in $found_list; do
    sought_list=`echo "$sought_list" | sed "s/,$found_country,/,,/g ; s/^$found_country,/,/g ; s/,$found_country$/,/g"`
  done
...
align_region_to_raster.sh $align_raster > /dev/null
g.region align=$align_raster -a

v.to.rast input=deleteme_vectors_we_want output=$output_raster use=val value=1 --o --q 2>&1 | grep -v centroid
