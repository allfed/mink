...
{
  echo ""
  echo "Usage: $0 -r raster_name -C country_names_csv -c comment -f file_name -s sample_dpi [-l] [-t n_cols_in_legend] [-e] [-L]"
  echo ""
...
while getopts r:C:c:v:f:s:lt:eL provided_value
do
...
#~grass/grass_scripts/universal/no_legend_map_function.sh -n -r deleteme_masked_raster -c "$raster_comment" -f $file_name -s $sample_dpi $landscape_flag_to_use $value_options_to_use
~grass/grass_scripts/universal/no_legend_no_boundaries_map_function.sh -n -r deleteme_masked_raster -c "$raster_comment" -f $file_name -s $sample_dpi $landscape_flag_to_use $value_options_to_use
