...
{
  echo ""
  echo "Usage: $0 -r raster_name [-p legend_position] [-v vector_names_csv] -f file_name -s sample_dpi [-l] [-t n_cols_in_legend] [-e] [-n] [-x fontsize]"
  echo ""
  echo "raster_name is the raster to be processed"
  echo "legend_position allows you to move the legend vertically"
  ...
  echo "fontsize for legend. default=20"
  echo ""
}
...
legend_position=2.75
fontsize=20
while getopts r:p:v:f:s:lt:ex: provided_value
do
  case "$provided_value" in
    r) raster_name="$OPTARG";echo "r:$OPTARG";;
    p) legend_position="$OPTARG";echo "p:$OPTARG";;
    v) vector_names_csv="$OPTARG";echo "v:$OPTARG";;
    f) file_name="$OPTARG";echo "f:$OPTARG";;
    s) sample_dpi="$OPTARG";echo "s:$OPTARG";;
    l) landscape_flag=1;echo "l:$OPTARG";;
    t) value_type="cat"; n_cols="$OPTARG";echo "t:$OPTARG" ;;
    e) eps_flag="-e";echo "e:$OPTARG";;
    x) fontsize="$OPTARG";echo "x:$OPTARG";;
    ?) echo "???:[$OPTARG]"; usage_message;;
  esac
done
...
colortable_where="  where 0.25 $legend_position"
