...
if [ $# -lt 3 ]; then
  echo "Usage: quick_dump.sh raster_name file_name \"descriptive blurb\" [legend_options=\"options\"] [\"vector [with options]\"] [legend_width=%]

The optional parameters can be missing, but those that are used should be kept in the order above." 
...
before_equals_4=${number_4%*=} # remove anything after the equals
...
#  legend_options=`echo "$number_4" | cut --delimiter="=" --fields=2`
  if [ "$before_equals_5" = "legend_width" ]; then
    # 5 is a width thing
    legend_width=`echo "$number_5" | cut --delimiter="=" --fields=2`
    # so 6 is either empty or a vector
    vector="$number_6"
  elif [ "$before_equals_6" = "legend_width" ]; then
    # 6 is a width thing
    legend_width=`echo "$number_6" | cut --delimiter="=" --fields=2`
    # so 5 is a vector
    vector="$number_5"
  fi
elif [ "$number_4" = "legend_width" ]; then
  legend_width=`echo "$number_4" | cut --delimiter="=" --fields=2`
  if [ "$before_equals_5" = "legend_options" ]; then
    # 5 is an option thing
    legend_options=`echo "$number_5" | cut --delimiter="=" --fields=2`
    # so 6 is either empty or a vector
    vector="$number_6"
  elif [ "$before_equals_6" = "legend_options" ]; then
    # 6 is a width thing
    legend_options=`echo "$number_6" | cut --delimiter="=" --fields=2`
    # so 5 is a vector
    vector="$number_5"
  fi
