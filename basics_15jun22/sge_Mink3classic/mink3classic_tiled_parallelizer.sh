...
if [ $# -ne 7 ]; then
  ...
  echo ""
  echo "Usage: $0 data_file_base_name X_template crop_nitro_name co2_level crop_irri_name chunks_per_case"
  echo ""
...
#    new_chunk_here=${chunk_file}CZX${QRC_here}XZC_${data_file_short_name}_${chunk_index}
    ...
    if [ -z "$wheat_test" ]; then
      ...
      mink3classic_run_DSSAT_tile.sh $new_chunk_here $X_template $crop_nitro_name $co2_level "$crop_irri_name" $days_to_shift_planting

    else
      echo "   !!!! running coordination style for suspected wheat... !!!!"
      mink3classic_run_DSSAT_tile.sh $new_chunk_here $X_template $crop_nitro_name $co2_level "$crop_irri_name" $days_to_shift_planting USE_CIMMYT_BETA
    fi
