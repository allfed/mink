...
//		String fullOtherName = null;
    ...
    String provenanceName = outputDirectory + "/" + outputFilename + "_" + "provenance.txt";
		...
		if (lineIndexToProcess >= 0) {
			cliStuffToWrite = DSSATHelperMethods.cliFileContentsAllSupplied(dataMatrix, geogMatrix, lineIndexToProcess, 1.0);

			fullCLIName   = outputDirectory + "/" +        outputFilename + "_" + (lineIndexToProcess + 1) + ".CLI";
			...
			for (int lineIndex = 0; lineIndex < nLinesInDataFile; lineIndex++) {
				...
				cliStuffToWrite = DSSATHelperMethods.cliFileContentsAllSupplied(dataMatrix, geogMatrix, lineIndex, 1.0);
