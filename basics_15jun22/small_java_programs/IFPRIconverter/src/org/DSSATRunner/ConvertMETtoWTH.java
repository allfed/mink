...
90275  12.5   5.4   1.8   1.9  -999  -999
		...
		int magicOffsetBetweenMETandWTH = -11 + 5; // now different because i'm dropping blank lines in the .met; old = -15 + 5;

		dailyOutputAsArray = new String[originalAsArray.length + magicOffsetBetweenMETandWTH]; // Beware the MAGIC NUMBER!!! the difference in length between the headers...
...
//			twoDigitYear = fourDigitYear - 100 * (fourDigitYear / 100) + dayNumber;
			dssatDateCode = Integer.toString(fourDigitYear).substring(2, 4) + FunTricks.padStringWithLeadingZeros(Integer.toString(dayNumber), 3);
