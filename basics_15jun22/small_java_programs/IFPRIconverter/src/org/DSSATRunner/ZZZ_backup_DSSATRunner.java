...
public class ZZZ_backup_DSSATRunner {
	...
	private DescriptiveStatisticsUtility countStuff = null;
	private long initialSleepTimeToUse = -1;
	

	public ZZZ_backup_DSSATRunner() {
		readyToRun = false;
	}
...
//		}
		...
		cropToUse                  = initFileContents[storageIndex++];
...
//		now prepare the dependent magic numbers...
		...
		if (cropToUse.equalsIgnoreCase(this.maizeString)) {
			cropToUseInt = this.maizeInt;
		} else if (cropToUse.equalsIgnoreCase(this.riceString)) {
			cropToUseInt = this.riceInt;
		} else if (cropToUse.equalsIgnoreCase(this.wheatString)) {
			cropToUseInt = this.wheatInt;
		} else if (cropToUse.equalsIgnoreCase(this.soybeansString)) {
			cropToUseInt = this.soybeansInt;
		} else if (cropToUse.equalsIgnoreCase(this.groundnutsString)) {
			cropToUseInt = this.groundnutsInt;
		} else if (cropToUse.equalsIgnoreCase(this.cottonString)) {
			cropToUseInt = this.cottonInt;
			
			
		} else {
			System.out.println("crop string [" + cropToUse + "]" + "not in our list of supported crops; or at least, not implemented");
			throw new Exception();
		}
...
//		outputStream = new FileWriter(weatherStationFileObject);
	...
	public int[] grabHappyResults(int nHappyYears) throws InterruptedException, IOException, Exception{
		
		DescriptiveStatisticsUtility happyYields = new DescriptiveStatisticsUtility(false);
//		DescriptiveStatisticsUtility happyPlantingDates = new DescriptiveStatisticsUtility(false);
...
//		System.out.println("Overrulling usual SUMMARY.OUT path...");
//		String magicDSSATSummaryToReadPath = "D:\\rdrobert\\crop_modeling\\DSSAT\\example_run\\SUMMARY.OUT";
//		long initialSleepTimeToUse = 10;
//		
//		File errorAsFileObject = new File("D:\\rdrobert\\crop_modeling\\DSSAT\\example_run\\ERROR_ERROR.OUT");
...
//								magicHarvestedWeightAtHarvestEndIndex).trim());
				...
				if (errorAsFileObject.exists()) {
					...
					System.out.println("happy plant: \t-- bad things [null pointer] --");
					throw new Exception();
				}
	...
	private String buildFertilizerScheme() {
		
		
		
		//////////////////////////////////
		// jawoo's regime creation code //
		//////////////////////////////////

		String cropCode = null;
		int dapAnthesis = 0;
		int dapMaturity = 0;
		double nitrogenFertilizerAmount = 5;
...
//    int[][] nfrt = getFertilizerRegime(fiCropCode, nitrogenFertilizerAmount, dapAnthesis, dapMaturity);
    ...
    // Applications
    xNext = xNext.replaceAll("_FT01", new String((nfrt[0][0]) + ""));
    xNext = xNext.replaceAll("_FR01", new String((nfrt[0][1]) + ""));
    xNext = xNext.replaceAll("_FT02", new String((nfrt[1][0]) + ""));
    xNext = xNext.replaceAll("_FR02", new String((nfrt[1][1]) + ""));
    xNext = xNext.replaceAll("_FT03", new String((nfrt[2][0]) + ""));
    xNext = xNext.replaceAll("_FR03", new String((nfrt[2][1]) + ""));
    xNext = xNext.replaceAll("_FT04", new String((nfrt[3][0]) + ""));
    xNext = xNext.replaceAll("_FR04", new String((nfrt[3][1]) + ""));
    xNext = xNext.replaceAll("_FT05", new String((nfrt[4][0]) + ""));
    xNext = xNext.replaceAll("_FR05", new String((nfrt[4][1]) + ""));
	...
	private void grabRealResults(int lineIndex, int plantingWindowIndex) throws InterruptedException, IOException, Exception{
...
//		happyCountStuff = new DescriptiveStatisticsUtility(true);
...
//		MultiFormatMatrix dataMatrix = MatrixOperations.read2DMFMfromTextForceFormat(gisTableBaseName + "_data",1);
//		MultiFormatMatrix geogMatrix = MatrixOperations.read2DMFMfromTextForceFormat(gisTableBaseName + "_geog",1);
		MultiFormatMatrix dataMatrix = MatrixOperations.read2DMFMfromTextForceFormat(gisTableBaseName + "_data",3);
		MultiFormatMatrix geogMatrix = MatrixOperations.read2DMFMfromTextForceFormat(gisTableBaseName + "_geog",3);
...
//		int nLinesInDataFile = dataLinesArray.length;
		...
		nHappyYearsCode = DSSATHelperMethods.padWithZeros(nHappyRandomSeedsToUse, 5);
...
//						randomSeedToUse = firstRandomSeed + randomSeedIndex;
			...
			if (lineIndex % (nLinesInDataFile / 100 + 1) == 0) {
				System.out.println(lineIndex + "/" + nLinesInDataFile + " = " + (float)((lineIndex + 1.0)/nLinesInDataFile));
			}
		} // end for for lineIndex // old end of while loop
