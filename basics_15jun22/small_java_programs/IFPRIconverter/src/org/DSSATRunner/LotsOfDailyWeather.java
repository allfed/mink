...
//		File weatherStationFile = null;
		...
		weatherExecutionCommand[5] = fullCLIName; // magicWeatherStationNameToUse; // this is the CLI file
		...
		String provenanceString = "The command line options were: \n" +
		"gisTableBaseName = " + commandLineOptions[0] + "\n" +
		"outputDirectory  = " + commandLineOptions[1] + "\n" +
		"outputFilename   = " + commandLineOptions[2] + "\n" +
		"nameOfWeatherExecutable = " + commandLineOptions[3] + "\n" +
		"firstWeatherYear = " + commandLineOptions[4] + "\n" +
		"nFakeYears       = " + commandLineOptions[5] + "\n" +
		"randomSeed       = " + commandLineOptions[6];
...
//		double idNumber = -1; // this will be the pixel-specific id number...
		...
		for (int lineIndex = 0; lineIndex < nLinesInDataFile; lineIndex++) {
			...
			cliStuffToWrite = DSSATHelperMethods.cliFileContentsAllSuppliedONLYCLIMATEINFO(dataMatrix, geogMatrix, lineIndex, 1.0);
...
//			theRunningProcess = Runtime.getRuntime().exec(weatherExecutionCommand , null , new File("./"));
			...
			if (!temporaryWeatherFile.exists()) {
				System.out.println("the file at [" + temporaryWeatherFileLocation + "]; (" + temporaryWeatherFile + ") FAILED...");
				break;
			}
...
//			finalWeatherFileLocation = outputDirectory + "/" + outputFilename + "_" + idNumber + ".WTH";
			...
			if (lineIndex % (nLinesInDataFile / 400 + 1) == 0) {
				...
				System.out.println(lineIndex + "/" + nLinesInDataFile + " " +
						FunTricks.fitInNCharacters((100 * (lineIndex + 1.0)/nLinesInDataFile),5) + "% " + timeSoFar + "m; " + (timeSoFar*nLinesInDataFile/(lineIndex+1)) + "m remaining");
			}			
		} // for lineIndex
