...
//					System.out.println("  mf");
	...
	private int parseIntegerResults(String[] stringArray,int start,int end,long lineIndex) {
		int valueParsed = -9;
		try {
	...
	private double parseDoubleResults(String[] stringArray,int start,int end,long lineIndex) {
	...
	private void grabSingleYearResults(boolean useOldSummaryIndices, long lineIndex) throws InterruptedException, Exception {
		// declarations
		int plantingDate = -1;
		int startingDate = -2;
		int daysSincePlantingForEvent = -3;
		...
		// yield
		yieldToUse = parseIntegerResults(candidateSummaryContents,heremagicHarvestedWeightAtHarvestStartIndex,heremagicHarvestedWeightAtHarvestEndIndex,lineIndex);

		// planting / anthesis / maturity dates
		if (!useOldSummaryIndices) {
			emergenceToUse = parseIntegerResults(candidateSummaryContents,magicEmergenceDateStartIndex,magicEmergenceDateEndIndex,lineIndex);
		} else {
			emergenceToUse = -1;
		}
		startingDate  = parseIntegerResults(candidateSummaryContents,heremagicStartingDateStartIndex,heremagicStartingDateEndIndex,lineIndex);
		plantingDate  = parseIntegerResults(candidateSummaryContents,heremagicPlantingDateStartIndex,heremagicPlantingDateEndIndex,lineIndex);
		anthesisToUse = parseIntegerResults(candidateSummaryContents,heremagicAnthesisDateStartIndex,heremagicAnthesisDateEndIndex,lineIndex);
		maturityToUse = parseIntegerResults(candidateSummaryContents,heremagicMaturityDateStartIndex,heremagicMaturityDateEndIndex,lineIndex);
		if (maturityToUse < 0) {
			maturityToUse = parseIntegerResults(candidateSummaryContents,heremagicHarvestingDateStartIndex,heremagicHarvestingDateEndIndex,lineIndex);
		}

		// all the other goodies...
		for (int extraIndex = 0; extraIndex < hereextraStartIndices.length; extraIndex++) {
			extractedValues[extraIndex] = parseDoubleResults(candidateSummaryContents,hereextraStartIndices[extraIndex],hereextraEndIndices[extraIndex],lineIndex);
			if (Double.isNaN(extractedValues[extraIndex])) {
				realBadThingsCountsEntirePixel.useLongValue(1); // assess that something bad happened...
			}
		}

		// !!!!
		/// need to deal with the skipThisOne logic
...
//			}
			...
			for (int plantingWindowIndex = 0; plantingWindowIndex < nPlantingWindowsPerMonth ; plantingWindowIndex++) {
				...
				// it used to be here, but that was wrong and we were stuck with the first seed every time. changed 21dec12
...
//				averageTimePerEvent = timeForWeather.getMean() + timeForORYZA.getMean();
				...
				System.out.println("prog: " + (lineIndex + 1) + "/" + nLinesInDataFile + " = " +
						...
						"\tr/w/l/e: " + FunTricks.fitInNCharacters(readingTimerStats.getMean(), 6) +
