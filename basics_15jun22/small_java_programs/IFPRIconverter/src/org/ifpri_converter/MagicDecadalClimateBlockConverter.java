...
public class MagicDecadalClimateBlockConverter {
  ...
  public static void main(String commandLineOptions[]) throws Exception {
    ...
    if (commandLineOptions.length == 0) {
      System.out.println("Usage: org.ifpri_converter.MagicDecadalClimateBlockConverter input_filename output_filename n_years_in-block delimiter\n ");
      System.exit(1);
    }
    ...
    if ( rawData.length % blockLength != 0) {
    	System.out.println("Warning! Unexpected file length: " + rawData.length + " % " + blockLength + " = " + (rawData.length % blockLength) + "; i.e., != 0");
    }
    ...
    for (int blockIndex = 0 ; blockIndex < nBlocksGuess ; blockIndex++) {
    	...
    	for (int yearIndex = 0; yearIndex < nYearsInBlock; yearIndex++) {
    		...
    		for (int monthIndex = 0; monthIndex < 12; monthIndex++) {
    			climateValues[blockIndex][yearIndex][monthIndex] = Double.parseDouble(splitLine[monthIndex]);
    		} // monthIndex
    	} // yearIndex
    } // blockIndex
