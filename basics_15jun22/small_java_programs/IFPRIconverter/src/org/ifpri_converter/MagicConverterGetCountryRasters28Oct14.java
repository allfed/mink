...
//      valueList[lineNumber] = Double.parseDouble(splitLine[valueField].trim());
      ...
      rawCol = pixelCode - (10000 * rawRow) ;      // i think we have a N/S flip going on for this particular implementation...
    ...
    String outputDictionaryFile = filename.substring(0,filename.lastIndexOf(".")) + "_countries_dictionary.asc";
...
//    outArcWriterObject.print("yllcorner     " + (minLatitude  - cellSize/2.0) + "\n");
    ...
    for (int dictIndex = 0; dictIndex < uniqueCodeGizmo.getNStored(); dictIndex++) {
    	dictionaryWithIndices[dictIndex] = dictIndex + "\t" + uniqueCodeGizmo.getDictionaryList()[dictIndex];
    }
