...
public class MagicConverterSeveralCodeNonNumerics {
  ...
  public static void main(String commandLineOptions[]) throws Exception {
    ...
    if (commandLineOptions.length == 0) {
      System.out.println("Usage: org.ifpri_converter.MagicConverter filename output_base_filename delimiter pixelField non_numeric_column\n ");
      System.exit(1);
    }
    
    String filename         = commandLineOptions[0];
    String outputBasename   = commandLineOptions[1];
    String delimiter        = commandLineOptions[2];
    int    pixelField       = Integer.parseInt(commandLineOptions[3]); // index; typically 2 meaning #3
    int    nonNumericColumn = Integer.parseInt(commandLineOptions[4]);
    
    System.out.println("The delimiter is [" + delimiter + "]");
    
    // parse the dataColumnsCSV to an array of integers
//    String commaString = ",";
//    String[] dataColumnsStrings = nonNumericColumn.split(commaString);
    int nValueFields = 1;
    int[] valueFields = new int[nValueFields];
    valueFields[0] = nonNumericColumn;
    
//    for (int valueFieldIndex = 0; valueFieldIndex < nValueFields; valueFieldIndex++) {
//      valueFields[valueFieldIndex] = Integer.parseInt(dataColumnsStrings[valueFieldIndex]);
//    }
...
//    double[][] valueList    = new double[nLinesTotal][nValueFields];
    int[][] valueList    = new int[nLinesTotal][nValueFields];
    ...
    int valueFieldToConsider = -5;
    
    StringToUniqueCode encoderObject = new StringToUniqueCode();
    for (int lineNumber = 0 ; lineNumber < nLinesTotal ; lineNumber++) {
      ...
      // adapted for handling multiple fields of values that we want...
      for (int fieldIndex = 0; fieldIndex < nValueFields; fieldIndex++) {
        valueFieldToConsider = valueFields[fieldIndex];
//        valueList[lineNumber][fieldIndex] = Double.parseDouble(splitLine[valueFieldToConsider].trim());
        
        // we're gonna do this using the non-numerics coding thing...
        valueList[lineNumber][fieldIndex] = encoderObject.getCode(splitLine[valueFieldToConsider].trim());
      }
      ...
      // The first four digits represent row number and the last four digits represent column number.
...
//    System.out.println("t: " + minLatitude + "/" + latitude[lineNumber] + "/" + maxLatitude +
...
//    double[][] valuesGeographic = new double[nRows][nCols];
    int[][] valuesGeographic = new int[nRows][nCols];
    String outputFile = null;
    
    for (int fieldIndex = 0; fieldIndex < nValueFields; fieldIndex++) {
//      valueFieldToConsider = valueFields[fieldIndex];
//      valuesGeographic = new double[nRows][nCols];
      valuesGeographic = new int[nRows][nCols];
...
//      System.out.println("lat = " + latitude[lineNumber] + "; lon = " + longitude[lineNumber] + "; row = " + row + "; col = " + col + "; value to store = " + valueList[lineNumber]);
        
        valuesGeographic[row][col] = valueList[lineNumber][fieldIndex];
      }
      
      // now try to output it as an arc ascii file...
//    String outputFile = filename.substring(0,filename.lastIndexOf(".")) + "_arc.asc";
//      outputFile = filename.substring(0,filename.lastIndexOf(".")) + "_" + fieldIndex + ".asc";
      outputFile = outputBasename + "_" + fieldIndex + ".asc";
      
      System.out.println("   Writing: " + outputFile + " at " + new Date());
      ...
      outArcWriterObject.print("NODATA_value  " + missingValue + "\n");
    ...
    outputFile = outputBasename + "_" + 0 + ".dict.txt";
    
    System.out.println("   Writing: " + outputFile + " at " + new Date());
    FileOutputStream outDictionaryStream = new FileOutputStream(outputFile);
    PrintWriter outDictionaryObject = new PrintWriter(outDictionaryStream);
    
    String[] dictionary = encoderObject.getDictionaryList();
    for (int dictIndex = 0; dictIndex < dictionary.length; dictIndex++) {
    	outDictionaryObject.println(dictIndex + "," + dictionary[dictIndex]);
    }
    outDictionaryObject.flush();
    outDictionaryObject.close();
