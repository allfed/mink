...
//						totalError -= Math.log(1.0-alternateStandardized);
	...
	//		MultiFormatMatrix testRow = null;
			...
			MultiFormatMatrix previousStackedParameters = new MultiFormatMatrix(1,previousGradient.getDimensions());
			MultiFormatMatrix currentStackedParameters  = new MultiFormatMatrix(1,previousGradient.getDimensions());
			// put in the weights
			for (long weightIndex = 0; weightIndex < nWeights; weightIndex++) {
			...
			MultiFormatMatrix v = MatrixOperations.subtractMatrices(currentGradient, previousGradient);
