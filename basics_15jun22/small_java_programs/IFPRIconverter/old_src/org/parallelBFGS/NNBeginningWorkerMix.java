...
public class NNBeginningWorkerMix {
	...
	public static void main(String commandLineOptions[]) throws Exception {
		...
		if (commandLineOptions.length != 5) {
			System.out.println("Usage: classname initFile dataFileBase nThreadsThis sequenceNumber nextMachine [euclideanStepSize,stoppingTolerance,fractionAboveMinGuess,LSInterval,useOldSchool]");
		}
		...
		if (commandLineOptions.length == 8) {
		...
		if (commandLineOptions.length >= 6) {
			...
			lineSearchInterval    = Integer.parseInt(  commandLineOptions[5].split(",")[3]);
			if (commandLineOptions[5].split(",").length > 4) {
				useOldSchool = true;
			}
		}
				...
				if (stepIndex > 1) {
					...
					if (stepErrorsTotal[stepIndex] > stepErrorsTotal[stepIndex - 1]) {
						...
						System.out.println("quad sectioning: sMB = " + startingMultiplierBoundary + "; eR = " +
								...
								"; eta equiv = " + (finalMultiplier * 
										normalizedDirection.getValue(0,0) /
										newGradient.getValue(0,0)));
...
//		MatrixOperations.write2DMFMtoText(bfgsDirectionAndBigG[1],gradientBaseName + "_bfgsG_" + 0, delimiter);
		...
		for (int epochIndex = 1; epochIndex < nEpochs; epochIndex++) {
			...
			if (epochIndex % lineSearchInterval == 0) {
...
//					bfgsDirectionAndBigG[1] = MatrixOperations.newCopy(bigGSeed);
			...
			// end of if do linesearch
			} else {
				// just do a gradient descent thing...
				// notify...
				continueLineSearchSender.sendObject(weDoNotWantAnotherRoundFromTheWorkers);
				
				// update parameters and move on...
