...
public class MatrixOperations {
  ...
  public static MultiFormatMatrix normalizeColumn(MultiFormatMatrix X) throws Exception {
    if (X.getDimensions()[1] != 1) {
      System.out.println(
          "Error: matrix must have a single column, this has " + X.getDimensions()[1]);
      throw new Exception();
    }
  ...
  public static MultiFormatMatrix transposeMatrix(MultiFormatMatrix X) throws Exception {
    ...
    MultiFormatMatrix transposedMatrix =
        new MultiFormatMatrix(X.getDataFormat(), new long[] {nCols, nRows});
  ...
  public static MultiFormatMatrix multiplyByConstant(double constantMultiplier, MultiFormatMatrix X)
      throws Exception {
  ...
  public static MultiFormatMatrix divideByConstant(double constantDivisor, MultiFormatMatrix X)
      throws Exception {
  ...
  public static MultiFormatMatrix addMatrices(MultiFormatMatrix X, MultiFormatMatrix Y)
      throws Exception {
  ...
  public static MultiFormatMatrix subtractMatrices(MultiFormatMatrix X, MultiFormatMatrix Y)
      throws Exception {
  ...
  public static MultiFormatMatrix multiplyMatricesElementwise(
      MultiFormatMatrix X, MultiFormatMatrix Y) throws Exception {
  ...
  public static MultiFormatMatrix multiplyMatrices(MultiFormatMatrix X, MultiFormatMatrix Y)
      throws Exception {
    ...
    MultiFormatMatrix XtY =
        new MultiFormatMatrix(X.getDataFormat(), new long[] {rowSizeX, colSizeY});
    ...
    for (long OutputRow = 0; OutputRow < rowSizeX; OutputRow++) {
      ...
      for (long OutputCol = 0; OutputCol < colSizeY; OutputCol++) {
        ...
        for (long ConformableIndex = 0; ConformableIndex < colSizeX; ConformableIndex++) {
          ...
          productSum +=
              X.getValue(OutputRow, ConformableIndex) * Y.getValue(ConformableIndex, OutputCol);
        }
  ...
  public static long[] compareMatrices(MultiFormatMatrix X, MultiFormatMatrix Y, double threshold)
      throws Exception {
    ...
    if (Y.getDimensions()[0] != nRows) {
      System.out.println(
          "compareMatrices: nRows not consistent (" + nRows + ", " + Y.getDimensions()[0]);
    ...
    if (Y.getDimensions()[1] != nCols) {
      System.out.println(
          "compareMatrices: nCols not consistent (" + nCols + ", " + Y.getDimensions()[1]);
  ...
  public static MultiFormatMatrix sumToCol(MultiFormatMatrix X) throws Exception {
    MultiFormatMatrix colCollapse =
        new MultiFormatMatrix(X.getDataFormat(), new long[] {X.getDimensions()[0], 1});
    ...
    for (long rowIndex = 0; rowIndex < nRows; rowIndex++) {
      for (long colIndex = 0; colIndex < nCols; colIndex++) {
        colCollapse.setValue(
            rowIndex, 0, colCollapse.getValue(rowIndex, 0) + X.getValue(rowIndex, colIndex));
      }
    }
  ...
  public static MultiFormatMatrix sumToRow(MultiFormatMatrix X) throws Exception {
    MultiFormatMatrix rowCollapse =
        new MultiFormatMatrix(X.getDataFormat(), new long[] {1, X.getDimensions()[1]});
    ...
    for (long rowIndex = 0; rowIndex < nRows; rowIndex++) {
      for (long colIndex = 0; colIndex < nCols; colIndex++) {
        rowCollapse.setValue(
            0, colIndex, rowCollapse.getValue(0, colIndex) + X.getValue(rowIndex, colIndex));
      }
    }
  ...
  public static MultiFormatMatrix identityMFM(long nOnDiagonal, int formatIndex) throws Exception {
    MultiFormatMatrix eye =
        new MultiFormatMatrix(formatIndex, new long[] {nOnDiagonal, nOnDiagonal});

    for (long[] diagCoords = new long[] {0, 0};
        diagCoords[0] < nOnDiagonal;
        diagCoords[0]++, diagCoords[1]++) {
  ...
  public static MultiFormatMatrix getSubmatrix(
    ...
    if ((RowStartIndex > RowEndIndex)) {
      System.out.println(
          "(RowStartIndex [" + RowStartIndex + "] > RowEndIndex [" + RowEndIndex + "])");
      throw new Exception();
    }
    ...
    if (RowStartIndex < 0 || RowEndIndex < 0 || ColStartIndex < 0 || ColEndIndex < 0) {
    ...
    for (long RowIndex = 0; RowIndex < NumRows; RowIndex++) {
      for (long ColIndex = 0; ColIndex < NumCols; ColIndex++) {
        SubMatrix.setValue(
            RowIndex, ColIndex, X.getValue(RowIndex + RowStartIndex, ColIndex + ColStartIndex));
      }
    }
  ...
  public static MultiFormatMatrix generateUniformRandom(
      long nRows, long nCols, long randomSeed, int formatIndex) throws Exception {
  ...
  public static MultiFormatMatrix generateConstant(
      long nRows, long nCols, double constantValue, int formatIndex) throws Exception {
    MultiFormatMatrix constantMatrix =
        new MultiFormatMatrix(formatIndex, new long[] {nRows, nCols});
  ...
  public static MultiFormatFloat[] randomizeRows2dMffSlow(
      MultiFormatFloat originalMFF, long randomSeed, double fractionInFirst, int formatCode)
      throws Exception {
    ...
    matrixPair[0] =
        new MultiFormatFloat(formatCode, new long[] {nRowsInFirst, originalMFF.getDimensions()[1]});
    matrixPair[1] =
        new MultiFormatFloat(
            formatCode, new long[] {nRowsInSecond, originalMFF.getDimensions()[1]});

    // pull out as many random numbers as there are rows
    // look up which row corresponds to that number
    // try to pull that row. if it is already taken, go to the next one. if we hit the end, wrap to
    // the beginning
    ...
    for (int pullIndex = 0; pullIndex < nRowsInFirst; pullIndex++) {
      ...
      // copy them over
      //    	System.out.println("-- pullIndex = " + pullIndex + " ; rowToPull = " +
      // getTopIndices[0]);
      setTopIndices[0] = pullIndex;

      for (setTopIndices[1] = 0; setTopIndices[1] < nCols; setTopIndices[1]++) {
        getTopIndices[1] = setTopIndices[1];
        matrixPair[0].setValue(setTopIndices, originalMFF.getValue(getTopIndices));
      }
    ...
    for (int pullIndex = 0; pullIndex < nRowsInSecond; pullIndex++) {
      ...
      for (setBottomIndices[1] = 0; setBottomIndices[1] < nCols; setBottomIndices[1]++) {
        ...
        matrixPair[1].setValue(setBottomIndices, originalMFF.getValue(getBottomIndices));
      }
  ...
  public static MultiFormatFloat[] randomizeRows2dMff(
      MultiFormatFloat originalMFF, long randomSeed, double fractionInFirst) throws Exception {

    return randomizeRows2dMff(
        originalMFF, randomSeed, fractionInFirst, originalMFF.getDataFormat());
  }
    ...
    for (long originalRowIndex = 0; originalRowIndex < nRows; originalRowIndex++) {
      if (rowUsedFlags[(int) originalRowIndex]) {
        ...
        for (long colIndex = 0; colIndex < nCols; colIndex++) {
          ...
          //      				);
          matrixPair[0].setValue(
              topStorageRow, colIndex, originalMFF.getValue(originalRowIndex, colIndex));
        }

        //      	System.out.println("[T:" + originalRowIndex + "/" + nRows + " ; " + topStorageRow +
        // "/" + nRowsInFirst + "]");
        topStorageRow++;
        ...
        for (long colIndex = 0; colIndex < nCols; colIndex++) {
          ...
          matrixPair[1].setValue(
              bottomStorageRow, colIndex, originalMFF.getValue(originalRowIndex, colIndex));
          ...
          //      			System.out.println("bottom; original row = " + originalRowIndex + "; col = " +
          // colIndex + "; storageIndex = "
          //      					+ bottomStorageRow + "; nBottomRows = " + nRowsInSecond);
          //      			System.out.println("actual nRows = " + matrixPair[1].getDimensions()[0] + ";
          // actual nCols = " + matrixPair[1].getDimensions()[1]);
          //      			System.out.println("nTrue = " + nTrue);
          //      			throw new Exception();
          //      		}
        }

        //      	System.out.println("[B:" + originalRowIndex + "/" + nRows + " ; " +
        // bottomStorageRow + "/" + nRowsInSecond + "]");
        bottomStorageRow++;
      }
    }
  ...
  public static MultiFormatFloat[] splitOffLastColumn(MultiFormatFloat X) throws Exception {
    ...
    MultiFormatFloat Body = new MultiFormatFloat(FormatIndexBody, new long[] {nRows, nCols - 1});
    MultiFormatFloat LastColumn = new MultiFormatFloat(FormatIndexLast, new long[] {nRows, 1});
    ...
    for (long rowIndex = 0; rowIndex < nRows; rowIndex++) {
      ...
      for (long colIndex = 0; colIndex < nCols - 1; colIndex++) {
        ...
        Body.setValue(bodyCoord, X.getValue(bodyCoord));
      }
  ...
  public static MultiFormatFloat[] splitOffLastTwoColumns(MultiFormatFloat X) throws Exception {
    ...
    MultiFormatFloat Body = new MultiFormatFloat(FormatIndexBody, new long[] {nRows, nCols - 2});
    MultiFormatFloat LastTwoColumns = new MultiFormatFloat(FormatIndexLast, new long[] {nRows, 2});

    //		System.out.println("     splitting last two (nCols = " + nCols + ", nRows = " + nRows + "):
    // Body cols = " + Body.getDimensions()[1] + "; LastTwoColumns cols = " +
    // LastTwoColumns.getDimensions()[1]);
  ...
  public static MultiFormatFloat read2DMFFfromText(String basefilename) throws Exception {
    ...
    MultiFormatFloat X = new MultiFormatFloat(FormatIndex, new long[] {nRows, nCols});
    ...
    for (int rowIndex = 0; rowIndex < nRows; rowIndex++) {
      ...
      while (currentColumn < nCols) {
        try {
          ...
          System.out.println(
              "Possibly ran past the end of file (this row = index #" + rowIndex + ")...");
        ...
        try {
          ...
          System.out.println(
              "row: " + rowIndex + "; col: " + currentColumn + "; string = [" + tempString + "]");
          eee.printStackTrace();
        }
  ...
  public static void write2DMFFtoText(MultiFormatFloat X, String basefilename, String delimiter)
      throws Exception {
    String FileName = basefilename + ".txt";
    String InfoFileName = basefilename + ".info.txt";
  ...
  public static void write2DMFFtoTextAsInt(
      MultiFormatFloat X, String basefilename, String delimiter) throws Exception {
    String FileName = basefilename + ".txt";
    String InfoFileName = basefilename + ".info.txt";
  ...
  public static MultiFormatMatrix read2DMFMfromTextForceFormat(String basefilename, int formatIndex)
      throws Exception {
    ...
    MultiFormatMatrix X = new MultiFormatMatrix(formatIndex, new long[] {nRows, nCols});
  ...
  public static void write2DMFMtoTextAsInt(
      MultiFormatMatrix X, String basefilename, String delimiter) throws Exception {
    String FileName = basefilename + ".txt";
    String InfoFileName = basefilename + ".info.txt";
  ...
  public static void write2DMFMtoTextDecimals(
      MultiFormatMatrix X, String basefilename, String delimiter, int nDecimals) throws Exception {
    ...
    System.out.println(
        "write2DMFMtoTextDecimals trying for (checked) " + nDecimalsToUse + " decimal places");
      ...
      for (long ColIndex = 0; ColIndex < NumCols; ColIndex++) {
        if (ColIndex < NumCols - 1) {
          ...
          outWriterObject.print(
              FunTricks.onlySomeDecimalPlaces(X.getValue(RowIndex, ColIndex), nDecimalsToUse)
                  + delimiter);
        } else {
          outWriterObject.print(
              FunTricks.onlySomeDecimalPlaces(X.getValue(RowIndex, ColIndex), nDecimalsToUse)
  ...
  public static void write2DMFMtoTextDecimalsAlt(
      MultiFormatMatrix X, String basefilename, String delimiter, int nDecimals) throws Exception {
    ...
    for (long RowIndex = 0; RowIndex < NumRows; RowIndex++) {
      for (long ColIndex = 0; ColIndex < NumCols; ColIndex++) {
        if (ColIndex < NumCols - 1) {
          // not the last one in the row...
          outWriterObject.print(
              ((long) (X.getValue(RowIndex, ColIndex) * trimValue)) / trimValue + delimiter);
        } else {
          outWriterObject.print(
              ((long) (X.getValue(RowIndex, ColIndex) * trimValue)) / trimValue + "\n");
        }
      }
    }
  ...
  public static void write2DMFMtoText(MultiFormatMatrix X, String basefilename, String delimiter)
      throws Exception {
    String FileName = basefilename + ".txt";
    String InfoFileName = basefilename + ".info.txt";
  ...
  public static void print2dMFF(MultiFormatFloat mff) throws Exception {
    for (long rowIndex = 0; rowIndex < mff.getDimensions()[0]; rowIndex++) {
      for (long colIndex = 0; colIndex < mff.getDimensions()[1]; colIndex++) {
        System.out.println(
            "[" + rowIndex + "][" + colIndex + "] = " + mff.getValue(rowIndex, colIndex));
      }
    }
  }
  ...
  public static void print2dMFM(MultiFormatMatrix mfm) throws Exception {
    for (long rowIndex = 0; rowIndex < mfm.getDimensions()[0]; rowIndex++) {
      for (long colIndex = 0; colIndex < mfm.getDimensions()[1]; colIndex++) {
        System.out.println(
            "[" + rowIndex + "][" + colIndex + "] = " + mfm.getValue(rowIndex, colIndex));
      }
    }
  }
  ...
  public static void print2dMFM(MultiFormatMatrix mfm, String label) throws Exception {
    for (long rowIndex = 0; rowIndex < mfm.getDimensions()[0]; rowIndex++) {
      for (long colIndex = 0; colIndex < mfm.getDimensions()[1]; colIndex++) {
        System.out.println(
            label + " [" + rowIndex + "][" + colIndex + "] = " + mfm.getValue(rowIndex, colIndex));
      }
    }
  }

  public static MultiFormatFloat[] balancedBlocks(
      MultiFormatFloat originalMFF, String PathOfWeights, int formatIndex) throws Exception {
    ...
    for (int lineIndex = 0; lineIndex < nLines; lineIndex++) {
      ...
      for (int withinTypeIndex = 0; withinTypeIndex < rawListBlocks[lineIndex]; withinTypeIndex++) {
        if (realBlockIndex == 0) {
          ...
          firstRowAfterBlock[realBlockIndex] =
              (long) (totRows * (rawListWeights[lineIndex] / totWeight));
          ...
          nRowsInBlock[realBlockIndex] =
              firstRowAfterBlock[realBlockIndex] - firstRowAfterBlock[realBlockIndex - 1];
          realBlockIndex++;
        } else {
          // anything in the middle. we just take the previous block boundary and add the weighted
          // fraction of the total number of rows...
          firstRowAfterBlock[realBlockIndex] =
              firstRowAfterBlock[realBlockIndex - 1]
                  + (long) (totRows * (rawListWeights[lineIndex] / totWeight));
          nRowsInBlock[realBlockIndex] =
              firstRowAfterBlock[realBlockIndex] - firstRowAfterBlock[realBlockIndex - 1];
          realBlockIndex++;
    ...
    for (int aBlockIndex = 0; aBlockIndex < totBlocks; aBlockIndex++) {
      //      System.out.println("for block #" + aBlockIndex + " the last row is " +
      // lastRowInBlock[aBlockIndex] + "; nRows = " + nRowsInBlock[aBlockIndex]);
      ...
      blockMatrices[aBlockIndex] =
          new MultiFormatFloat(formatIndex, new long[] {nRowsInBlock[aBlockIndex], nCols});
      ...
      for (long outRowIndex = 0; outRowIndex < nRowsThis; outRowIndex++) {
        ...
        for (long colIndex = 0; colIndex < nCols; colIndex++) {
          ...
          blockMatrices[aBlockIndex].setValue(setCoords, originalMFF.getValue(getCoords));
        }
      }
  ...
  public static MultiFormatFloat descriptiveStatistics(MultiFormatFloat X) throws Exception {
    ...
    for (long rowIndex = 0; rowIndex < nRows; rowIndex++) {
      ...
      for (long colIndex = 0; colIndex < nCols; colIndex++) {
        ...
        //        tempDouble = X.getValue(rowIndex,colIndex);
        tempDouble = X.getValue(getCoords);
        columnSumDiffSquares[(int) colIndex] +=
            (tempDouble - meansDouble[(int) colIndex]) * (tempDouble - meansDouble[(int) colIndex]);
      }
    }
  ...
  public static void writeMFF(MultiFormatFloat matrix, String FileName) throws Exception {

    MultiFormatFloat newMatrix = new MultiFormatFloat(3, matrix.getDimensions(), FileName);
  ...
  public static void writeMFM(MultiFormatMatrix matrix, String FileName) throws Exception {

    MultiFormatMatrix newMatrix = new MultiFormatMatrix(3, matrix.getDimensions(), FileName);
  ...
  public static MultiFormatFloat recenterAndRescale(
      MultiFormatFloat X, MultiFormatMatrix CenteringValuesIn, MultiFormatMatrix ScalingValuesIn)
      throws Exception {
    ...
    MultiFormatFloat standardizedMatrix =
        new MultiFormatFloat(formatIndex, new long[] {nRowsX, nColsX});
