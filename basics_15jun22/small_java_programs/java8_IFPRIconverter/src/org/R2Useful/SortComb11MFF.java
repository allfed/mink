...
public class SortComb11MFF {

	public String getModuleInfo() {
		return "MffSortComb11. This module sorts a 2-d MFF matrix by the values in the specified column. That is, " +
		"the rows are preserved together and are put in increasing order by the values in the specified column.<p>" +
		"This will also sort a 1-d long array if supplied (otherwise pass in a null). This is so that the matrix could " +
		"be restored to another form (e.g., a map). A long array is necessary due " +
		"to the shortcomings of floats in storing large integers. A second long array will be created " +
		"(and similarly sorted) " +
		"to contain the original row number within the matrix.";
	}
	...
	private float[] contentsA = null;
	private float[] contentsB = null;
	int rowListValueA = -1;
	int rowListValueB = -1;

	private MultiFormatFloat originalMatrix = null;
	private long             columnToSortBy = -1;

	int[] originalRowList = null;
	MultiFormatFloat sortedMatrix = null;

	private boolean readyToRun = false;
	private boolean sortingHasBeenDone = false;

	public SortComb11MFF(MultiFormatFloat OriginalMatrix, long ColumnToSortBy) {
	...
	private void  swapRows(MultiFormatFloat sortMatrix, int[] originalRowList, long rowA, long rowB) throws Exception {
	...
	public void doSorting() throws Exception {
		...
		if (columnToSortBy >= nCols) {
			System.out.println("sorting column does not exist: ColumnToSortBy (" + columnToSortBy + ") >= nCols (" + nCols + ")");
			throw new Exception();
		}
		...
		long nColsSorted = nCols; // changing because i'm going to keep the list of original indices separate
		...
		sortedMatrix = new MultiFormatFloat(originalMatrix.getDataFormat(), theDims);
//		long[] originalRowList = new long[(int)nRows];
		...
		for (int rowIndex = 0; rowIndex < nRows; rowIndex++) {
			...
			// initialize the MFF
			tempFloat = originalMatrix.getValue(getIndices);
			sortedMatrix.setValue(setIndices,tempFloat);
		}
		...
		long gap = nRows;
		float nSwaps = -1; // initialize to a non-zero value
...
//		long[] sortIndicesB = new long[(int)nColsSorted];
		contentsA = new float[(int)nColsSorted];
		contentsB = new float[(int)nColsSorted];

		float magicShrinkFactor = 1.3F;

		long rowA, rowB = 0;
		float valueRowA, valueRowB = 0.0F;

		int approxPassesToOne = (int)Math.round(Math.log(nRows) / Math.log(magicShrinkFactor));
//		int maxPasses = 20;
		System.out.println("It will require roughly " + approxPassesToOne + " passes to get to a gap of unity.");
...
//				System.out.println(" rowA [" + rowA + "] = " + valueRowA + "; rowB [" + rowB + "] = " + valueRowB);

				if ( valueRowA > valueRowB ) {
					swapRows(sortedMatrix,originalRowList,rowA,rowB);
					nSwaps++;
//					System.out.println("  -- now swapping rows " + internalRowIndex + " and " + (internalRowIndex + gap) + "; nSwaps now = " + nSwaps);
				}
				internalRowIndex++;

//				System.out.println(" == bottom: internalRowIndex = " + internalRowIndex + "; + gap = " + (internalRowIndex + gap) + "; nRows = " + nRows);
			} // end comb while loop
			...
			nPasses++;
//			System.out.println("at bottom [" + nPasses + "]: gap = " + gap + "; nSwaps = " + nSwaps + "; keepGoing = " + keepGoing);
//			if (nPasses == maxPasses) {
//			keepGoing = false;
//			}
			System.out.println("Finishing pass [" + nPasses + "/" + approxPassesToOne + "] (gap = " + gap + "; nSwaps = " + nSwaps + ") at " + new Date());
			System.gc();
		} // end gap while loop
		...
		System.out.println("Finished with sort (" + nPasses + " passes and " + nSwaps + " swaps required) " + new Date() );
	...
	public MultiFormatFloat getSortedColumnOnly() {
		if (this.sortingHasBeenDone) {
	...
	public static MultiFormatFloat reconstructFullSortedMatrix(
			MultiFormatFloat originalMatrix,
			int[] originalRowIndices
	) throws Exception {
		MultiFormatFloat sortedMatrix = new MultiFormatFloat(originalMatrix.getDataFormat(),originalMatrix.getDimensions());
	...
	public MultiFormatFloat getFullySortedMatrix() throws Exception {
