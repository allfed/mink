...
public class DrawFromCDF {
  ...
  public DrawFromCDF(double[] fractions, double[] values) throws Exception {
    if (fractions.length == values.length) {
      ...
      System.out.println(
          "arrays different lengths: fractions ["
              + fractions.length
              + "] != ["
              + values.length
              + "]");
  ...
  public DrawFromCDF(String percentileValueFilename, String delimiter) throws IOException {
    ...
    for (int quantileIndex = 0; quantileIndex < nQuantiles; quantileIndex++) {
      ...
      quantileFractions[quantileIndex] =
          Double.parseDouble(fileContents[quantileIndex].split(delimiter)[0]) / 100.0;
      quantileValues[quantileIndex] =
          Double.parseDouble(fileContents[quantileIndex].split(delimiter)[1]);
    }
  ...
  public double provideSingleDraw() {
    ...
    // do brute force search for which places to interpolate between... this will accomodate
    // irregular CDF definitions
    ...
    double finalValue =
        (thisRandomDraw - quantileFractions[bottomQuantileIndex])
                / (quantileFractions[bottomQuantileIndex + 1]
                    - quantileFractions[bottomQuantileIndex])
