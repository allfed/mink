...
public class SimpleReceiver implements Runnable {
  ...
  public synchronized void startListening() throws Exception {
    if (verboseStatus) {
      System.out.println(
          " [" + this.toString() + "; " + label + "] attempting to listen on " + PortNumber);
    }
  ...
  public synchronized void stopListening() throws Exception {
    if (verboseStatus) {
      System.out.println(
          " [" + this.toString() + "; " + label + "] about to close Socket " + theSocketWeUse);
    }
