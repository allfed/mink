...
public class FunTricks {
	...
	public static void appendLineToTextFile(String lineToAppend, String inputFile, boolean appendFlag) throws FileNotFoundException, IOException {
...
//		System.out.println("onlySomeDecimalPlaces: patter format = [" + patternFormat + "]");
	...
	public static String onlySomeDecimalPlaces(double value, int nDecimals) {
		...
		if (nDecimals > 0) {
		    for (int charIndex = withPossibleExtraZeros.length() - 1; charIndex > withPossibleExtraZeros.length() - nDecimals - 1; charIndex--) {
			digitToConsider = withPossibleExtraZeros.substring(charIndex, charIndex + 1);
//			System.out.println("sv=[" + withPossibleExtraZeros + "], index to consider=[" + charIndex + "], digit to consider=[" + digitToConsider + "]");
...
//		String previousResult = FunTricks.onlySomeDecimalPlaces(value, nDecimalPlacesToUse);
		String previousResult = FunTricks.onlySomeDecimalPlacesKeepTrailingZeros(value, nDecimalPlacesToUse);
		// trim off the decimal point
		previousResult = previousResult.substring(0, previousResult.lastIndexOf("."));
		String partialResult = FunTricks.onlySomeDecimalPlacesKeepTrailingZeros(value, nDecimalPlacesToUse);
		while (partialResult.length() <= n) {
			previousResult = partialResult;
			
			partialResult = FunTricks.onlySomeDecimalPlacesKeepTrailingZeros(value, ++nDecimalPlacesToUse);
//			System.out.println("v=" + value + "; nDecimalPlacesToUse=" + nDecimalPlacesToUse + "; previous=[" + previousResult + "]; partialResult=[" + partialResult + "]");
		}
...
//				finalString = FunTricks.padStringWithLeadingSpaces(numberPart, n);
		...
		// ok, there is a funny case when you have something like -9.999999906077472 and 5 characters, it will
		// end up rounding to -10.00; rather than figure out the "right way", i'm just going to manually correct it
		// at the end here.
//		if (finalString.length() != n) {
//			int nDecimalPlacesInFinalString = finalString.length() - finalString.indexOf(".") - 1;
//			double backToDouble = Double.parseDouble(finalString);
//			// the idea is that this is usually a problem with negative numbers (positive numbers just
//			// run out of space in our tiny mantissa), so we add a tiny bit to bring it closer to zero
//			// and hence
...
//			String newFinalString = FunTricks.onlySomeDecimalPlaces(value, nDecimalPlacesInFinalString - 1);
//			
//			System.out.println("      fitInNCharacters funny case [" + value + "] in " + n + 
//					" --> [" + finalString + "] nDPIFS=" + nDecimalPlacesInFinalString + "; newValue=" + newValue);
//
//		}
		return finalString;
	}
	...
	public static String[] parseRepeatedDelimiterToArray(String lineContents, String delimiter) {
		...
		// be able to take fixed spaced outputs and parse them like they are delimited because they might
	...
	public static String statusCheck(int currentCaseIndex, int nTotalCases, int nChecks) throws Exception {
		
		String statusString = null;
		
		if (currentCaseIndex % (nTotalCases / nChecks + 1) == 0) {
			statusString = currentCaseIndex + "/" + nTotalCases + " = " + FunTricks.fitInNCharacters(100 * (currentCaseIndex + 1.0)/nTotalCases, 5) + "%";
		}

		return statusString;
	}

	public static String statusCheck(int currentCaseIndex, int nTotalCases, int nChecks, long startTimeMillis) throws Exception {
		
		String statusString = null;
		
		if (currentCaseIndex % (nTotalCases / nChecks + 1) == 0) {
			double elapsedTimeSeconds = (System.currentTimeMillis() - startTimeMillis) / 1000.0;
			double remainingTimeSeconds = elapsedTimeSeconds / (currentCaseIndex + 1) * (nTotalCases - currentCaseIndex - 1);
			double remainingTimeMinutes = remainingTimeSeconds / 60.0;
			
			
			statusString = currentCaseIndex + "/" + nTotalCases + " = " + FunTricks.fitInNCharacters(100 * (currentCaseIndex + 1.0)/nTotalCases, 5) 
			+ "% at " + FunTricks.onlySomeDecimalPlaces(elapsedTimeSeconds, 1) + "; to go = " + FunTricks.onlySomeDecimalPlaces(remainingTimeSeconds, 1) + "s/"
			+ FunTricks.onlySomeDecimalPlaces(remainingTimeMinutes, 1) + "m";
		}
