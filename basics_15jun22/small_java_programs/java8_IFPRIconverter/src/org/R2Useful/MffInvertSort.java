...
public class MffInvertSort {
  ...
  public static MultiFormatFloat InvertSortMFF(
      MultiFormatFloat InputMatrix, int[] RowList, long nElementsThreshold) throws Exception {

    // "MffInvertSort: This module inverts the sorting of the module MffSortComb11. That is, a 1-d
    // integer array " +
    //      		"is created by MffSortComb11 which contains the original row index. This module puts
    // the rows " +
    ...
    int formatIndex = -1;
    if (nElements
        < nElementsThreshold) { // small means keep it in core; single dimensional in memory is best
    ...
    for (int rowIndex = 0; rowIndex < nRows; rowIndex++) {
      ...
      for (int colIndex = 0; colIndex < nCols; colIndex++) {
        ...
        // copy them over
        reorderedMatrix.setValue(setIndices, InputMatrix.getValue(getIndices));
      }
    }
