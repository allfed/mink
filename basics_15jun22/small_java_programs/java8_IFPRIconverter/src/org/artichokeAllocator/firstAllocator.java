...
public class firstAllocator {
  ...
  public static void main(String commandLineOptions[]) throws Exception {
    ...
    // (DMR) Because these were specific to Ricky's computer, they're now commented out. Were causing issues with reformatting. This is the old code:
    /*
    ...
    if (true) {
        ...
        outputBasename = "D:\\rdrobert\\global_futures\\toy_land_use\\java_allocator\\to_GRASS\\ALLOCATEDs_irrigated_TMM_THA";
    } else {
        cropFootprintsFile =
    ...
    */
    if (true) {
      ...
      outputBasename = ""; // again, let's just supply this from the outside world...
    }
    ...
    */
    ...
    MultiFormatMatrix pixelAreaAvailable =
        MatrixOperations.getSubmatrix(allPixelData, 0, -1, (2 * nCrops), (2 * nCrops), 1);
    ...
    MultiFormatMatrix pixelFractions =
        MatrixOperations.getSubmatrix(allPixelData, 0, -1, 0, (nCrops - 1), 1);
    ...
    MultiFormatMatrix pixelRevenueLike =
        MatrixOperations.getSubmatrix(allPixelData, 0, -1, nCrops, (2 * nCrops - 1), 1);
    ...
    while (maxSurplus > surplusTolerance) {
      ...
      if (highestRevenueHere == lowestRevenueHere) {
        ...
        randomColumnMFM =
            MatrixOperations.generateUniformRandom(
                nPixels, 1, randomSeed, 1); // Beware the MAGIC NUMBER!!! only one column needed
        //                  MultiFormatFloat randomColumnMFF =
        // MatrixOperations.MFMtoMFF(randomColumnMFM);
        revenueSortingObject =
            new SortComb11MFM(
                randomColumnMFM, 0, verboseSorting); // Beware the MAGIC NUMBER!!! only one column
        revenueSortingObject.doSorting();
      ...
      // what we want to do is start with the lowest revenue (or fraction [or random]) pixels and
      // peel off any area there
      ...
      for (int inIndexArrayIndex = 0; inIndexArrayIndex < nPixels; inIndexArrayIndex++) {
        ...
        bountifulCropAreaToDrop =
            originalFractions[maxSurplusIndex]
        ...
        if (bountifulCropAreaToDrop < remainingSurplus) {
          ...
          // apparently the 1.0 minus the one i want to drop is not working properly because of
          // rounding errors...
          ...
          //                  System.out.println(" total = " + thisTotalOldFractionsNonMax + " ?=? +
          // " + (1.0 - originalFractions[maxSurplusIndex]) + " complement");
          ...
          for (int cropIndex = 0; cropIndex < nCrops; cropIndex++) {
            ...
            newFractions[cropIndex] =
                originalFractions[cropIndex] * probabilityComplementMultiplier;
          }
          ...
          //                  probabilityComplementMultiplier = (1.0 - fractionToKeep) / (1.0 -
          // originalFractions[maxSurplusIndex]);
        ...
        //              System.out.println(" remaining surplus = " + remainingSurplus + " pixel #" +
        // inIndexArrayIndex);
        ...
        for (int cropIndex = 0; cropIndex < nCrops; cropIndex++) {
          ...
          finalPixelFractions.setValue(
              pixelIndexInRealTables, cropIndex, (float) newFractions[cropIndex]);
        ...
        if (Math.abs(pixelTotalFractionCheck - 1.0) > 0.00000001) {
          ...
          System.out.println(
              "probabilityComplementMultiplier = " + probabilityComplementMultiplier);
          for (int cropIndex = 0; cropIndex < nCrops; cropIndex++) {
            if (cropIndex == maxSurplusIndex) {
              System.out.println(
                  "["
