...
//		System.out.println("the most overallocated crop is " + cropNamesArray[maxSurplusIndex] + " with " + maxSurplus + " extra ha");
	...
	private static boolean fractionCheckIfProblems(double pixelTotalFractionCheck, int inIndexArrayIndex, int pixelIndexInRealTables,
			double thisTotalOldFractionsNonMax, double probabilityNumerator, double probabilityDenominator, double[] originalFractions,
			double[] newFractions, int nCrops, int maxSurplusIndex) {
...
//	if (Math.abs(pixelTotalFractionCheck - 1.0) > 0.00000001) {
		if (Math.abs(pixelTotalFractionCheck - 1.0) > 0.00000001 || Double.isNaN(pixelTotalFractionCheck)) {
...
//			System.out.println("probabilityComplementMultiplier = " + probabilityComplementMultiplier);
	...
	private static MultiFormatMatrix rationalizeFractions(MultiFormatMatrix pixelFractions, MultiFormatMatrix pixelAreaAvailable, boolean verboseFlag) throws Exception {
		...
		MultiFormatMatrix newPixelFractions = MatrixOperations.generateConstant(nPixels, nCrops, 0.0, 1);
		...
		for (int pixelIndex = 0; pixelIndex < nPixels; pixelIndex++) {
			...
			if (pixelAreaAvailable.getValue(pixelIndex,0) <= 0.0) {
				...
				for (int cropIndex = 0; cropIndex < nCrops; cropIndex++) {
//					newPixelFractions.setValue(pixelIndex,cropIndex, -1.0 * pixelFractions.getValue(pixelIndex,cropIndex));
			...
			if (fractionLacking > 0) {
				...
				newPixelFractions.setValue(pixelIndex,lowestFractionIndex, lowestFraction + fractionLacking);
				nBoosted++;
				if (verboseFlag) {
					System.out.println("p " + pixelIndex + " fl = " + fractionLacking + " lfi = " + lowestFractionIndex + " lf = " + lowestFraction + " nv = " + (lowestFraction + fractionLacking));
				}
			} else if (fractionLacking < 0) {
				// we need to take a little bit away
				// rob from the rich
				newPixelFractions.setValue(pixelIndex,highestFractionIndex, highestFraction + fractionLacking);
				nRobbed++;
				if (verboseFlag) {
					System.out.println("p " + pixelIndex + " fl = " + fractionLacking + " hfi = " + highestFractionIndex + " hf = " + highestFraction + " nv = " + (highestFraction + fractionLacking));
				}
			}
		}

		System.out.println("nBoosted = " + nBoosted + " nRobbed = " + nRobbed + " nNoArea = " + nNoArea + " % changed: " + (nBoosted * 1.0 + nRobbed) * 100.0 / nPixels);

		return newPixelFractions;
	}

	private static double[] agnosticRedistribution(double newFractionForMaxSurplusCrop, int pixelIndexInRealTables, int maxSurplusIndex, MultiFormatMatrix pixelRevenueLike) throws Exception {
		...
		// let us see if a non-informative prior will move us along differently than using the revenues...
//		if (true) {
		if (totalRelevantRevenues == 0.0) {
			...
			for (int cropIndex = 0; cropIndex < nCrops; cropIndex++) {
				if (cropIndex == maxSurplusIndex) {
					...
					newFractions[cropIndex] = amountToDistribute * 1.0 / (nCrops - 1.0); // we don't want the crop we just cleaned out...
				}
			}
		} else {
			...
			for (int cropIndex = 0; cropIndex < nCrops; cropIndex++) {
				if (cropIndex == maxSurplusIndex) {
					...
					newFractions[cropIndex] = amountToDistribute * pixelRevenueLike.getValue(pixelIndexInRealTables,cropIndex) / totalRelevantRevenues; 
				}
			}
		}
	...
	private static double[] useScalingReduction(MultiFormatMatrix pixelFractions, MultiFormatMatrix pixelRevenueLike, MultiFormatMatrix pixelAreaAvailable,
			int pixelIndex, int maxSurplusIndex, double multiplierNeededToReduceToTarget) throws Exception {
...
//			newFractions[cropIndex] = 0.0;
		...
		bountifulCropAreaToDrop = originalFractions[maxSurplusIndex] * pixelAreaAvailable.getValue(pixelIndex,0);
		...
		// now we need to reduce the area dedicated to our surplus crop and then re-allocate it to everything else....
		newFractionForMaxSurplusCrop = multiplierNeededToReduceToTarget * originalFractions[maxSurplusIndex];
//		altNewFractionForMaxSurplusCrop = multiplierNumerator * originalFractions[maxSurplusIndex] / multiplierDenominator;
		
//		if (newFractionForMaxSurplusCrop - altNewFractionForMaxSurplusCrop != 0) {
//			System.out.println("nF = " + newFractionForMaxSurplusCrop + " != " + altNewFractionForMaxSurplusCrop + " aNF; area diff = " +
//					(pixelAreaAvailable.getValue(pixelIndex,0) * (newFractionForMaxSurplusCrop - altNewFractionForMaxSurplusCrop))
//			);
//			
//		}
		...
		// it could be that everything gets piled up, so we need to be clever about how to reduce in that case...
		// this is another agnostic situation...
		if (probabilityDenominator == 0.0) {

			// do the agnostic redistribution; but based loosely on the revenue-like values...
			newFractions = agnosticRedistribution( newFractionForMaxSurplusCrop, pixelIndex, maxSurplusIndex, pixelRevenueLike);
			...
			if (nPositiveOriginalFractions > 1) {
				for (int cropIndex = 0; cropIndex < nCrops; cropIndex++) {
					if (cropIndex == maxSurplusIndex) {
						...
						newFractions[cropIndex] = probabilityNumerator * originalFractions[cropIndex] / probabilityDenominator;
					}
				}
			} else {
				newFractions = agnosticRedistribution( newFractionForMaxSurplusCrop, pixelIndex, maxSurplusIndex, pixelRevenueLike);
	...
	private static Object[] useSortingReduction(int[] sortedIndices, int inIndexArrayIndex,
			MultiFormatMatrix pixelFractions, MultiFormatMatrix pixelRevenueLike, MultiFormatMatrix pixelAreaAvailable,
			int maxSurplusIndex, double remainingSurplus) throws Exception {
...
//			newFractions[cropIndex] = 0.0;
		...
		if (thisTotalOldFractionsNonMax == 0) {
			...
			newFractions = agnosticRedistribution(fractionToKeep, pixelIndexInRealTables, maxSurplusIndex, pixelRevenueLike);
		} else {
			// do the normal redistribution
			// update to make new fractions	there are several ways we could do this. the easiest is to scale everything up...
			// make sure this matches with the way we're doing it for the partial case...
			for (int cropIndex = 0; cropIndex < nCrops; cropIndex++) {
				if (cropIndex == maxSurplusIndex) {
...
//		step six:	go back to step two, but rule out the crop(s) for which the correction has already been accomplished
		...
		MultiFormatMatrix pixelAreaAvailable = MatrixOperations.getSubmatrix(allPixelData, 0, -1, (2 * nCrops), (2 * nCrops), defaultFormatIndex);
		...
		MultiFormatMatrix pixelFractions = MatrixOperations.getSubmatrix(allPixelData, 0, -1, 0, (nCrops - 1), defaultFormatIndex);
...
//		MultiFormatMatrix finalPixelFractions = MatrixOperations.newCopy(pixelFractions);
		...
		MultiFormatMatrix pixelRevenueLike = MatrixOperations.getSubmatrix(allPixelData, 0, -1, nCrops, (2 * nCrops - 1), defaultFormatIndex);
	  ...
	  double probabilityDenominator = -4;
		...
		while (maxSurplus > surplusTolerance && allocationCounter < maximumIterations) {
			...
			if (!diversityOfRevenues) {
				...
				multiplierNeededToReduceToTarget = footprintsAsDoubles[maxSurplusIndex]/currentAllocations[maxSurplusIndex];
				System.out.println("   -> multiplier = " + multiplierNeededToReduceToTarget + " n = " +
						footprintsAsDoubles[maxSurplusIndex] + " d = " + currentAllocations[maxSurplusIndex] + " mS = " +
						maxSurplus);
...
//					newFractions = useScalingReduction(finalPixelFractions,
					newFractions = useScalingReduction(pixelFractions,
							pixelRevenueLike, pixelAreaAvailable, pixelIndex,
							maxSurplusIndex, multiplierNeededToReduceToTarget);
//					,
...
//					);
					...
					for (int cropIndex = 0; cropIndex < nCrops; cropIndex++) {
						if (newFractions[cropIndex] < 0 || newFractions[cropIndex] > 1) {
							System.out.println("EVEN REDUCTION bad fraction: " + newFractions[cropIndex] + " realPixelIndex = " + pixelIndex + " cropIndex = " + cropIndex);
						}
//						originalFractions[cropIndex] = finalPixelFractions.getValue(pixelIndex,cropIndex);
						originalFractions[cropIndex] = pixelFractions.getValue(pixelIndex,cropIndex);
//						finalPixelFractions.setValue(pixelIndex, cropIndex, newFractions[cropIndex]);
					...
					// some idiot checking for the moment... this is only checking on pixels we mess with
					// note that we are going straight through, so the "in array" index and the "real" index are the same...
					fractionCheckIfProblems(pixelTotalFractionCheck, pixelIndex, pixelIndex,
							thisTotalOldFractionsNonMax, probabilityNumerator, probabilityDenominator, originalFractions,
							newFractions, nCrops, maxSurplusIndex);
...
//				MatrixOperations.getSubmatrix(finalPixelFractions, 0, -1, maxSurplusIndex, maxSurplusIndex, defaultFormatIndex)
				previousAllocationThisCrop = MatrixOperations.multiplyMatricesElementwise(pixelAreaAvailable,
						MatrixOperations.getSubmatrix(pixelFractions, 0, -1, maxSurplusIndex, maxSurplusIndex, defaultFormatIndex)
						);
				
				
				// now, we want to sort on the previous allocation (which has but a single column) for that crop...
...
//					 finalPixelFractions,  pixelRevenueLike,  pixelAreaAvailable,
					sortingResultBundle = useSortingReduction(sortedIndices,  inIndexArrayIndex,
							 pixelFractions,  pixelRevenueLike,  pixelAreaAvailable,
							 maxSurplusIndex,  remainingSurplus);
					...
					for (int cropIndex = 0; cropIndex < nCrops; cropIndex++) {
						if (newFractions[cropIndex] < 0 || newFractions[cropIndex] > 1) {
							System.out.println("SORTING bad fraction: " + newFractions[cropIndex] + " for sortedIndex = " + inIndexArrayIndex + " realPixelIndex = " + pixelIndexInRealTables + " cropIndex = " + cropIndex);
						}
//						originalFractions[cropIndex] = finalPixelFractions.getValue(pixelIndexInRealTables,cropIndex);
						originalFractions[cropIndex] = pixelFractions.getValue(pixelIndexInRealTables,cropIndex);
//						finalPixelFractions.setValue(pixelIndexInRealTables, cropIndex, newFractions[cropIndex]);
						pixelFractions.setValue(pixelIndexInRealTables, cropIndex, newFractions[cropIndex]);
						pixelTotalFractionCheck += newFractions[cropIndex];
					}
					
					// some idiot checking for the moment... this is only checking on pixels we mess with
					boolean thereWereProblems = fractionCheckIfProblems(pixelTotalFractionCheck, inIndexArrayIndex, pixelIndexInRealTables,
							thisTotalOldFractionsNonMax, probabilityNumerator, probabilityDenominator, originalFractions,
							newFractions, nCrops, maxSurplusIndex);
					
					if (thereWereProblems) {
						System.out.println("Area Available above was = " + pixelAreaAvailable.getValue(pixelIndexInRealTables,0));
					}
...
//					finalPixelFractions.finishRecordingMatrix();
			...
			FunTricks.appendLineToTextFile(allocationCounter + "\t" + maxSurplus + "\t" + maxSurplusIndex, solvePathFilename, true);
...
//			currentAllocations = determineTotalAllocations(finalPixelFractions, pixelAreaAvailable);
			...
			System.out.println("our most overallocated crop name is " + cropNamesArray[maxSurplusIndex] +
					" mSI = " + maxSurplusIndex + " mS = " + maxSurplus + " ");
...
//				MatrixOperations.write2DMFMtoText(pixelFractions, outputBasename + (allocationCounter % 3), "\t");
//			}
