...
public class TimThomasCSVtoWTH {
    ...
    static private int numberOfDaysBeforeMonthNumber(int monthNumber, int yearNumber) {
	...
	if (monthNumber == 1) {
	    ...
	    if (yearNumber % 4 == 0) { return 60; } else { return 59; }
	} else if (monthNumber == 4) {
	    if (yearNumber % 4 == 0) { return 91; } else { return 90; }
	} else if (monthNumber == 5) {
	    if (yearNumber % 4 == 0) { return 121; } else { return 120; }
	} else if (monthNumber == 6) {
	    if (yearNumber % 4 == 0) { return 152; } else { return 151; }
	} else if (monthNumber == 7) {
	    if (yearNumber % 4 == 0) { return 182; } else { return 181; }
	} else if (monthNumber == 8) {
	    if (yearNumber % 4 == 0) { return 213; } else { return 212; }
	} else if (monthNumber == 9) {
	    if (yearNumber % 4 == 0) { return 244; } else { return 243; }
	} else if (monthNumber == 10) {
	    if (yearNumber % 4 == 0) { return 274; } else { return 273; }
	} else if (monthNumber == 11) {
	    if (yearNumber % 4 == 0) { return 305; } else { return 304; }
	} else if (monthNumber == 12) {
	    if (yearNumber % 4 == 0) { return 335; } else { return 334; }
	} else {
	    return -1;
	}
	...
	if (!outputFileObject.exists()) {
	    System.out.println("intended output [" + outputFileObject.getCanonicalPath() + "] has failed to exist.");
	    throw new Exception();
	}
	...
	for (int fileIndex = 0; fileIndex < inputFiles.length; fileIndex++) {
		...
		// look at the very first thing (longitude) and if it is not parsable as a double then we bail out
		// figure out the day we're looking at...
		try {
		    longitude = Double.parseDouble(splitLine[longitudeColumn]);
		} catch (Exception eee) {
		    // we have hit something we can't read. let us skip over it because it should be a header/footer line
		...
		if (Double.isNaN(previousLatitude)) {
		    ...
		    System.out.println("initializing what is probably the very first line: pLat = " + previousLatitude + "; pLon = " + previousLongitude + "; lI = " + lineIndex);
		}

//		System.out.println("lI= " + lineIndex + "\tpLat=" + previousLatitude + "\tlat=" + latitude + "\tpLon=" + previousLongitude + "\tlon=" + longitude);
		if (previousLongitude != longitude || previousLatitude != latitude) {
		    ...
		    outputHeaderAsArray[3] = "  XXXX   " + FunTricks.fitInNCharacters(previousLatitude, 6) + "   " +
			    FunTricks.fitInNCharacters(previousLongitude, 6) + "  -999 " +
		    ...
		    destinationFile = destinationPath + File.separator + inputFiles[fileIndex].getName().replaceFirst(sourceSuffix, "") + "_" + previousLatitude + "_" + previousLongitude + weatherSuffix;

		    FunTricks.writeStringArrayToFile(finalCombinedOutputArray, destinationFile);

		    System.out.println("-----> finished writing " + destinationFile + "; nGoodDays = " + nGoodDays);
		...
		outputHeaderAsArray[1] = "";
		outputHeaderAsArray[2] = "@ INSI      LAT     LONG  ELEV   TAV   AMP REFHT WNDHT";
		outputHeaderAsArray[3] = "  XXXX   " + FunTricks.fitInNCharacters(latitude, 6) + "   " +
			FunTricks.fitInNCharacters(longitude, 6) + "  -999 " +
			FunTricks.fitInNCharacters(tav, 5) + " " +
			FunTricks.fitInNCharacters(amp, 5) + "  -999  -999";
		outputHeaderAsArray[4] = "@DATE  SRAD  TMAX  TMIN  RAIN   PAR  CO2D";
		...
		// get rid of the suffix and build the latitude and longitude into the filename
		destinationFile = destinationPath + File.separator + inputFiles[fileIndex].getName().replaceFirst(sourceSuffix, "") + "_" + latitude + "_" + longitude + weatherSuffix;
