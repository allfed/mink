...
//    static final private String magicOverviewRunBreak = "**************************************************************************************************************";
    ...
    public Mink3classic(String initFileNameToUse) throws IOException, Exception {
	...
	// declarations
	int plantingDate = -1, eventDate = -2, daysSincePlantingForEvent = -3, yieldToUse = -3;
	...
	for (int fakeYearIndex = 0; fakeYearIndex < nYears; fakeYearIndex++) {
		...
		// so, we'll check for how many values we have and then skip if it doesn't match up...

		//				if (splitLine.length == magicNumberOfOutputsExpected) {
		// doing a quick modification to handle/ignore official 4.6 style outputs that have two extra bits of info at the end... 
		if (splitLine.length >= magicNumberOfOutputsExpected) {
		    skipExtras = false;
		    for (int extraIndex = 0; extraIndex < extraNames.length; extraIndex++) {
		    ...
		    System.out.println("REAL: got " + splitLine.length + " rather than expected " + magicNumberOfOutputsExpected + 
			    " so skipping; (data line " + lineIndex + "; fakeYear " + fakeYearIndex + " planting window index " + plantingWindowIndex + "):");
		    realBadThingsCountsEntirePixel.useLongValue(1); // assess that something bad happened...
		...
		// another adjustment to handle the extra couple bits of info from new/real 4.6
		for (int extraIndex = 0;
			extraIndex < 
			Math.min(splitLine.length - magicOffsetForExtraNamesOutputIndex,extraNames.length);
			extraIndex++) {
...
//		    }
    ...
    static private Object[] countGrowthStages(String[] overviewFileArray) {
	...
	int magicHugeMaxNumberOfStages = 12; // i'm going to be lame and just initialize
...
//		int magicLengthOfGrowthStageNameAndTimeSpan = 30;
		...
		paddedGrowthStageName = magicExtraPrefixForGrowthStages + growthStagesFound + "_" 
...
//	    System.out.println("      #### at bottom; startLineIndex=" + startLineIndex + " ; length = " + overviewFileArray.length + " ; nFound = " + nGroupsFound + " ####");
    ...
    private DescriptiveStatisticsUtility[][] grabOverviewResults(DescriptiveStatisticsUtility[][] growthStageStressAccumulators, String overviewToTry) throws IOException, Exception {
	
//	String overviewToTry = magicOverviewToRead;
//	File overviewTestFile = new File(overviewToTry); // assuming it is in the current working directory, of course
//	
//	boolean weShouldBotherParsing = false;
...
//	    // and try this one....
//	    if (overviewTestFile.exists()) {
//		weShouldBotherParsing = true;
//	    }
//	} else {
//	    weShouldBotherParsing = true;
//	}
//	
//	if (!weShouldBotherParsing) {
//	    return null;
//	}
	...
	DescriptiveStatisticsUtility[][] updatedGrowthStageStressAccumulators = extractStressIndicesWpsWgNpsNg(
		overviewAsArray, growthStageNames, waterSynthStartIndexToUse, nitrogenSynthStartIndexToUse, growthStageStressAccumulators
		);

	
	return updatedGrowthStageStressAccumulators;
...
//			initializationBlock;
		...
		// OVERVIEW.OUT setup
		// on the very first line, we'll need to be clever and set everything up. after that, we'll just re-use...
		if (lineIndex == 0) {
...
//				statisticsOutLine += delimiter + stressAccumulators[growthStageIndex][stressIndexIndex].getMean();
		...
		statisticsOutLine += delimiter + (realTimeToPlantingEntirePixel.getMean() - this.spinUpTimeDays)
			...
			statisticsOutLine += delimiter + extraSummaryAccumulators[magicHWAHindex].getStd();
...
//		    stressAccumulators[growthStageWithinBlockLineIndex][0].useDoubleValue(waterPhotosynthesisStress);    // water/photosynthesis
//		    stressAccumulators[growthStageWithinBlockLineIndex][1].useDoubleValue(waterGrowthStress);            // water/growth
//		    stressAccumulators[growthStageWithinBlockLineIndex][2].useDoubleValue(nitrogenPhotosynthesisStress); // nitrogen/photosynthesis
//		    stressAccumulators[growthStageWithinBlockLineIndex][3].useDoubleValue(nitrogenGrowthStress);         // nitrogen/growth
