...
//	int lastYear = -2;
	...
	// write out the names of the columns for future reference....
	    ...
	    giantOutputLine = 
		    ...
		    "longitude" + delimiter +
		    "annualSW" + delimiter +
		    "annualTmax" + delimiter +
		    "annualTmin" + delimiter +
		    "annualTave" + delimiter +
		    "annualThalfamplitude" + delimiter +
		    "annualPrecTotal"  + delimiter;
	    ;
	    ...
	    // and some provenance...
	    FunTricks.appendLineToTextFile("sourceDirectory = [" + sourceDirectory + "]", outputBaseName + ".cols.txt", true);
	    FunTricks.appendLineToTextFile("outputBaseName = [" + outputBaseName + "]", outputBaseName + ".cols.txt", true);
	    FunTricks.appendLineToTextFile("rainyDayMinimumRainfallThreshold = [" + rainyDayMinimumRainfallThreshold + "]", outputBaseName + ".cols.txt", true);
	...
	for (int fileIndex = 0; fileIndex < inputFiles.length; fileIndex++) {
     ...
     0         1         2         3         4
		...
		for (int pieceIndex = 0 ; pieceIndex < lineThreeInPieces.length; pieceIndex++) {
		    System.out.println("lineThreeInPieces[" + pieceIndex + "] = [" + lineThreeInPieces[pieceIndex] + "]");
		}
...
//		    if (yearInFile > lastYear)  { lastYear  = yearInFile; }
		...
		} // (int lineIndex = magicFirstLineToWorkOn; lineIndex < originalAsArray.length; lineIndex++)
	    } // (fileIndex == 0)
		...
		if (currentYearBeingRead != yearInFile) {
		    ...
		    FunTricks.appendLineToTextFile(giantOutputLine, outputBaseName + "_" + currentYearBeingRead + ".txt", true);
