...
//	final int rainEnd = 29;
	...
	for (int splitIndex = 0; splitIndex < thresholdSplit.length; splitIndex++) {
		...
		System.out.println("thiThresholds[" + storageNumber + "] = " + thiThresholds[storageNumber]);
		...
		if (storageNumber > 0 && thiThresholds[storageNumber] < thiThresholds[storageNumber - 1]) {
		    ...
		    System.out.println("(storageNumber > 0 && thiThresholds[storageNumber] < thiThresholds[storageNumber - 1])");
		    System.out.println("(" + storageNumber + "> 0 && " + thiThresholds[storageNumber] + " < " + thiThresholds[storageNumber - 1] + ")");
		    throw new Exception();
		}
	...
	for (int lineIndex = magicFirstLineToWorkOn; lineIndex < originalAsArray.length; lineIndex++) {
		...
		FunTricks.appendLineToTextFile("thresholdsCSV = [" + thresholdsCSV + "]", particularProvenanceFilename, true);
	    }
...
//		    System.out.println("    bI " + binIndex + ": THI = " + approximateTHItoday + " ; thresh = " + thiThresholds[binIndex]);
		    if (approximateTHItoday < thiThresholds[binIndex]) {
			// the thresholds are supposed to be in order, so if we are under the threshold being tested
			// it means that we have found the first threshold that is bigger than our value
			// so, let's put it in that bin...
//			System.out.println("       inside: we found something; old count = " + daysBelowThreshold[binIndex]);
		...
		if (!foundABin) {
//			System.out.println("       bigger: we found something; old count = " + daysBelowThreshold[nThresholds]);
