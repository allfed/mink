...
public class WTHcleaner {
	...
	public static void main(String commandLineOptions[]) throws Exception {
		...
		double sradMinimumThreshold             = Double.parseDouble(commandLineOptions[3]);
		double tDiffToEnforce                   = Double.parseDouble(commandLineOptions[4]);
		...
		for (int fileIndex = 0; fileIndex < inputFiles.length; fileIndex++) {
			...
			destinationFile = destinationPath + File.separator + "concentrated_" + rainyDayMinimumRainfallThreshold + "_" + inputFiles[fileIndex].getName();
    ...
    /*
     ...
     0         1         2         3         4
	    */
		...
		// the plan is to read through all the days and look at the rainfall. if the rainfall is below threshold, we
		// keep accumulating it up until it is enough to be worthwhile and then dump it all out at that point.
