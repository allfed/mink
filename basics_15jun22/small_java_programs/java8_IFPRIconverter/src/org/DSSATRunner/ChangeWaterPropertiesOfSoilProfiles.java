...
public class ChangeWaterPropertiesOfSoilProfiles {
  ...
  public static void main(String commandLineOptions[]) throws Exception {
    ...
    int maxNameLength =
        10; // the maximum length for the profile name. it might actually be 12, but i don't know...
    ...
    if (totalWaterMultiplier < 0) {
      System.out.println(
          "water multiplier must be non-negative (i.e., not " + totalWaterMultiplier + ")");
      throw new Exception();
    }

    // all right: let's try to build the original soil profile object

    SoilProfileModifyExisting collectionOfProfiles =
        new SoilProfileModifyExisting(originalSoilFile);

    // and we will need to rename the soil profiles. traditionally, the first two letters of
    // the soil profile name match the first two letters of the soil file name (which should be
    // ??.SOL)
    // ok, and i will brute force some path stripping...
    File newSoilFileObject = new File(newSoilFile);
    String firstTwoLettersOffileNameWithoutDirectories =
        newSoilFileObject.getName().substring(0, 2);
    ...
    for (int profileIndex = 0; profileIndex < collectionOfProfiles.getNProfiles(); profileIndex++) {
      ...
      if (possiblyDuplicateProfileIndex >= 0) {
        ...
        if (newName.length() > maxNameLength) {
          System.out.println(
              "Cannot adequately rename profile, fallback of pre-pending failed (too long).");
          System.out.println("old name: " + oldName);
          System.out.println("new name: " + newName);
          System.out.println(
              "first try conflicts with existing profile index "
                  + possiblyDuplicateProfileIndex
                  + "...");
        ...
        if (possiblyDuplicateProfileIndex >= 0) {
          System.out.println(
              "Cannot adequately rename profile, fallback of pre-pending failed (both names already"
                  + " taken).");
          System.out.println("old name: " + oldName);
          System.out.println("new name: " + newName);
          System.out.println(
              "final try conflicts with existing profile index "
                  + possiblyDuplicateProfileIndex
                  + "...");
          System.out.println("with: " + oldProfileNames[possiblyDuplicateProfileIndex]);
      ...
      for (int layerIndex = 0; layerIndex < nLayersHere; layerIndex++) {
        oldDrainedUpper =
            collectionOfProfiles.getDrainedUpperLimit_SDUL()[profileIndex][layerIndex];
        ...
        if (changeUpperFlag && changeLowerFlag) {
          // we'll split the difference. so, find the middle of the originals, then go up and down
          // by half of the new range.
        ...
        // ok, and idiot check to make sure that they are are in the order {0 , new lower , new
        // upper , saturated}.
        if (newLower < 0.0F) {
          newLower = 0.0F;
        }
        ...
        if (newLower
            > collectionOfProfiles.getSaturatedUpperLimit_SSAT()[profileIndex][layerIndex]) {
