...
public class FSMiddleHeavyThreeSplitWithFlowering implements NitrogenOnlyFertilizerScheme {
  ...
  public String buildNitrogenOnlyBlock(
    ...
    daysAfterPlantingForApplication[1] =
        Math.max(
            Math.round(
                25.0F / 67.0F * (daysAfterPlantingAnthesis - magicDaysToAnticipateFlowering)),
            daysAfterPlantingForApplication[0] + 1);
    rateAppliedKgPerHa[1] = (int) (nitrogenFertilizerAmount * 0.5);
    daysAfterPlantingForApplication[2] =
        Math.max(
            daysAfterPlantingAnthesis - magicDaysToAnticipateFlowering,
            daysAfterPlantingForApplication[1] + 1);
