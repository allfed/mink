...
public class Mink3p3searchclassic {
    ...
    // for some reason, the Aflatoxin.Out file sticks around and is just added onto which tends to fill up the filesystem
    // so, i am now (03may18) making a list of files to clear out every time, just to be sure....
    public static final String[] magicListOfFilesToClearOut = { "Aflatoxin.Out" , "Hourly_Soil_Temp.Out" , "AfConcentration.OUT" , "MovingAv.OUT" }; // new in 3p3
    ...
    // some key words to mark the beginnings and ends of lists in the initialization file that we will be searching over
    private static final String snxCodeValueListStarter = "SNX CODE VALUE LIST";
    private static final String culCodeValueListStarter ="CUL CODE VALUE LIST";
    private static final String ecoCodeValueListStarter ="ECO CODE VALUE LIST";
    private static final String speCodeValueListStarter ="SPE CODE VALUE LIST";
    private static final String endCodeValueListMarker  ="END VALUE LISTS";

    private static final String searchValuesDelimiter = ",";
    ...
    private String  cultivarFileToUse      = null;
    private String  cultivarFileToPut      = null;
    private String  ecotypeFileToUse       = null;
    private String  ecotypeFileToPut       = null;
    private String  speciesFileToUse       = null;
    private String  speciesFileToPut       = null;

    private String templateXFileContents = null;
    private String templateCultivarFileContents = null;
    private String templateEcotypeFileContents  = null;
    private String templateSpeciesFileContents  = null;

    private String[] xFileSearchCodes = null;
    private String[] cultivarFileSearchCodes = null;
    private String[] ecotypeFileSearchCodes = null;
    private String[] speciesFileSearchCodes = null;

    private String[][] xFileSearchValues = null;
    private String[][] cultivarFileSearchValues = null;
    private String[][] ecotypeFileSearchValues = null;
    private String[][] speciesFileSearchValues = null;
    ...
    private File[] magicListOfFilesToClearOutAsObjects = null;
...
//    static final private String magicOverviewRunBreak = "**************************************************************************************************************";
    ...
    public Mink3p3searchclassic(String initFileNameToUse) throws IOException, Exception {
	...
	cultivarFileToUse      = initFileContents[storageIndex++];
	cultivarFileToPut      = initFileContents[storageIndex++];
	ecotypeFileToUse       = initFileContents[storageIndex++];
	ecotypeFileToPut       = initFileContents[storageIndex++];
	speciesFileToUse       = initFileContents[storageIndex++];
	speciesFileToPut       = initFileContents[storageIndex++];
	...
	// next come the lists of search-and-replace codes along with their values lists
	// read the next line and see if it is the starting marcker
	String nextLine = initFileContents[storageIndex++];
	if ( !nextLine.equals(snxCodeValueListStarter)) {
	    System.out.println("expecting [" + snxCodeValueListStarter + "], found [" + nextLine + "]");
	    throw new Exception();
	}
	
	// ok, now we need to march through until we hit the next set of values....
	// but, we don't know how many there are, so let's mark this index so we can come back to it
	int topOfSearchValuesLineIndex = storageIndex;
	

	// initialize the next line
	nextLine = initFileContents[storageIndex++];

	
	while (!nextLine.equals(culCodeValueListStarter)) {
	    // look at the next line
	    nextLine = initFileContents[storageIndex++];
	}
	
	// ok, so now we should be at the end of that block
	int botOfSearchValuesLineIndex = storageIndex;


	// i think we want to subtract off an extra one so that we don't keep the separator line thingee
	int nParametersToSearch = botOfSearchValuesLineIndex - topOfSearchValuesLineIndex - 1;
	// initialize and then we'll fill it in...
	xFileSearchCodes = new String[nParametersToSearch];
	xFileSearchValues = new String[nParametersToSearch][];

	// run through the block again and interpret...
	// figure out how many entries there are..
	int nValues = -1;
	String[] splitLine = null;
	for (int lineIndex = 0 ; lineIndex < nParametersToSearch ; lineIndex++) {
	    // grab the line we want
	    splitLine = initFileContents[lineIndex + topOfSearchValuesLineIndex].split(searchValuesDelimiter);
	    
	    // Beware the MAGIC NUMBER!!! we are assuming that the first entry is a search and replace code
	    // and the rest are values to search over. so the number is one less than that. of course, this
	    // can and will be irregular: fortunately, java can handle that. :)
	    
	    
	    xFileSearchCodes[lineIndex] = splitLine[0];
	    
	    // now initialize and cycle through the values
	    nValues = splitLine.length - 1;
	    
	    xFileSearchValues[lineIndex] = new String[nValues];

	    for (int valueIndex = 0 ; valueIndex < nValues ; valueIndex++) {
		// Beware the MAGIC NUMBER!!! the 1 is to move past the label and into the particular values we want to try
		xFileSearchValues[lineIndex][valueIndex] = splitLine[1 + valueIndex];
	    } // valueIndex
	} // lineIndex

	// carry on to the next set....
	// once we get here, the storageIndex should be on the cultivar code value list marker

	
	/////////////////////////////////////////
	/////// Cultivar File Search Stuff //////
	/////////////////////////////////////////
	
	
	// ok, now we need to march through until we hit the next set of values....
	// but, we don't know how many there are, so let's mark this index so we can come back to it
	topOfSearchValuesLineIndex = storageIndex;
	
	
	while (!nextLine.equals(ecoCodeValueListStarter)) {
	    // look at the next line
	    nextLine = initFileContents[storageIndex++];
	}
	
	// ok, so now we should be at the end of that block
	botOfSearchValuesLineIndex = storageIndex;
	
	nParametersToSearch = botOfSearchValuesLineIndex - topOfSearchValuesLineIndex - 1;
	// initialize and then we'll fill it in...
	cultivarFileSearchCodes = new String[nParametersToSearch];
	cultivarFileSearchValues = new String[nParametersToSearch][];
	...
	for (int lineIndex = 0 ; lineIndex < nParametersToSearch ; lineIndex++) {
	    ...
	    cultivarFileSearchCodes[lineIndex] = splitLine[0];
	    
	    // now initialize and cycle through the values
	    nValues = splitLine.length - 1;
	    cultivarFileSearchValues[lineIndex] = new String[nValues];

	    for (int valueIndex = 0 ; valueIndex < nValues ; valueIndex++) {
		// Beware the MAGIC NUMBER!!! the 1 is to move past the label and into the particular values we want to try
		cultivarFileSearchValues[lineIndex][valueIndex] = splitLine[1 + valueIndex];
	    } // valueIndex
	} // lineIndex

	
	/////////////////////////////////////////
	/////// Ecotype File Search Stuff //////
	/////////////////////////////////////////
	...
	// this needs to be for the next block marker
	while (!nextLine.equals(speCodeValueListStarter)) {
	    // look at the next line
	    nextLine = initFileContents[storageIndex++];
	}
	...
	nParametersToSearch = botOfSearchValuesLineIndex - topOfSearchValuesLineIndex - 1;
	// initialize and then we'll fill it in...
	ecotypeFileSearchCodes = new String[nParametersToSearch];
	ecotypeFileSearchValues = new String[nParametersToSearch][];
	...
	for (int lineIndex = 0 ; lineIndex < nParametersToSearch ; lineIndex++) {
	    ...
	    ecotypeFileSearchCodes[lineIndex] = splitLine[0];
	    
	    // now initialize and cycle through the values
	    nValues = splitLine.length - 1;
	    ecotypeFileSearchValues[lineIndex] = new String[nValues];

	    for (int valueIndex = 0 ; valueIndex < nValues ; valueIndex++) {
		// Beware the MAGIC NUMBER!!! the 1 is to move past the label and into the particular values we want to try
		ecotypeFileSearchValues[lineIndex][valueIndex] = splitLine[1 + valueIndex];
	    } // valueIndex
	} // lineIndex
	...
	// this needs to be for the next block marker
	while (!nextLine.equals(endCodeValueListMarker)) {
	    // look at the next line
	    nextLine = initFileContents[storageIndex++];
	}
	...
	nParametersToSearch = botOfSearchValuesLineIndex - topOfSearchValuesLineIndex - 1;
	// initialize and then we'll fill it in...
	speciesFileSearchCodes = new String[nParametersToSearch];
	speciesFileSearchValues = new String[nParametersToSearch][];
	...
	for (int lineIndex = 0 ; lineIndex < nParametersToSearch ; lineIndex++) {
	    ...
	    speciesFileSearchCodes[lineIndex] = splitLine[0];
	    
	    // now initialize and cycle through the values
	    nValues = splitLine.length - 1;
	    speciesFileSearchValues[lineIndex] = new String[nValues];

	    for (int valueIndex = 0 ; valueIndex < nValues ; valueIndex++) {
		// Beware the MAGIC NUMBER!!! the 1 is to move past the label and into the particular values we want to try
		speciesFileSearchValues[lineIndex][valueIndex] = splitLine[1 + valueIndex];
	    } // valueIndex
	} // lineIndex

	
	
	// we should be all done until we decide to extend this some other way....
	
	///////////////////////////////////////
	// read in the whole template X file //
	// and the CUL/ECO/SPE templates     //
	///////////////////////////////////////
	templateXFileContents = FunTricks.readTextFileToString(templateXFile);
	templateCultivarFileContents = FunTricks.readTextFileToString(cultivarFileToUse);
	templateEcotypeFileContents  = FunTricks.readTextFileToString(ecotypeFileToUse);
	templateSpeciesFileContents  = FunTricks.readTextFileToString(speciesFileToUse);
	...
	// set up all the individual files for me to clear out
	magicListOfFilesToClearOutAsObjects = new File[magicListOfFilesToClearOut.length];
	for (int fileIndex = 0; fileIndex < magicListOfFilesToClearOut.length; fileIndex++) {
		magicListOfFilesToClearOutAsObjects[fileIndex] = new File(magicListOfFilesToClearOut[fileIndex]);
	}
...
//	String[] splitLine = null;
	...
	for (int fileIndex = 0; fileIndex < magicListOfFilesToClearOut.length ; fileIndex++) {
		provenanceOut.print("magicListOfFilesToClearOut[" + fileIndex + "] =\t\t\t"  + magicListOfFilesToClearOut[fileIndex] + "\n");
	}
	...
	provenanceOut.print("cultivarFileToUse " + cultivarFileToUse + "\n");
	provenanceOut.print("cultivarFileToPut " + cultivarFileToPut + "\n");
	provenanceOut.print("ecotypeFileToUse "  + ecotypeFileToUse + "\n");
	provenanceOut.print("ecotypeFileToPut "  + ecotypeFileToPut + "\n");
	provenanceOut.print("speciesFileToUse "  + speciesFileToUse + "\n");
	provenanceOut.print("speciesFileToPut "  + speciesFileToPut + "\n");
	
	provenanceOut.print("--- begin copy of template cultivar file [" + cultivarFileToUse + "] ---" + "\n");
	provenanceOut.print(templateCultivarFileContents);

	provenanceOut.print("--- begin copy of template ecotype file [" + ecotypeFileToUse + "] ---" + "\n");
	provenanceOut.print(templateEcotypeFileContents);

	provenanceOut.print("--- begin copy of template species file [" + speciesFileToUse + "] ---" + "\n");
	provenanceOut.print(templateSpeciesFileContents);
...
//		    }
    ...
    static private int[] giantIndexVectorFromSequenceIndex (long sequenceIndex, int[] nValuesByRow) {
	
	// build the block sizes
	long[] blockSizeHere = new long[nValuesByRow.length];
	blockSizeHere[0] = 1; // initialize the first one manually
	for (int rowIndex = 1 ; rowIndex < nValuesByRow.length ; rowIndex++) {
	    blockSizeHere[rowIndex] = blockSizeHere[rowIndex - 1] * nValuesByRow[rowIndex - 1];
	}

	// now, construct the indices for this sequential index, working backwards...
	long accountedForSoFar = 0; // actually needs to be zero
	int[] indexVector = new int[nValuesByRow.length];
	
	// remember, starting at the end and working backwards...
	for (int rowIndex = nValuesByRow.length - 1; rowIndex >= 0 ; rowIndex--) {
	    // how many full blocks came before?
	    indexVector[rowIndex] = (int) ((sequenceIndex - accountedForSoFar) / blockSizeHere[rowIndex]);
	    accountedForSoFar += indexVector[rowIndex] * blockSizeHere[rowIndex];
	}
    ...
    static private String[][] unpackGiantIndexVectorIntoFourBundlesOfValues(
		int[] giantIndexVector, String[][] xFileSearchValues, String[][] cultivarFileSearchValues, String[][] ecotypeFileSearchValues, String[][]speciesFileSearchValues
		) {
	
	String[][] xCulEcoSpeParticularValues = new String[4][];
	final int magicXrow   = 0;
	final int magicCulrow = 1;
	final int magicEcorow = 2;
	final int magicSperow = 3;
	
	
	// we will just march through and see what happens
	// we need to keep track of which index we are reading in the giant vector
	
	int readIndexInGiantVector = 0;
	
	xCulEcoSpeParticularValues[magicXrow] = new String[xFileSearchValues.length];
	for (int xIndex = 0 ; xIndex < xFileSearchValues.length ; xIndex++) {
	    xCulEcoSpeParticularValues[magicXrow][xIndex] = xFileSearchValues[xIndex][giantIndexVector[readIndexInGiantVector]];
	    // bump up the counter
	    readIndexInGiantVector++;
	}

	xCulEcoSpeParticularValues[magicCulrow] = new String[cultivarFileSearchValues.length];
	for (int culIndex = 0 ; culIndex < cultivarFileSearchValues.length ; culIndex++) {
	    xCulEcoSpeParticularValues[magicCulrow][culIndex] = cultivarFileSearchValues[culIndex][giantIndexVector[readIndexInGiantVector]];
	    // bump up the counter
	    readIndexInGiantVector++;
	}
	
	xCulEcoSpeParticularValues[magicEcorow] = new String[ecotypeFileSearchValues.length];
	for (int ecoIndex = 0 ; ecoIndex < ecotypeFileSearchValues.length ; ecoIndex++) {
	    xCulEcoSpeParticularValues[magicEcorow][ecoIndex] = ecotypeFileSearchValues[ecoIndex][giantIndexVector[readIndexInGiantVector]];
	    // bump up the counter
	    readIndexInGiantVector++;
	}
	
	xCulEcoSpeParticularValues[magicSperow] = new String[speciesFileSearchValues.length];
	for (int speIndex = 0 ; speIndex < speciesFileSearchValues.length ; speIndex++) {
	    xCulEcoSpeParticularValues[magicSperow][speIndex] = speciesFileSearchValues[speIndex][giantIndexVector[readIndexInGiantVector]];
	    // bump up the counter
	    readIndexInGiantVector++;
	}
	...
	return xCulEcoSpeParticularValues;
    }
...
//	    System.out.println("      #### at bottom; startLineIndex=" + startLineIndex + " ; length = " + overviewFileArray.length + " ; nFound = " + nGroupsFound + " ####");
    ...
    public void doSimulationsAndPregenerateWeatherExhaustiveSearching() throws Exception {
	...
	boolean happyWasWorseThanBestStressed = false;
...
//	Object[] growthStageInfoPacket = null;
	...
	// we need a set of "to use" combinations
	String[] xParametersToUse		= new String[xFileSearchValues.length];
	String[] cultivarFileParametersToUse	= new String[cultivarFileSearchValues.length];
	String[] ecotypeFileParametersToUse		= new String[ecotypeFileSearchValues.length];
	String[] speciesFileParametersToUse		= new String[speciesFileSearchValues.length];


	// we need to have a place to write down the winning values
	double  bestYieldDuringSearch = Double.NEGATIVE_INFINITY;
	double  anthesisOfBestYieldDuringSearch = -5.0;
	double  maturityOfBestYieldDuringSearch = -10.0;
	long bestSequenceIndex = -1;
	String[] xParametersBest		= new String[xFileSearchValues.length];
	String[] cultivarFileParametersBest	= new String[cultivarFileSearchValues.length];
	String[] ecotypeFileParametersBest	= new String[ecotypeFileSearchValues.length];
	String[] speciesFileParametersBest	= new String[speciesFileSearchValues.length];

	String cultivarContentsReadyToWrite = null;
	String ecotypeContentsReadyToWrite = null;
	String speciesContentsReadyToWrite = null;
...
//	    DescriptiveStatisticsUtility[][] growthStageStressAccumulators = null;
	...
	String XHappyInvariantsReplacedBeforeSearch = invariantsReplaced[0];
	String XInvariantsReplacedBeforeSearch      = invariantsReplaced[1];

	String XHappyInvariantsReplaced = null;
	String XInvariantsReplaced      = null;
	...
	// determine the total number of search cases we will be working with
	// just multiply everything together for how many there are in each parameter

	int[] giantIndexVector = null;
	String[][] fourBundlesOfStringValues = null;
	int[] nValuesByRow = new int[xFileSearchValues.length + cultivarFileSearchValues.length + ecotypeFileSearchValues.length + speciesFileSearchValues.length];

	long totalNumberOfSearchCases = 1;
	int storageIndex = 0;
	System.out.println("A: totalNumberOfSearchCases = " + totalNumberOfSearchCases);
	for (int xParameterIndex = 0 ; xParameterIndex < xFileSearchValues.length ; xParameterIndex++ ) {
	    // do the multiplication
	    System.out.println("xFSV[" + xParameterIndex + "] has " + xFileSearchValues[xParameterIndex].length + " elements");
	    totalNumberOfSearchCases *= xFileSearchValues[xParameterIndex].length;
	    // copy these counts of options into the single giant column for later interpretive use
	    nValuesByRow[storageIndex] = xFileSearchValues[xParameterIndex].length; storageIndex++;
	}
	System.out.println("B: totalNumberOfSearchCases = " + totalNumberOfSearchCases);
	for (int cultivarParameterIndex = 0 ; cultivarParameterIndex < cultivarFileSearchValues.length ; cultivarParameterIndex++ ) {
	    // do the multiplication
	    totalNumberOfSearchCases *= cultivarFileSearchValues[cultivarParameterIndex].length;
	    // copy these counts of options into the single giant column for later interpretive use
	    nValuesByRow[storageIndex] = cultivarFileSearchValues[cultivarParameterIndex].length; storageIndex++;
	}
	System.out.println("C: totalNumberOfSearchCases = " + totalNumberOfSearchCases);
	for (int ecotypeParameterIndex = 0 ; ecotypeParameterIndex < ecotypeFileSearchValues.length ; ecotypeParameterIndex++ ) {
	    // do the multiplication
	    totalNumberOfSearchCases *= ecotypeFileSearchValues[ecotypeParameterIndex].length;
	    // copy these counts of options into the single giant column for later interpretive use
	    nValuesByRow[storageIndex] = ecotypeFileSearchValues[ecotypeParameterIndex].length; storageIndex++;
	}
	System.out.println("D: totalNumberOfSearchCases = " + totalNumberOfSearchCases);
	for (int speciesParameterIndex = 0 ; speciesParameterIndex < speciesFileSearchValues.length ; speciesParameterIndex++ ) {
	    // do the multiplication
	    totalNumberOfSearchCases *= speciesFileSearchValues[speciesParameterIndex].length;
	    // copy these counts of options into the single giant column for later interpretive use
	    nValuesByRow[storageIndex] = speciesFileSearchValues[speciesParameterIndex].length; storageIndex++;
	}
	System.out.println("E: totalNumberOfSearchCases = " + totalNumberOfSearchCases);
	...
	for (int lineIndex = 0; lineIndex < nLinesInDataFile; lineIndex++) {
	    ...
	    ///////////////////////////////////////
	    ///////////////////////////////////////
	    // top of parameter search goes here //
	    ///////////////////////////////////////
	    ///////////////////////////////////////
	    ///////////////////////////////////////
	    ...
	    // we will eliminate the loops and make them a single loop over the sequence index.
	    
	    // the sequence index will be converted into the giant array of indices which will then be converted into the 4 sets of indices which will be converted into the 4 sets of values to use
	    
	    // so i need something like unpackGiantIndexVectorIntoFourBundlesOfValues()

//	    System.out.println("++++ top of parameter search for loops ++++");
//	    System.out.println("++++ totalNumberOfSearchCases = " + totalNumberOfSearchCases + " ++++");


	    // reset the "best yield" holder
	    bestYieldDuringSearch = Double.NEGATIVE_INFINITY;
	    bestSequenceIndex = -1;

	    for (long sequenceIndex = 0; sequenceIndex < totalNumberOfSearchCases; sequenceIndex++) {


		// i think we originally had this in the wrong place outsie the sequenceIndex loop
		...
		giantIndexVector = giantIndexVectorFromSequenceIndex(sequenceIndex, nValuesByRow);
		fourBundlesOfStringValues = unpackGiantIndexVectorIntoFourBundlesOfValues(
			giantIndexVector,xFileSearchValues,cultivarFileSearchValues,ecotypeFileSearchValues,speciesFileSearchValues
			);

		xParametersToUse 		= fourBundlesOfStringValues[0];
		cultivarFileParametersToUse	= fourBundlesOfStringValues[1];
		ecotypeFileParametersToUse	= fourBundlesOfStringValues[2];
		speciesFileParametersToUse	= fourBundlesOfStringValues[3];
		
	    
//		    System.out.println("    === inside innermost loop for parameter search ===");
		    System.out.print(sequenceIndex);
//		    System.out.println(sequenceIndex + " 1stX = " + xParametersToUse[0]);

		    
		    
		    
		    
		    // at this point, we need to write out the CUL, ECO, and SPE files; the X file will only be partially made up...
		    // x-files for both happy and real
		    // the rest of the x-file is filled in later....

			XHappyInvariantsReplaced = XHappyInvariantsReplacedBeforeSearch;
			XInvariantsReplaced      = XInvariantsReplacedBeforeSearch;

//			System.out.println("---- BEFORE --- XInvariantsReplaced = [" + XInvariantsReplaced + "]");

		    for (int xReplacerIndex = 0; xReplacerIndex < xFileSearchValues.length ; xReplacerIndex++ ) {
//			System.out.println("[" + xFileSearchCodes[xReplacerIndex] + "] replaced by [" + xParametersToUse[xReplacerIndex] + "]");
			XHappyInvariantsReplaced = XHappyInvariantsReplaced.replaceAll(xFileSearchCodes[xReplacerIndex], xParametersToUse[xReplacerIndex]);
			XInvariantsReplaced      = XInvariantsReplaced.replaceAll(     xFileSearchCodes[xReplacerIndex], xParametersToUse[xReplacerIndex]);
		    }
	    
		    // check to see if anything changed
		    /*
		    if (XInvariantsReplacedBeforeSearch.equals(XInvariantsReplaced)) {
			System.out.println("NOTHING CHANGED!!!");
			// now see look for the first index of the first of the replacer codes...
			System.out.println("first index of first prior in original = " + XInvariantsReplacedBeforeSearch.indexOf(xFileSearchCodes[0]));
		    } else {
			System.out.println("SUPPOSEDLY DIFFERENT!!!");
			// now see look for the first index of the first of the replacer codes...
			System.out.println("first index of first prior in changed one = " + XInvariantsReplaced.indexOf(xFileSearchCodes[0]));
			
		    }
		    
		    */
//		    System.out.println("---- AFTER --- XInvariantsReplaced = [" + XInvariantsReplaced + "]");

		    // initialize with the appropriate template
		    cultivarContentsReadyToWrite = templateCultivarFileContents;

		    // replace everything
		    for (int cultivarReplacerIndex = 0; cultivarReplacerIndex < cultivarFileSearchValues.length ; cultivarReplacerIndex++ ) {
			cultivarContentsReadyToWrite = cultivarContentsReadyToWrite.replaceAll(cultivarFileSearchCodes[cultivarReplacerIndex], cultivarFileParametersToUse[cultivarReplacerIndex]);
		    }
		    
		    // write it out in the right place
		    FunTricks.writeStringToFile(cultivarContentsReadyToWrite, cultivarFileToPut);
		    

		    
		    // initialize with the appropriate template
		    ecotypeContentsReadyToWrite = templateEcotypeFileContents;

		    // replace everything
		    for (int ecotypeReplacerIndex = 0; ecotypeReplacerIndex < ecotypeFileSearchValues.length ; ecotypeReplacerIndex++ ) {
			ecotypeContentsReadyToWrite = ecotypeContentsReadyToWrite.replaceAll(ecotypeFileSearchCodes[ecotypeReplacerIndex], ecotypeFileParametersToUse[ecotypeReplacerIndex]);
		    }
		    
		    // write it out in the right place
		    FunTricks.writeStringToFile(ecotypeContentsReadyToWrite, ecotypeFileToPut);

		    
		    
		    
		    // initialize with the appropriate template
		    speciesContentsReadyToWrite = templateSpeciesFileContents;

		    // replace everything
		    for (int speciesReplacerIndex = 0; speciesReplacerIndex < speciesFileSearchValues.length ; speciesReplacerIndex++ ) {
			speciesContentsReadyToWrite = speciesContentsReadyToWrite.replaceAll(speciesFileSearchCodes[speciesReplacerIndex], speciesFileParametersToUse[speciesReplacerIndex]);
		    }
		    
		    // write it out in the right place
//		    System.out.println("--- about to write out the species file: " + speciesFileToPut + " ; contents length is " + speciesContentsReadyToWrite.length());
		    FunTricks.writeStringToFile(speciesContentsReadyToWrite, speciesFileToPut);
...
//		if (lineIndex == 0 && plantingWindowIndex == 0) {
		if (!weHaveFoundASummaryFile) {
			...
			extraSummaryAccumulators = new DescriptiveStatisticsUtility[extraNames.length];
			for (int extraIndex = 0; extraIndex < extraNames.length; extraIndex++) {
			    System.out.println("extraName[" + extraIndex + "] = [" + extraNames[extraIndex] + "]");
			    extraSummaryAccumulators[extraIndex] = new DescriptiveStatisticsUtility(true);
			}
		...
		happyWasWorseThanBestStressed = false;
		...
		// one more check due to searching....
		// if this happy one is worse than the real best so far, don't bother with the real....
		if (bestYieldDuringSearch > phenologyInDays[0]) {
		    happyIsGoodEnoughToProceedWithReal = false;
		    happyWasWorseThanBestStressed = true;
		}
...
//		    elseTimerStats.useDoubleValue(elseTimer.tocMillis());
	    ...
	    /////////////////////////////////////////////////////////////////////
	    /////////////////////////////////////////////////////////////////////
	    // this is where the bottom of the parameter search loop should go //
	    // we will then need to completely redo the output part....        //
	    /////////////////////////////////////////////////////////////////////
	    /////////////////////////////////////////////////////////////////////
	    /////////////////////////////////////////////////////////////////////

	 // interpret the yield and update; to be eligible to update, we need the yield to exist (i.e., we got at least one year's worth of results) and better than anything we have seen before

	    // let us be stupid and just keep everything.....
	    
//	    if ( Double.isNaN(realYieldsEntirePixel.getMean()) && realYieldsEntirePixel.getMean() > bestYieldDuringSearch ) {
	    
	    ///////////////////////////////////////////////
	    // ok, let's do some "serious" debugging ... //
	    ///////////////////////////////////////////////
...
//	    System.out.println("# found = " + realYieldsEntirePixel.getN() + "; this mean = " + realYieldsEntirePixel.getMean() + "; previous best = " + bestYieldDuringSearch);
//	    System.out.println("-- cultivar contents --");
//	    System.out.println(cultivarContentsReadyToWrite);
//	    System.out.print("verdict is....");
	    
	    if ( realYieldsEntirePixel.getN() == 0 ) {
		System.out.print("o ");
//		System.out.println("o [this=" + realYieldsEntirePixel.getMean() + "]");
	    } else if ( realYieldsEntirePixel.getMean() > bestYieldDuringSearch ) {
		// let's dump something to the screen for debugging
//		System.out.print("^" + FunTricks.onlySomeDecimalPlaces(realYieldsEntirePixel.getMean(),1));
//		System.out.print("^ ");
		System.out.print("^[" + realYieldsEntirePixel.getN() + "] ");
//		System.out.println("^ [this=" + realYieldsEntirePixel.getMean() + "]");
		
		// update the yield itself
		bestYieldDuringSearch = realYieldsEntirePixel.getMean();
		
		// and some of the phenology
		anthesisOfBestYieldDuringSearch = realAnthesisEntirePixel.getMean();
		maturityOfBestYieldDuringSearch = realMaturityEntirePixel.getMean();

		bestSequenceIndex = sequenceIndex;
		
	      // copy these parameters...
	      for (int xParameterCopyIndex = 0 ; xParameterCopyIndex < xFileSearchValues.length ; xParameterCopyIndex++ ) {
	        xParametersBest[xParameterCopyIndex] = xParametersToUse[xParameterCopyIndex];
	      }
	      for (int cultivarParameterCopyIndex = 0 ; cultivarParameterCopyIndex < cultivarFileSearchValues.length ; cultivarParameterCopyIndex++ ) {
	        cultivarFileParametersBest[cultivarParameterCopyIndex] = cultivarFileParametersToUse[cultivarParameterCopyIndex];
	      }
	      for (int ecotypeParameterCopyIndex = 0 ; ecotypeParameterCopyIndex < ecotypeFileSearchValues.length ; ecotypeParameterCopyIndex++ ) {
	        ecotypeFileParametersBest[ecotypeParameterCopyIndex] = ecotypeFileParametersToUse[ecotypeParameterCopyIndex];
	      }
	      for (int speciesParameterCopyIndex = 0 ; speciesParameterCopyIndex < speciesFileSearchValues.length ; speciesParameterCopyIndex++ ) {
	        speciesFileParametersBest[speciesParameterCopyIndex] = speciesFileParametersToUse[speciesParameterCopyIndex];
	      }

	    } else if (happyWasWorseThanBestStressed) {
		System.out.println("v ");
//		System.out.print("v [this=" + realYieldsEntirePixel.getMean() + "; happy=" + happyYieldsEntirePixel.getMean() + "]");
	    } else {
		// let's dump something to the screen to show that there was no improvement
//		System.out.print("_" + FunTricks.onlySomeDecimalPlaces(realYieldsEntirePixel.getMean(),1));
		System.out.print("_ ");
//		System.out.println("_ [this=" + realYieldsEntirePixel.getMean() + "; happy=" + happyYieldsEntirePixel.getMean() + "]");
	    } // if this is a better yield

	    
	    
	    } // sequenceIndex    

	    System.out.println("...final yield decision is yield = " + bestYieldDuringSearch + "; index = " + bestSequenceIndex + "...");
		...
		for (int failureLineIndex = 0 ; failureLineIndex < nLinesToRecordAtTopDueToFailedInitialRunsAndNoSummaryHeaderKnowledge ; failureLineIndex++) {
		    ...
		    // first the most important aspects
		    statisticsOutLine = 
			    FunTricks.onlySomeDecimalPlaces(-11, nDecimalsInOutput) + delimiter +
			    FunTricks.onlySomeDecimalPlaces(-12, nDecimalsInOutput) + delimiter +
			    FunTricks.onlySomeDecimalPlaces(-13, nDecimalsInOutput) + delimiter +
			    -14;

		    // now, we need the parameters leading to these wonderful things...
		    // we will be converting from string -> double -> string in order to appropriately drop leading/trailing zeros and that sort of thing.
		      // copy these parameters...
		      for (int xParameterCopyIndex = 0 ; xParameterCopyIndex < xFileSearchValues.length ; xParameterCopyIndex++ ) {
		        statisticsOutLine += delimiter + FunTricks.onlySomeDecimalPlaces(-14,nDecimalsInOutput);
		      }
		      for (int cultivarParameterCopyIndex = 0 ; cultivarParameterCopyIndex < cultivarFileSearchValues.length ; cultivarParameterCopyIndex++ ) {
		        statisticsOutLine += delimiter + FunTricks.onlySomeDecimalPlaces(-15,nDecimalsInOutput);
		      }
		      for (int ecotypeParameterCopyIndex = 0 ; ecotypeParameterCopyIndex < ecotypeFileSearchValues.length ; ecotypeParameterCopyIndex++ ) {
		        statisticsOutLine += delimiter + FunTricks.onlySomeDecimalPlaces(-16,nDecimalsInOutput);
		      }
		      for (int speciesParameterCopyIndex = 0 ; speciesParameterCopyIndex < speciesFileSearchValues.length ; speciesParameterCopyIndex++ ) {
		        statisticsOutLine += delimiter + FunTricks.onlySomeDecimalPlaces(-17,nDecimalsInOutput);
		      }
	    ...
	    if (weHaveFoundASummaryFile) {
//	    if (realYieldsEntirePixel.getN() == 0) {
		// i think we just want to check if we ever found anything good or not...
		// an infinite yield would mean it has not been updated from negative infinity...
	    if (Double.isInfinite(bestYieldDuringSearch)) {

		    

		    // first the most important aspects
		    statisticsOutLine = 
			    FunTricks.onlySomeDecimalPlaces(-1, nDecimalsInOutput) + delimiter +
			    FunTricks.onlySomeDecimalPlaces(-2, nDecimalsInOutput) + delimiter +
			    FunTricks.onlySomeDecimalPlaces(-3, nDecimalsInOutput) + delimiter +
			    -4;
		    ...
		    // we will be converting from string -> double -> string in order to appropriately drop leading/trailing zeros and that sort of thing.
		      // copy these parameters...
		      for (int xParameterCopyIndex = 0 ; xParameterCopyIndex < xFileSearchValues.length ; xParameterCopyIndex++ ) {
		        statisticsOutLine += delimiter + FunTricks.onlySomeDecimalPlaces(-4,nDecimalsInOutput);
		      }
		      for (int cultivarParameterCopyIndex = 0 ; cultivarParameterCopyIndex < cultivarFileSearchValues.length ; cultivarParameterCopyIndex++ ) {
		        statisticsOutLine += delimiter + FunTricks.onlySomeDecimalPlaces(-5,nDecimalsInOutput);
		      }
		      for (int ecotypeParameterCopyIndex = 0 ; ecotypeParameterCopyIndex < ecotypeFileSearchValues.length ; ecotypeParameterCopyIndex++ ) {
		        statisticsOutLine += delimiter + FunTricks.onlySomeDecimalPlaces(-6,nDecimalsInOutput);
		      }
		      for (int speciesParameterCopyIndex = 0 ; speciesParameterCopyIndex < speciesFileSearchValues.length ; speciesParameterCopyIndex++ ) {
		        statisticsOutLine += delimiter + FunTricks.onlySomeDecimalPlaces(-7,nDecimalsInOutput);
		      }
		    ...
		    // first the most important aspects
//		    System.out.println("outputting this yield: " + FunTricks.onlySomeDecimalPlaces(bestYieldDuringSearch, nDecimalsInOutput));
		    statisticsOutLine = 
			    FunTricks.onlySomeDecimalPlaces(bestYieldDuringSearch, nDecimalsInOutput) + delimiter +
			    FunTricks.onlySomeDecimalPlaces(anthesisOfBestYieldDuringSearch, nDecimalsInOutput) + delimiter +
			    FunTricks.onlySomeDecimalPlaces(maturityOfBestYieldDuringSearch, nDecimalsInOutput) + delimiter +
			    bestSequenceIndex;
		    ...
		    // we will be converting from string -> double -> string in order to appropriately drop leading/trailing zeros and that sort of thing.
		      // copy these parameters...
		      for (int xParameterCopyIndex = 0 ; xParameterCopyIndex < xFileSearchValues.length ; xParameterCopyIndex++ ) {
		        statisticsOutLine += delimiter + FunTricks.onlySomeDecimalPlaces(Double.parseDouble(xParametersBest[xParameterCopyIndex]),nDecimalsInOutput);
		      }
		      for (int cultivarParameterCopyIndex = 0 ; cultivarParameterCopyIndex < cultivarFileSearchValues.length ; cultivarParameterCopyIndex++ ) {
		        statisticsOutLine += delimiter + FunTricks.onlySomeDecimalPlaces(Double.parseDouble(cultivarFileParametersBest[cultivarParameterCopyIndex]),nDecimalsInOutput);
		      }
		      for (int ecotypeParameterCopyIndex = 0 ; ecotypeParameterCopyIndex < ecotypeFileSearchValues.length ; ecotypeParameterCopyIndex++ ) {
		        statisticsOutLine += delimiter + FunTricks.onlySomeDecimalPlaces(Double.parseDouble(ecotypeFileParametersBest[ecotypeParameterCopyIndex]),nDecimalsInOutput);
		      }
		      for (int speciesParameterCopyIndex = 0 ; speciesParameterCopyIndex < speciesFileSearchValues.length ; speciesParameterCopyIndex++ ) {
		        statisticsOutLine += delimiter + FunTricks.onlySomeDecimalPlaces(Double.parseDouble(speciesFileParametersBest[speciesParameterCopyIndex]),nDecimalsInOutput);
		      }
	    ...
	    // now that we are done with everything for this line, i am going to try to clear out the magic out files...
		for (int fileIndex = 0; fileIndex < magicListOfFilesToClearOut.length; fileIndex++) {
			magicListOfFilesToClearOutAsObjects[fileIndex].delete();
		}
	...
	// first the most important aspects
	String columnList = 
		"best_yield" + delimiter +
		"anthesis_of_best" + delimiter +
		"maturity_of_best" + delimiter +
		"index_of_best";
	...
	// copy these parameters...
	for (int xParameterCopyIndex = 0 ; xParameterCopyIndex < xFileSearchValues.length ; xParameterCopyIndex++ ) {
	    columnList += delimiter + xFileSearchCodes[xParameterCopyIndex];
	}
	for (int cultivarParameterCopyIndex = 0 ; cultivarParameterCopyIndex < cultivarFileSearchValues.length ; cultivarParameterCopyIndex++ ) {
	    columnList += delimiter + cultivarFileSearchCodes[cultivarParameterCopyIndex];
	}
	for (int ecotypeParameterCopyIndex = 0 ; ecotypeParameterCopyIndex < ecotypeFileSearchValues.length ; ecotypeParameterCopyIndex++ ) {
	    columnList += delimiter + ecotypeFileSearchCodes[ecotypeParameterCopyIndex];
	}
	for (int speciesParameterCopyIndex = 0 ; speciesParameterCopyIndex < speciesFileSearchValues.length ; speciesParameterCopyIndex++ ) {
	    columnList += delimiter + speciesFileSearchCodes[speciesParameterCopyIndex];
	}
	...
	// Beware the MAGIC NUMBER!!!
	int nCols = 4 + xFileSearchValues.length + cultivarFileSearchValues.length + ecotypeFileSearchValues.length + speciesFileSearchValues.length; // the three we just saw
