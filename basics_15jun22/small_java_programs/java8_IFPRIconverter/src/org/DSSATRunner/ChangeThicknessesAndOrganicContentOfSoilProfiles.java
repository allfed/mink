...
public class ChangeThicknessesAndOrganicContentOfSoilProfiles {
  ...
  public static void main(String commandLineOptions[]) throws Exception {
    ...
    if (thicknessMultiplier < 0) {
      System.out.println(
          "thickness multiplier must be non-negative (i.e., not " + thicknessMultiplier + ")");
      throw new Exception();
    }

    if (organicCarbonMultiplier < 0) {
      System.out.println(
          "organic carbon multiplier must be non-negative (i.e., not "
              + organicCarbonMultiplier
              + ")");
      throw new Exception();
    }
    ...
    for (int profileIndex = 0; profileIndex < collectionOfProfiles.getNProfiles(); profileIndex++) {
      ...
      for (int layerIndex = 0; layerIndex < nLayersHere; layerIndex++) {
        ...
        // ok, this should be less tricky than i originally thought. since this is a truly linear
        // stretching,
        // i just need to change all the depths by multiplying them directly rather than trying to
        // relate them
        // to the above and below and such.
        // grab the old thickness and stretch to make the new thickness.

        oldDepthAtBase =
            collectionOfProfiles.getLayerDepthAtBase_SLB()[profileIndex][layerIndexToUse];
