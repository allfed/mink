...
public class Mink3p5daysearch {
    ...
    private static final int magicValueToUseWithNegativeEmergence = -1;
    ...
    static final int nStressIndices = 4; // a magic number: we have photosynthesis and growth crossed with water and nitrogen

	static final int phenologyYieldIndex = 0;
	static final int phenologyEmergenceIndex = 1;
	static final int phenologyAnthesisIndex = 2;
	static final int phenologyMaturityIndex = 3;
    ...
    private int howManyMonthsOfWeatherToSummarize = -1;
...
//    private int    nPlantingWindowsPerMonth = -5;
    ...
    private int    happyEmergenceThresholdToDoRealRuns = 0;
    ...
    private String plantingDateInMonthsShiftInDaysCSV = null;
    private int[] plantingDateInMonthShiftInDays = null;
...
//    private DescriptiveStatisticsUtility[][] extraYearly = null;
    ...
    // this is to parse the weather data to see individual months during the growing season
//    private double[][] monthlyAveragesAfterPlanting = null;

    static final private String[] weatherAveragesNames = { "sw" , "tmax" , "tmin" , "prec" };

    
    // and we need a second copy of everything so we can pick the highest yielding daily planting offset.
    // but, i think we can just go with double and long arrays....
    
    
    long soFarRealBadThingsCounts,soFarRealYieldMin,soFarRealYieldMax;
    double
    soFarRealYieldMean,
    soFarRealYieldStd,
    soFarRealEmergenceMean,
    soFarRealEmergenceStd,
    soFarRealAnthesisMean,
    soFarRealAnthesisStd,
    soFarRealMaturityMean,
    soFarRealMaturityStd,
    soFarHappyYieldsMean,
    soFarHappyMaturityMean;

    double[] soFarExtraSummaryAccumulatorsMeans = null;
    double[] soFarExtraSummaryAccumulatorsStds = null;
    double soFarRealTimeToPlanting, soFarRealNoPlanting, soFarRealYieldsExactlyZero;
    long soFarRealYieldsN, soFarHappyYieldsN;
    double[][] soFarStressAccumulators = null;
//    double[][] soFarMonthlyAveragesAfterPlanting = new double[nFakeYears][howManyMonthsOfWeatherToSummarize * weatherAveragesNames.length];
...
//    static final private String magicOverviewRunBreak = "**************************************************************************************************************";
    ...
    public Mink3p5daysearch(String initFileNameToUse) throws IOException, Exception {
	...
	int storageIndex = 0;
	howManyMonthsOfWeatherToSummarize = Integer.parseInt(
...
//		initFileContents[storageIndex++]);
	...
	happyMaturityThresholdToDoRealRuns = Integer.parseInt(
		initFileContents[storageIndex++]);
	happyEmergenceThresholdToDoRealRuns = Integer.parseInt(
		initFileContents[storageIndex++]);
	...
	// let's make this a CSV and then parse it to an int array
	
	plantingDateInMonthsShiftInDaysCSV = initFileContents[storageIndex++];
	// now, interpret it...
	String[] plantingDateInMonthShiftInDaysAsStringArray = plantingDateInMonthsShiftInDaysCSV.split(",");
	plantingDateInMonthShiftInDays = new int[plantingDateInMonthShiftInDaysAsStringArray.length];
	
	for (int dayShiftIndex = 0 ; dayShiftIndex < plantingDateInMonthShiftInDaysAsStringArray.length ; dayShiftIndex++) {
	    plantingDateInMonthShiftInDays[dayShiftIndex] = Integer.parseInt(plantingDateInMonthShiftInDaysAsStringArray[dayShiftIndex]);
	}
...
//	}
	...
	provenanceOut.print("howManyMonthsOfWeatherToSummarize:\t\t"      + howManyMonthsOfWeatherToSummarize + "\n");
...
//	provenanceOut.print("nPlantingWindowsPerMonth:\t"   + nPlantingWindowsPerMonth + "\n");
	...
	provenanceOut.print("happyEmergenceThresholdToDoRealRuns:\t" + happyEmergenceThresholdToDoRealRuns + "\n");
	...
	int actualPlantingYearNumberFourDigit = (actualPlantingDate / 1000);
	int actualPlantingYearNumber = actualPlantingYearNumberFourDigit - 100 * (actualPlantingYearNumberFourDigit / 100);
	int actualPlantingDayOfYearNumber = actualPlantingDate - actualPlantingYearNumberFourDigit * 1000;
...
//		System.out.println(storageMonthIndex);
    ...
    private double[] grabHappyResultsByName(int nYears, String[] splitUpNamesHere, int[] endingIndicesForSplitUpNamesHere) throws InterruptedException, Exception {
...
//	 String[] splitUpNamesHere, int[] endingIndicesForSplitUpNamesHere
	...
	int yieldToUse = -3;
	int emergenceToUse = -6, anthesisToUse = -3, maturityToUse = -3;
	...
	for (int fakeYearIndex = 0; fakeYearIndex < nYears; fakeYearIndex++) {
		...
		emergenceToUse = (int)Math.round(pullValueUsingName(candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead],
			emergenceColumnName, splitUpNamesHereToUse, endingIndicesForSplitUpNamesHereToUse));
...
//		happyYearlyYields[fakeYearIndex].useLongValue(yieldToUse);
		...
		// what about emergence? who knows? let's check...
		if (emergenceToUse < 0) {
//		    System.out.println("    HAPPY: negative emergence: [" + emergenceToUse + "], censoring to -1" + magicValueToUseWithNegativeEmergence + "; fakeYearIndex=" + fakeYearIndex);
		    // transplanting leads to negative ninety-nine emergence...
		    emergenceToUse = magicValueToUseWithNegativeEmergence;
		}
...
//		}
		...
		if (maturityToUse > 0 && plantingDate > 0) {
		    ...
		    // checking for reasonableness...
		    if (daysSincePlantingForEvent > 0) {
			// transplanting leads to negative emergence 
			happyEmergenceDates.useLongValue(daysSincePlantingForEvent);
		    }
//		    if (eventDate < 0) {
//		    // transplanting leads to negative emergence 
//			happyEmergenceDates.useLongValue(eventDate);
//		    }
		}
	...
	// debug: dump some emergence stuff out
//	System.out.println("HAPPY DEBUG: happyEmergenceDates.N = " + happyEmergenceDates.getN());
	
	return new double[] {happyYields.getMean(),
		happyEmergenceDates.getMean(),
		happyAnthesisDates.getMean(),
		happyMaturityDates.getMean()};
...
//    private void grabRealResultsByName(int nYears, long lineIndex, String[] splitUpNamesHere, int[] endingIndicesForSplitUpNamesHere, String[] JJJweatherFileContents, int howManyToSummarize)
    private void grabRealResultsByName(int nYears, long lineIndex, String[] splitUpNamesHere, int[] endingIndicesForSplitUpNamesHere)
...
//	double[] monthlyValuesDirectly = null;
...
//			System.out.println("grab real: pd = " + plantingDate + " ; mat = " + maturityToUse + " ; diff = " + daysSincePlantingForEvent);
		...
		// 02jun21 adding an emergence threshold, too, since i am annoyed at super long emergences...
...
//			    System.out.println("read extra: " + extraNames[extraIndex] + " [" + extraIndex + "] = " + extractedValues[extraIndex]);
			...
			System.out.println("REAL: had trouble reading #" + extraIndex + " " + extraNames[extraIndex] + ": [" + 
				pullStringValueUsingName(candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead],
					extraNames[extraIndex], splitUpNamesHere, endingIndicesForSplitUpNamesHere)
			+ 
			"] (data line " + lineIndex + "; fakeYear " + fakeYearIndex + " need to put in daily offset perhaps" + "):");
		...
		System.out.println("REAL: had trouble reading one of the following (data line " + lineIndex + "; fakeYear " + fakeYearIndex + " perhaps add daily offset " + "):");
		System.out.println("yield [" + pullStringValueUsingName(candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead],
			yieldColumnName, splitUpNamesHere, endingIndicesForSplitUpNamesHere) + "]");
...
//	    monthlyValuesDirectly = monthlyWeatherAveragesDuringSeason(weatherFileContents, plantingDate, howManyToSummarize);
//	    monthlyAveragesAfterPlanting[fakeYearIndex] = monthlyValuesDirectly;
//	    monthlyAveragesAfterPlanting[fakeYearIndex] = monthlyWeatherAveragesDuringSeason(weatherFileContents, plantingDate, howManyToSummarize);

//	    for (int displayIndex = 0; displayIndex < monthlyValuesDirectly.length; displayIndex++) {
//		System.out.println("[" + displayIndex + "] = " + monthlyValuesDirectly[displayIndex]);
//	    }
...
//	    } // end if everything is valid, do the recording...
	...
	if (stressAccumulators == null) {
	    ...
	    stressAccumulators = new DescriptiveStatisticsUtility[nGrowthStages][nStressIndices];
...
//	}
	...
	int magicGeographyFileExpectedColumns = 4;
	if (geogMatrix.getDimensions()[1] != magicGeographyFileExpectedColumns) {
...
//	String[] weatherFileContents = null;

	int indexOfHighestYieldingDailyOffsetSoFar = -1;
...
//		System.out.println("lineIndex = " + lineIndex + ", trying to initialize extra accumulators and copies");
		for (int extraIndex = 0; extraIndex < extraNames.length; extraIndex++) {
		    extraSummaryAccumulators[extraIndex].reset();

		    // and the equivalent soFar variables
		    soFarExtraSummaryAccumulatorsMeans = new double[extraNames.length];
		    soFarExtraSummaryAccumulatorsStds = new double[extraNames.length];

		}
	    } // lineIndex > 0

	    
	    // initialize the "soFar" variables that will allow us to figure out which daily offset is the best
	    indexOfHighestYieldingDailyOffsetSoFar = -1;
	    soFarRealBadThingsCounts = 0;
	    soFarRealYieldMin = 0;
	    soFarRealYieldMax = 0;
	    
	    soFarRealYieldMean = 0;
	    soFarRealYieldStd = 0;
	    soFarRealEmergenceMean = 0;
	    soFarRealEmergenceStd = 0;
	    soFarRealAnthesisMean = 0;
	    soFarRealAnthesisStd = 0;
	    soFarRealMaturityMean = 0;
	    soFarRealMaturityStd = 0;
	    soFarHappyYieldsMean = 0;
	    soFarHappyMaturityMean = 0;

	    soFarRealTimeToPlanting = 0;
	    soFarRealNoPlanting = 0;
	    soFarRealYieldsExactlyZero = 0;
	    soFarRealYieldsN = 0;
	    soFarHappyYieldsN = 0;
	    
	    // stress accumulators should be initialized lower down when we check for overview.out
	    soFarStressAccumulators = null;
...
//	    }
	    ...
	    // clear out the monthly weather accumulator
//	    monthlyAveragesAfterPlanting = new double[nFakeYears][howManyMonthsOfWeatherToSummarize * weatherAveragesNames.length];
	    ...
	    // now, we are trying to summarize by month after the planting date, so we need the stuff...
//	    weatherFileContents = FunTricks.readTextFileToArray(weatherExecutionCommand[6]);
	    ...
	    for (int dailyOffsetSearchIndex = 0; dailyOffsetSearchIndex < plantingDateInMonthShiftInDays.length ; dailyOffsetSearchIndex++) {
		elseTimer.tic();

		// pick the starting day/etc for this window
		startingDayToPlantForThisWindow = firstPlantingDay;

		startingDateAsInt = DSSATHelperMethods.yyyyDDDaddDaysIgnoreLeap(fakePlantingYear*1000 + startingDayToPlantForThisWindow, plantingDateInMonthShiftInDays[dailyOffsetSearchIndex]);
...
//		startingDateAsInt = Integer.parseInt(startingDayToPlantCode);
		...
		for (int rerunIndex = 0; rerunIndex < rerunAttemptsMax ; rerunIndex++) {
			...
			System.out.println("     +++ happy timed out #" + rerunIndex + "(max = " + (rerunAttemptsMax - 1) + ") line=" + lineIndex + " need to put daily offset in here..." 
		...
		readingTimer.tic();
		phenologyInDays = this.grabHappyResultsByName(nHappyPlantRunsForPhenology,splitUpNamesHere, endingIndicesForSplitUpNamesHere); // 02jun21 changed to put emergence in spot index 1
...
//		if (lineIndex == 0 && plantingWindowIndex == 0) {
		if (!weHaveFoundASummaryFile) {
			...
			//// begin new stuff to allow for recording every year.... ////
			...
			//// end new stuff to allow for recording every year.... ////
			...
			if (weShouldBotherParsingOverviewDotOut) {
			    ...
			    soFarStressAccumulators = new double[growthStageNames.length][nStressIndices];
			...
			System.out.println("    failed to find a " + magicDSSATSummaryToRead +" so far at line " 
				+ lineIndex + " need to put in daily offeset ");
		...
		// and i am getting sick of stupid emergence, so we are putting a threshold on that too...
		happyIsGoodEnoughToProceedWithReal = false;
//		System.out.println("yield = " + phenologyInDays[phenologyYieldIndex] + "; yield threshold = " + this.happyYieldThresholdToDoRealRuns + "; emergence = " + phenologyInDays[phenologyEmergenceIndex] + "; emergence threshold =" + this.happyEmergenceThresholdToDoRealRuns +
//			"; maturity = " + phenologyInDays[phenologyMaturityIndex] + "; maturity threshold = " + this.happyMaturityThresholdToDoRealRuns);

//		System.out.println("A: measured happy emergence = " + phenologyInDays[phenologyEmergenceIndex] + " vs " + this.happyEmergenceThresholdToDoRealRuns + " = threshold");
//		System.out.println("happy maturity = " + phenologyInDays[phenologyMaturityIndex] + "  happy yield = " + phenologyInDays[phenologyYieldIndex]);
		...
		if ( this.cropFertilizerSchemeToUse.equalsIgnoreCase(cassavaString) ||
		    ...
		    if (phenologyInDays[phenologyYieldIndex] >= this.happyYieldThresholdToDoRealRuns) {
			happyIsGoodEnoughToProceedWithReal = true;
		    }
		} else {
		    if (phenologyInDays[phenologyYieldIndex] >= this.happyYieldThresholdToDoRealRuns && 
			    (phenologyInDays[phenologyEmergenceIndex] <= this.happyEmergenceThresholdToDoRealRuns || Double.isNaN(phenologyInDays[phenologyEmergenceIndex])) && 
			    phenologyInDays[phenologyMaturityIndex] <= this.happyMaturityThresholdToDoRealRuns) {
			happyIsGoodEnoughToProceedWithReal = true;
		    }
		}
		...
		if (happyIsGoodEnoughToProceedWithReal) {
		    ...
		    fertilizerBlock = nitrogenFertilizerScheme.buildNitrogenOnlyBlock(
			    (int)Math.floor(phenologyInDays[phenologyAnthesisIndex]) + phenologyBufferInDays, (int)Math.floor(phenologyInDays[phenologyMaturityIndex]) + phenologyBufferInDays, nitrogenLevel
			    );
			...
			if (realRunnerThing.finishedCleanly() != SystemCallWithTimeout.SYSTEM_CALL_RAN_FINE) {
			    ...
			    System.out.println("     +++ real timed out #" + rerunIndex + "(max = " + (rerunAttemptsMax - 1) + ") line=" + lineIndex + " need to put daily shift in here=" 
				    + " lines in Summary.OUT = " + nLinesFoundInSummary + " goal = " + (nFakeYears + magicDSSATSummaryLineIndexToRead) );
		    ...
		    grabRealResultsByName(nFakeYears, lineIndex, splitUpNamesHere, endingIndicesForSplitUpNamesHere);
//		    grabRealResultsByName(nFakeYears, lineIndex, plantingWindowIndex, splitUpNamesHere, endingIndicesForSplitUpNamesHere);

		    // see if we got anything in the extraAccumulators
//		    int indexToCheck = 0;
//		    System.out.println("see if we got anything in HWAM. in " + indexToCheck + " (" + extraNames[indexToCheck] + ") we have " + extraSummaryAccumulators[indexToCheck].getN() + " contributions with an average of " + extraSummaryAccumulators[indexToCheck].getMean());
		    
		    // ok, i think i want to add in the reading of the OVERVIEW.OUT file here...
		    if (weShouldBotherParsingOverviewDotOut) {

			// reset things...
			// 26may21, i am eliminating the plantingWindowIndex. so, i think that means that this should get reset every time we get here
...
//		    System.out.println("funnytest dailyOffsetSearchIndex" + dailyOffsetSearchIndex + "; n_yields = " + realYieldsEntirePixel.getN() + ": new yield = " + realYieldsEntirePixel.getMean() + " > " + soFarRealYieldMean + " = so far...");

		// check the mean real yield and update the soFar-s

		if ( realYieldsEntirePixel.getMean() > soFarRealYieldMean) {
		    // YES! we have something better!
//		    System.out.println("YES! " + dailyOffsetSearchIndex + ": new yield = " + realYieldsEntirePixel.getMean() + " > " + soFarRealYieldMean + " = so far...");
		    indexOfHighestYieldingDailyOffsetSoFar = dailyOffsetSearchIndex;

		    soFarRealBadThingsCounts = realBadThingsCountsEntirePixel.getTotalLong();
		    soFarRealYieldMin = realYieldsEntirePixel.getMinAsLong();
		    soFarRealYieldMax = realYieldsEntirePixel.getMaxAsLong();
		    
		    soFarRealYieldMean = realYieldsEntirePixel.getMean();
		    soFarRealYieldStd = realYieldsEntirePixel.getStd();
		    soFarRealEmergenceMean = realEmergenceEntirePixel.getMean();
		    soFarRealEmergenceStd = realEmergenceEntirePixel.getStd();
		    soFarRealAnthesisMean = realAnthesisEntirePixel.getMean();
		    soFarRealAnthesisStd = realAnthesisEntirePixel.getStd();
		    soFarRealMaturityMean = realMaturityEntirePixel.getMean();
		    soFarRealMaturityStd = realMaturityEntirePixel.getStd();
		    soFarHappyYieldsMean = happyYieldsEntirePixel.getMean();
		    soFarHappyMaturityMean = happyMaturityEntirePixel.getMean();

		    for (int nameIndex = 0; nameIndex < extraNames.length; nameIndex++) {
			soFarExtraSummaryAccumulatorsMeans[nameIndex] = extraSummaryAccumulators[nameIndex].getMean();
			soFarExtraSummaryAccumulatorsStds[nameIndex] = extraSummaryAccumulators[nameIndex].getStd();
		    }
		    
		    
		    soFarRealTimeToPlanting = realTimeToPlantingEntirePixel.getMean();
		    soFarRealNoPlanting = realNoPlantingEntirePixel.getN();
		    soFarRealYieldsExactlyZero = realYieldsExactlyZeroEntirePixel.getN();
		    soFarRealYieldsN = realYieldsEntirePixel.getN();
		    soFarHappyYieldsN = happyYieldsEntirePixel.getN();
		...
		// now that we have captured the info, we need to reset them...
		    // initialize the descriptive statistics utility and storage array
		    realYieldsEntirePixel.reset();
		    realYieldsExactlyZeroEntirePixel.reset();
		    happyYieldsEntirePixel.reset();
...
//			System.out.println("lineIndex = " + lineIndex + ", trying to initialize extra accumulators and copies");
			for (int extraIndex = 0; extraIndex < extraNames.length; extraIndex++) {
			    ...
			    // and the equivalent soFar variables
//			    why would i want to reset the so fars? i think this might be my problem...
...
//		else {
//		    System.out.println("NO!  " + dailyOffsetSearchIndex + ": new yield = " + realYieldsEntirePixel.getMean() + " > " + soFarRealYieldMean + " = so far...");
//		}
		
	    } // end daily offset search
		...
		for (int failureLineIndex = 0 ; failureLineIndex < nLinesToRecordAtTopDueToFailedInitialRunsAndNoSummaryHeaderKnowledge ; failureLineIndex++) {
		    ...
		    statisticsOutLine = 
			    ...
			    + indexOfHighestYieldingDailyOffsetSoFar;
		    ...
		    statisticsOutLine += delimiter + (-this.spinUpTimeDays) + delimiter + (-1) + delimiter + (0) +
			    delimiter + soFarRealYieldsN + delimiter + soFarHappyYieldsN;
//		    delimiter + realYieldsEntirePixel.getN() + delimiter + happyYieldsEntirePixel.getN();
			...
			for (int growthStageIndex = 0; growthStageIndex < growthStageNames.length; growthStageIndex++) {
			    for (int stressIndexIndex = 0; stressIndexIndex < nStressIndices; stressIndexIndex++) {
			...
			// the weather averages, if so requested...
//			for (int weatherTypeIndex = 0; weatherTypeIndex < weatherAveragesNames.length; weatherTypeIndex++ ) {
//			    for (int monthToAverageIndex = 0; monthToAverageIndex < howManyMonthsOfWeatherToSummarize; monthToAverageIndex++) {
//				for (int fakeYearIndex = 0; fakeYearIndex < nFakeYears; fakeYearIndex++) {
//				    statisticsOutLine += delimiter + -999;
//				} // fakeYearIndex
//			    } // monthToAverageIndex
//			} // weather type index; i.e., sw / tmin / tmax / prec
		...
		if (indexOfHighestYieldingDailyOffsetSoFar < 0) {
		    offsetToRecord = indexOfHighestYieldingDailyOffsetSoFar;
		} else {
		    offsetToRecord = plantingDateInMonthShiftInDays[indexOfHighestYieldingDailyOffsetSoFar];
		}
		
		    statisticsOutLine =
			    soFarRealBadThingsCounts + delimiter
			    + soFarRealYieldMin + delimiter 
			    + soFarRealYieldMax + delimiter 
			    + FunTricks.onlySomeDecimalPlaces(soFarRealYieldMean,nDecimalsInOutput) + delimiter
			    + FunTricks.onlySomeDecimalPlaces(soFarRealYieldStd,nDecimalsInOutput) + delimiter
			    + FunTricks.onlySomeDecimalPlaces(soFarRealEmergenceMean,nDecimalsInOutput) + delimiter
			    + FunTricks.onlySomeDecimalPlaces(soFarRealEmergenceStd,nDecimalsInOutput) + delimiter
			    + FunTricks.onlySomeDecimalPlaces(soFarRealAnthesisMean,nDecimalsInOutput) + delimiter
			    + FunTricks.onlySomeDecimalPlaces(soFarRealAnthesisStd,nDecimalsInOutput) + delimiter
			    + FunTricks.onlySomeDecimalPlaces(soFarRealMaturityMean,nDecimalsInOutput) + delimiter
			    + FunTricks.onlySomeDecimalPlaces(soFarRealMaturityStd,nDecimalsInOutput) + delimiter
			    + FunTricks.onlySomeDecimalPlaces(soFarHappyYieldsMean,nDecimalsInOutput) + delimiter
			    + FunTricks.onlySomeDecimalPlaces(soFarHappyMaturityMean,nDecimalsInOutput) + delimiter
			    + offsetToRecord;

		    
		    
		    for (int extraIndex = 0; extraIndex < extraNames.length; extraIndex++) {
			statisticsOutLine += delimiter + FunTricks.onlySomeDecimalPlaces(soFarExtraSummaryAccumulatorsMeans[extraIndex],nDecimalsInOutput);
		    }

		    statisticsOutLine += delimiter + FunTricks.onlySomeDecimalPlaces((soFarRealTimeToPlanting - this.spinUpTimeDays),nDecimalsInOutput)
		    + delimiter + soFarRealNoPlanting + delimiter + soFarRealYieldsExactlyZero +
		    delimiter + soFarRealYieldsN + delimiter + soFarHappyYieldsN;

		    statisticsOutLine += delimiter + FunTricks.onlySomeDecimalPlaces(
			    soFarExtraSummaryAccumulatorsStds[findIndexForColumnName(yieldColumnName, splitUpNamesHere)],
			    nDecimalsInOutput);
			...
			for (int growthStageIndex = 0; growthStageIndex < growthStageNames.length; growthStageIndex++) {
				...
				statisticsOutLine += delimiter + 
					FunTricks.onlySomeDecimalPlaces(soFarStressAccumulators[growthStageIndex][stressIndexIndex],nDecimalsInOutput);
				//			statisticsOutLine += delimiter + 0;
			    } // for stress index index
			} // for growth stage index
		    } // if we should bother with overview
...
//		System.out.println("C: measured happy emergence = " + phenologyInDays[phenologyEmergenceIndex] + " vs " + this.happyEmergenceThresholdToDoRealRuns + " = threshold");
	...
	String columnList = "n_bad_things" + delimiter 
		...
		+ "happy_yield_mean" + delimiter + "happy_maturity_mean" + delimiter
		+ "shift_in_days";
...
//	    for (int monthToAverageIndex = 0; monthToAverageIndex < howManyMonthsOfWeatherToSummarize; monthToAverageIndex++) {
//		for (int fakeYearIndex = 0; fakeYearIndex < nFakeYears; fakeYearIndex++) {
//		    columnList += delimiter + weatherAveragesNames[weatherTypeIndex] + "_" + monthToAverageIndex + "_" + fakeYearIndex;
//		} // fakeYearIndex
//	    } // monthToAverageIndex
//	} // weather type index; i.e., sw / tmin / tmax / prec
...
//	  1  soFarRealBadThingsCounts + delimiter
//	  2  + soFarRealYieldMin + delimiter 
//	  3  + soFarRealYieldMax + delimiter 
//	  4  + FunTricks.onlySomeDecimalPlaces(soFarRealYieldMean,nDecimalsInOutput) + delimiter
//	  5  + FunTricks.onlySomeDecimalPlaces(soFarRealYieldStd,nDecimalsInOutput) + delimiter
//	  6  + FunTricks.onlySomeDecimalPlaces(soFarRealEmergenceMean,nDecimalsInOutput) + delimiter
//	  7  + FunTricks.onlySomeDecimalPlaces(soFarRealEmergenceStd,nDecimalsInOutput) + delimiter
//	  8  + FunTricks.onlySomeDecimalPlaces(soFarRealAnthesisMean,nDecimalsInOutput) + delimiter
//	  9  + FunTricks.onlySomeDecimalPlaces(soFarRealAnthesisStd,nDecimalsInOutput) + delimiter
//	 10  + FunTricks.onlySomeDecimalPlaces(soFarRealMaturityMean,nDecimalsInOutput) + delimiter
//	 11  + FunTricks.onlySomeDecimalPlaces(soFarRealMaturityStd,nDecimalsInOutput) + delimiter
//	 12  + FunTricks.onlySomeDecimalPlaces(soFarHappyYieldsMean,nDecimalsInOutput) + delimiter
//	 13  + FunTricks.onlySomeDecimalPlaces(soFarHappyMaturityMean,nDecimalsInOutput) + delimiter
//	 14  + offsetToRecord;

	nCols = 14 + extraNames.length + 2 + 1 + 2 + 1;
	// min / max/ mean / std / bad / happy mean / happy std / real anthesis mean / real anthesis std / real maturity mean / real maturity std / happy maturity mean / happy maturity std
	// time / no planting / exactly zero / n real / n happy / HWAH std

	if (weShouldBotherParsingOverviewDotOut) {
	    for (int growthStageIndex = 0; growthStageIndex < growthStageNames.length; growthStageIndex++) {
		nCols += nStressIndices; // Beware the MAGIC NUMBER!!! there are four stress indices per growth stage...W
		    
	    } // for growth stage index
	} // if we should bother with overview
	
	// bump up the count for any monthly weather summaries
//	nCols += weatherAveragesNames.length * howManyMonthsOfWeatherToSummarize * nFakeYears;
	...
	System.out.println(" total time in DSSAT loop = " + totalDSSATTimeMillis / 1000 / 60 + "min ; per run average = " 
		+ totalDSSATTimeMillis / nLinesInDataFile / nFakeYears  + "ms");
	System.out.println("overall per run average = " +
		overallTimeMillis/ nLinesInDataFile / nFakeYears + "ms"	);
