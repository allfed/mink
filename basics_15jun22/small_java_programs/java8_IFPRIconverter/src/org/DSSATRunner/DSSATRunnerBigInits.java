...
public class DSSATRunnerBigInits {
  ...
  private static final int[] extraStartIndices =
      new int[] {
        140, 146, 154, 162, 170, 178, 184, 192, 198, 206, 212, 218, 224, 230, 236, 242, 248, 254,
        260, 266, 272, 278, 284, 290, 296, 302, 308, 314, 320, 326, 332, 338, 344, 350, 356, 362,
        368, 374, 381, 388, 395, 402, 410, 418, 427, 436, 445, 454, 463, 472, 481, 490, 499, 508,
        517, 526, 532, 538, 544, 550, 556, 563, 570
      };
  private static final int[] extraEndIndices =
      new int[] {
        146, 154, 162, 170, 178, 184, 192, 198, 206, 212, 218, 224, 230, 236, 242, 248, 254, 260,
        266, 272, 278, 284, 290, 296, 302, 308, 314, 320, 326, 332, 338, 344, 350, 356, 362, 368,
        374, 381, 388, 395, 402, 410, 418, 427, 436, 445, 454, 463, 472, 481, 490, 499, 508, 517,
        526, 532, 538, 544, 550, 556, 563, 570, 577
      };

  // oldie moldies for wheat....
  //	private int magicDSSATSummaryLineLength = 467; // 350;

  private int OLDmagicHarvestedWeightAtHarvestStartIndex =
      139; // 139; // character # 137, index 136
  ...
  private static final String[] OLDextraNames = {
    "DWAP", "CWAM", "HWAM", "HWAH", "BWAH", "PWAM", "HWUM", "H#AM", "H#UM", "HIAM", "LAIX", "IR#M",
    "IRCM", "PRCM", "ETCM", "EPCM", "ESCM", "ROCM", "DRCM", "SWXM", "NI#M", "NICM", "NFXM", "NUCM",
    "NLCM", "NIAM", "CNAM", "GNAM", "PI#M", "PICM", "PUPC", "SPAM", "KI#M", "KICM", "KUPC", "SKAM",
    "RECM", "ONTAM", "ONAM", "OPTAM", "OPAM", "OCTAM", "OCAM", "DMPPM", "DMPEM", "DMPTM", "DMPIM",
  ...
  private static final int[] OLDextraStartIndices =
      new int[] {
  ...
  private static final int[] OLDextraEndIndices =
      new int[] {
  ...
  public static void generateDemoInitFile(String filename) throws FileNotFoundException {
    ...
    initFileContents +=
        "depthForNitrogen: depth (cm) over which to distribute the initial soil nitrogen" + "\n";
    initFileContents +=
        "residueNitrogenPercent: percent nitrogen content of initial crop residues left on the"
            + " field\n";
    initFileContents +=
        "incorporationRate: rate of incorporation for the initial crop residues" + "\n";
    initFileContents +=
        "incorporationDepth: depth of incorporation for the initial crop residues" + "\n";
    initFileContents +=
        "clayLoamSandStableCarbonRatesFilename: path/filename for where to find a text file with"
            + " values for stable carbon by layer for major soil textures\n";
  ...
  public void readInitFile(String filename) throws IOException, Exception {
    ...
    //		initFileContents += "depthForNitrogen: depth (cm) over which to distribute the initial soil
    // nitrogen" + "\n";
    //		initFileContents += "residueNitrogenPercent: percent nitrogen content of initial crop
    // residues left on the field" + "\n";
    //		initFileContents += "incorporationRate: rate of incorporation for the initial crop residues"
    // + "\n";
    //		initFileContents += "incorporationDepth: depth of incorporation for the initial crop
    // residues" + "\n";
    //		initFileContents += "clayLoamSandStableCarbonRatesFilename: path/filename for where to find
    // a text file with values for stable carbon by layer for major soil textures" + "\n";
  ...
  private void grabUnifiedNewManyResults(int nYears, boolean useOldSummaryIndices, long lineIndex)
    ...
    int[] hereextraStartIndices, hereextraEndIndices;
      ...
      try {
        ...
        yieldToUse =
            Integer.parseInt(
                candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                    .substring(
                        heremagicHarvestedWeightAtHarvestStartIndex,
                        heremagicHarvestedWeightAtHarvestEndIndex)
                    .trim());
        ...
        if (!useOldSummaryIndices) {
          emergenceToUse =
              Integer.parseInt(
                  candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                      .substring(magicEmergenceDateStartIndex, magicEmergenceDateEndIndex)
                      .trim());
          ;
        } else {
          emergenceToUse = -1;
        ...
        startingDate =
            Integer.parseInt(
                candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                    .substring(heremagicStartingDateStartIndex, heremagicStartingDateEndIndex)
                    .trim());
        plantingDate =
            Integer.parseInt(
                candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                    .substring(heremagicPlantingDateStartIndex, heremagicPlantingDateEndIndex)
                    .trim());
        anthesisToUse =
            Integer.parseInt(
                candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                    .substring(heremagicAnthesisDateStartIndex, heremagicAnthesisDateEndIndex)
                    .trim());
        ;
        maturityToUse =
            Integer.parseInt(
                candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                    .substring(heremagicMaturityDateStartIndex, heremagicMaturityDateEndIndex)
                    .trim());
        ...
        if (maturityToUse < 0) {
          maturityToUse =
              Integer.parseInt(
                  candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                      .substring(heremagicHarvestingDateStartIndex, heremagicHarvestingDateEndIndex)
                      .trim());
        }
        ...
        for (int extraIndex = 0; extraIndex < hereextraStartIndices.length; extraIndex++) {
          stringToParse =
              candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                  .substring(hereextraStartIndices[extraIndex], hereextraEndIndices[extraIndex])
                  .trim();
      ...
      if (!skipThisOne) {
        ...
        if (maturityToUse > 0 && plantingDate > 0) {
          ...
          if (daysSincePlantingForEvent > 300) {
            //							System.out.println("pd = " + plantingDate + " ; mat = " + maturityToUse + " ;
          ...
          //						System.out.println("grab real results: maturity failed...; not counting in
          // maturity average");
  ...
  private void grabUnifiedNewManyResultsOLD(int nYears, boolean useOldSummaryIndices)
      throws InterruptedException, Exception {
    // declarations
    int plantingDate = -1;
    int startingDate = -2;
    ...
    for (int fakeYearIndex = 0; fakeYearIndex < nYears; fakeYearIndex++) {
      ...
      try {
        ...
        //				throw new Exception();
        System.out.println(
            "     -> something bad happened with the essentials, skipping this line... <-");
      }
  ...
  private int[] grabUnifiedHappyResults(int nYears, boolean useOldSummaryIndices)
      throws InterruptedException, Exception {
    ...
    for (int fakeYearIndex = 0; fakeYearIndex < nYears; fakeYearIndex++) {
      ...
      try {
        ...
        startingDate =
            Integer.parseInt(
                candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                    .substring(magicStartingDateStartIndex, magicStartingDateEndIndex)
                    .trim());
        plantingDate =
            Integer.parseInt(
                candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
  ...
  private int[] grabOLDHappyResults(int nYears) throws InterruptedException, Exception {
    // if we wait properly using .waitFor(), we shouldn't have to check if things exist...
      ...
      try {
        ...
        yieldToUse =
            Integer.parseInt(
                candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                    .substring(
                        OLDmagicHarvestedWeightAtHarvestStartIndex,
                        OLDmagicHarvestedWeightAtHarvestEndIndex)
                    .trim());
        // planting / anthesis / maturity dates
        plantingDate =
            Integer.parseInt(
                candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                    .substring(OLDmagicPlantingDateStartIndex, OLDmagicPlantingDateEndIndex)
                    .trim());
        //				emergenceToUse = Integer.parseInt(candidateSummaryContents[fakeYearIndex +
        // magicDSSATSummaryLineIndexToRead]
        //
        ...
        anthesisToUse =
            Integer.parseInt(
                candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                    .substring(OLDmagicAnthesisDateStartIndex, OLDmagicAnthesisDateEndIndex)
                    .trim());
        ;
        maturityToUse =
            Integer.parseInt(
                candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                    .substring(OLDmagicMaturityDateStartIndex, OLDmagicMaturityDateEndIndex)
                    .trim());
  ...
  private void grabOLDManyResults(int nYears) throws InterruptedException, Exception {
    // declarations
    int plantingDate = -1;
    int daysSincePlantingForEvent = -3;
    ...
    int anthesisToUse = -3;
    int maturityToUse = -3;
    double[] extractedValues = new double[OLDextraStartIndices.length];
    boolean[] everythingIsValid = new boolean[OLDextraStartIndices.length];
    boolean skipThisOne = false;
    ...
    for (int fakeYearIndex = 0; fakeYearIndex < nYears; fakeYearIndex++) {
      ...
      skipThisOne = false;
      for (int validIndex = 0; validIndex < OLDextraStartIndices.length; validIndex++) {
        everythingIsValid[validIndex] = true; // good unless decided otherwise
      }
      ...
      try {
        ...
        // magicDSSATSummaryLineIndexToRead]
        //
        // .substring(OLDmagicEmergenceDateStartIndex,
        //
        //	OLDmagicEmergenceDateEndIndex).trim());;
        anthesisToUse =
            Integer.parseInt(
                candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                    .substring(OLDmagicAnthesisDateStartIndex, OLDmagicAnthesisDateEndIndex)
        ...
        if (maturityToUse == magicMissingValue) {
          maturityToUse =
              Integer.parseInt(
                  candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                      .substring(OLDmagicHarvestingDateStartIndex, OLDmagicHarvestingDateEndIndex)
                      .trim());
        }
        ...
        for (int extraIndex = 0; extraIndex < OLDextraStartIndices.length; extraIndex++) {
          stringToParse =
              candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                  .substring(OLDextraStartIndices[extraIndex], OLDextraEndIndices[extraIndex])
                  .trim();
          // check if there are the silly stars in there...
          if (stringToParse.contains("*")) {
            // ok, let's try this again, but we'll just skip over the offending value but keep the
      ...
      if (!skipThisOne) {
        ...
        //					realBadThingsCountsEntirePixel.useLongValue(1); // assess a problem...
        //				}
        realEmergenceEntirePixel.useLongValue(
            -1); // Beware the MAGIC NUMBER!!! we aren't doing emergence, so put in a nonsense value
  ...
  public void doSimulationsUniqueSeedsAllClimateSupplied(boolean useOldSummaryIndices)
      throws Exception {
    ...
    for (int lineIndex = 0; lineIndex < nLinesInDataFile; lineIndex++) {
      ...
      for (int plantingWindowIndex = 0;
        ...
        harvestingDayForThisWindow =
            firstPlantingDay
                + plantingWindowIndex * plantingWindowSpacing
                + optionalHarvestInterval;
        ...
        harvestDayCode =
            (DSSATHelperMethods.pad2CharactersZeros(harvestingYearForThisWindow)
        ...
        // it used to be here, but that was wrong and we were stuck with the first seed every time.
        // changed 21dec12
        //				XHappyStuffToWrite =
        // XHappyInvariantsReplaced.replaceAll(this.soilInitializationPlaceholder,
        // dummyInitializationBlock);
        //
        //				// do the search and replace thing; the invariants have already been done above...
        //				XHappyStuffToWrite = XHappyStuffToWrite.replaceAll(soilPlaceholder               ,
        // soilTypeString);
        //				XHappyStuffToWrite = XHappyStuffToWrite.replaceAll(initializationStartPlaceholder,
        // initializationDayCode);
        //				XHappyStuffToWrite = XHappyStuffToWrite.replaceAll(plantingDateStartPlaceholder  ,
        // startingDayToPlantCode);
        //				XHappyStuffToWrite = XHappyStuffToWrite.replaceAll(plantingDateEndPlaceholder    ,
        // endingDayToPlantCode);
        //				XHappyStuffToWrite = XHappyStuffToWrite.replaceAll(harvestingDatePlaceholder     ,
        // harvestDayCode);

        for (int happyPlantRunIndex = 0;
            happyPlantRunIndex < this.nHappyPlantRunsForPhenology;
            happyPlantRunIndex++) {
          ...
          //					XHappyStuffToWrite =
          // XHappyStuffToWrite.replaceAll(this.randomSeedPlaceholder,Integer.toString(this.firstRandomSeed + happyPlantRunIndex));
          randomSeedActualValue =
              DSSATHelperMethods.padWithZeros(firstRandomSeed + happyPlantRunIndex, 5);
          ...
          XHappyStuffToWrite =
              XHappyStuffToWrite.replaceAll(this.randomSeedPlaceholder, randomSeedActualValue);
        ...
        if (phenologyInDays[0] >= this.happyYieldThresholdToDoRealRuns
          ...
          for (int normalPlantRunIndex = 0;
              normalPlantRunIndex < this.nRandomSeedsToUse;
              normalPlantRunIndex++) {
            ...
            // put in the random seed to use
            //						XStuffToWrite =
            // XStuffToWrite.replaceAll(this.randomSeedPlaceholder,Integer.toString(firstRandomSeed
            // + normalPlantRunIndex));

            randomSeedActualValue =
                DSSATHelperMethods.padWithZeros(firstRandomSeed + normalPlantRunIndex, 5);
            if (randomSeedActualValue.length() != 5) {
              System.out.println(
                  "fRS = "
                      + firstRandomSeed
            ...
            XStuffToWrite =
                XStuffToWrite.replaceAll(this.randomSeedPlaceholder, randomSeedActualValue);
            XStuffToWrite =
            ...
            //						XStuffToWrite =
            // XStuffToWrite.replaceAll(this.randomSeedPlaceholder,Integer.toString(this.firstRandomSeed + normalPlantRunIndex));
            ...
            // make sure the streams got closed for the process after we managed to read
            // everything...
            theRunningErrorStream = theRunningProcess.getErrorStream();
            theRunningErrorStream.close();
            theRunningInputStream = theRunningProcess.getInputStream();
  ...
  public void doSimulationsMultipleYearsAllClimateSupplied(boolean useOldSummaryIndices)
      throws Exception {
    ...
    /*
    	...
    	long nitrogenKgPerHaCol      = 70;
    	long surfaceResidueWeightCol = 71;
    	long rootWeightCol           = 72;
    ...
    //	XHappyInvariantsReplaced = XHappyInvariantsReplaced.replaceAll(this.soilInitializationPlaceholder, dummyInitializationBlock);
    ...
    //		dssatTimerLocal.tic();
    		...
    		for (int plantingWindowIndex = 0; plantingWindowIndex < nPlantingWindowsPerMonth ; plantingWindowIndex++) {
    			...
    			// mean yield / anthesis / maturity in days after planting...
    			if (useOldSummaryIndices) {
    				phenologyInDays = this.grabOLDHappyResults(nHappyPlantRunsForPhenology);
    			} else {
    ...
    //			System.out.println("       -> after grab happy " + lineIndex + " <-");
    			...
    			// make sure the streams got closed for the process after we managed to read everything...
    			// i'm not sure if we really need this any more, but i'm leaving it here in case we do...
    //			theRunningErrorStream = theRunningProcess.getErrorStream();
    //			theRunningErrorStream.close();
    //			theRunningInputStream = theRunningProcess.getInputStream();
    //			theRunningInputStream.close();
    ...
    //				initializationBlock = soilProfiles.makeInitializationBlockFractionBetweenBounds(
    //						soilTypeString,
    //						fractionBetweenLowerLimitAndDrainedUpperLimit,
    //						initializationDayCode,
    //						nitrogenPPMforBothNH4NO2);
    ...
    //		}
    	...
    	// Beware the MAGIC NUMBER!!!
    	int nCols = 13 + extraStartIndices.length; // min / max/ mean / std / bad / happy mean / happy std / real anthesis mean / real anthesis std / real maturity mean / real maturity std / happy maturity mean / happy maturity std
    	FunTricks.writeInfoFile(yieldOutputBaseName  + "_STATS", nRows, nCols, delimiter);
