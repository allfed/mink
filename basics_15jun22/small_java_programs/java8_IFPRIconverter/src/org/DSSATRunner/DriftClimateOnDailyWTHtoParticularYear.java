...
public class DriftClimateOnDailyWTHtoParticularYear {
	...
	//
	// this version is to let us specify a particular year and convert ALL of the daily weather to that year's climate
...
//		double mixingMultiplier = Double.NaN;
		...
		for (int pixelIndex = 0; pixelIndex < nValidPixels; pixelIndex++) {
			...
			if (overrideGeogFile) {
				latitudeToPullFrom = forcedLatitude; // geogMatrix.getValue(pixelIndex,2); // Beware the MAGIC NUMBER!!!
				longitudeToPullFrom = forcedLongitude; // geogMatrix.getValue(pixelIndex,3); // Beware the MAGIC NUMBER!!!
			} else {
...
//				linearMixingCoefficient = (nYearsForTransition - ((double)yearToAssumeAfterSpinup + (nNewYearsSinceBeginningOfWeatherFile - nJanuariesForSpinup - 1) - baselineClimateYear)) / nYearsForTransition;
				...
				// we are going to use the same mixing coefficient for all years so we can get a bunch of realizations for a particular climate
				linearMixingCoefficient = (nYearsForTransition - ((double)yearToAssume - baselineClimateYear)) / nYearsForTransition;
