...
public class DriftClimateOnDailyWTHtoParticularYear {

  // NEW: we want to apply climate drift according to a comparison between a baseline and future
  // climate and slap
  // that on top of the daily weather.
  //
  // then we will have to adjust the tamp and tav in hopes of keeping it reasonable
  //
  // we with *not* renumber the years nor do monthly aggregation since i want to keep that in a
  // separate step
  // to keep everything consistent....
  // this is being done starting on 15aug14.
  //
  // this version is to let us specify a particular year and convert ALL of the daily weather to
  // that year's climate
  // rather than drifting slowly along
  ...
  public static void main(String commandLineOptions[]) throws Exception {
    ...
    int baselineClimateYear = Integer.parseInt(commandLineOptions[7]);
    int futureClimateYear = Integer.parseInt(commandLineOptions[8]);
    int yearToAssume = Integer.parseInt(commandLineOptions[9]);

    // let's optionally allow us to override the latitude/longitude in the geog file..
    boolean overrideGeogFile = false;
    double forcedLatitude = Double.NaN;
    double forcedLongitude = Double.NaN;

    if (commandLineOptions.length > 10) {
      forcedLatitude = Double.parseDouble(commandLineOptions[10]);
      forcedLongitude = Double.parseDouble(commandLineOptions[11]);
      overrideGeogFile = true;
    }
    ...
    double shortwaveToUse = Double.POSITIVE_INFINITY,
        tmaxToUse,
        tminToUse,
        rainToUse = Double.NEGATIVE_INFINITY;
    ...
    */
    ...
    System.out.println(
        "-- remember: the lat/long in the geog file needs to match the weather file names... --");
    System.out.println("-- no idiot checking is being done except general failure... --");
    int formatIndexToForce = 1;
    // the data file will be assumed to be base_srad / base_tmin / base_tmax / base_rain /
    // future_srad / future_tmin / future_tmax / future_rain
    MultiFormatMatrix dataMatrix =
        MatrixOperations.read2DMFMfromTextForceFormat(
            climateTransitionBaseName + "_data", formatIndexToForce);
    MultiFormatMatrix geogMatrix =
        MatrixOperations.read2DMFMfromTextForceFormat(
            climateTransitionBaseName + "_geog", formatIndexToForce);
    ...
    for (int pixelIndex = 0; pixelIndex < nValidPixels; pixelIndex++) {
      ...
      if (overrideGeogFile) {
        latitudeToPullFrom =
            forcedLatitude; // geogMatrix.getValue(pixelIndex,2); // Beware the MAGIC NUMBER!!!
        longitudeToPullFrom =
            forcedLongitude; // geogMatrix.getValue(pixelIndex,3); // Beware the MAGIC NUMBER!!!
      } else {
      ...
      // is whether we are starting on january 1 or not. and we want to skip a few years for
      // potential spinup...
      ...
      for (int lineIndex = magicFirstLineToWorkOn;
        ...
        shortwaveHere =
            Double.parseDouble(
                originalAsArray[lineIndex].substring(magicLengthOfDate, shortwaveEnd).trim());
        tmaxHere =
            Double.parseDouble(originalAsArray[lineIndex].substring(shortwaveEnd, tmaxEnd).trim());
        tminHere =
        ...
        try {
          longwaveHere =
              Double.parseDouble(originalAsArray[lineIndex].substring(longwaveStart).trim());
        ...
        //
        // then for the first drifting year, alpha should be zero and for the last one, it should be
        // one,
        // and beyond that, it should be bigger than one...
        //
        // BUT!!! rainfall and sunshine are absolute quantities (ok, temperature is too, but we're
        // ignoring that)
        ...
        //				linearMixingCoefficient = (nYearsForTransition - (double)yearIndexAfterStart) /
        // nYearsForTransition;
        ...
        // for some reason i need a "minus one" to get the zero-change year to be the last full year
        // of spinup time...
        //				linearMixingCoefficient = (nYearsForTransition - ((double)yearToAssumeAfterSpinup +
        // (nNewYearsSinceBeginningOfWeatherFile - nJanuariesForSpinup - 1) - baselineClimateYear))
        // / nYearsForTransition;

        // we are going to use the same mixing coefficient for all years so we can get a bunch of
        // realizations for a particular climate
        linearMixingCoefficient =
            (nYearsForTransition - ((double) yearToAssume - baselineClimateYear))
                / nYearsForTransition;

        // these are the multipliers on the baseline and future references. the baseline has the
        // "-1.0" because
        // we want the changes, not the levels, so we have to subtract off the starting point for
        // the change, aka, the baseline
        // or perhaps i just had the mixing coefficient wrong...
        //					weightForBaseline = (1.0 - linearMixingCoefficient);
        //					  weightForFuture = (linearMixingCoefficient);

        // i think i had them backward initially. think of it like this: at the beginning, we are
        // zero years past the start.
        ...
        //						System.out.println("year=" + yearToUse + "; nNYSBOWF=" +
        // nNewYearsSinceBeginningOfWeatherFile +
        //								"; fakeYear=" + (yearToAssumeAfterSpinup + (nNewYearsSinceBeginningOfWeatherFile -
        // nJanuariesForSpinup - 1)) +
        //								"; lMC=" + linearMixingCoefficient + "; wF=" + weightForFuture);

        // Beware the MAGIC NUMBER!!! all the column numbers are magical at the moment...

        shortwaveBase =
            dataMatrix.getValue(new long[] {pixelIndex, nValuesPerMonth * thisMonthIndex + 0});
        tmaxBase =
            dataMatrix.getValue(new long[] {pixelIndex, nValuesPerMonth * thisMonthIndex + 1});
        tminBase =
            dataMatrix.getValue(new long[] {pixelIndex, nValuesPerMonth * thisMonthIndex + 2});
        rainBase =
            dataMatrix.getValue(new long[] {pixelIndex, nValuesPerMonth * thisMonthIndex + 3});

        shortwaveFuture =
            dataMatrix.getValue(new long[] {pixelIndex, nValuesPerMonth * thisMonthIndex + 4});
        tmaxFuture =
            dataMatrix.getValue(new long[] {pixelIndex, nValuesPerMonth * thisMonthIndex + 5});
        tminFuture =
            dataMatrix.getValue(new long[] {pixelIndex, nValuesPerMonth * thisMonthIndex + 6});
        rainFuture =
            dataMatrix.getValue(new long[] {pixelIndex, nValuesPerMonth * thisMonthIndex + 7});

        // rain and sun have a problem because we want to use multipliers and then apply them to the
        // daily
        // values which are coming, duh, from a separate source. the problems include these:
        // A: baseline == 0 && future > 0 ; daily = zero on all days (i.e., we'll never pull it up
        // off of zero); APPROACH: if nothing by day 28, put a constant boost on that day
        // B: baseline == 0 && future > 0 ; daily = nonzero on some days (i.e., how much should we
        // pull it up?); APPROACH: nonzero days get constant boost
        // C: multiplier is fine ; daily = zero on all days (i.e., we'll never pull it up off of
        // zero); APPROACH: if nothing by day 28, put a constant boost on that day
        // D: multiplier is huge ; daily >> 0 (i.e., we'll get massively inflated numbers);
        // APPROACH: set max acceptable value

        /////////////////////////////////////////
        // shortwave solar radiation: sunshine //
        /////////////////////////////////////////

        // srad should be better behaved and we should be able to scale it. we shouldn't get much of
        // significance going
        // from zero to something with sunshine and even if we do, it will be in super-marginal
        // places anyway.
        // so let's do straight fractional changes on srad with a cap to keep it sorta reasonable.

        if (shortwaveBase <= 0.0) {
          // in this case, we have a problem, how far do we move up? a multiplier won't work due to
          // division by zero
          // i'm afraid to move up to the full future amount, because it might be whacky compared to
          // its neighbors
          // in any case, these will be unfavorable conditions anyway
          if (shortwaveFuture <= 0.0) {
            shortwaveToUse = shortwaveHere;
          } else {
            // but the baseline is, by construction, zero or negative, so we don't need to list
            // it...
            //						shortwaveToUse = shortwaveHere + weightForBaseline * shortwaveBase +
            // weightForFuture * shortwaveFuture;
            shortwaveToUse = shortwaveHere + weightForFuture * shortwaveFuture;
          }
        } else {
          rawFractionalChangeShortwave = shortwaveFuture / shortwaveBase;
          shortwaveToUse =
              shortwaveHere * (1 + weightForFuture * (rawFractionalChangeShortwave - 1));
        ...
        // ok, so what we want is basically to keep track of the cumulative rainfall and srad
        // through the month
        ...
        dayOfTheMonthIndex =
            dayToUse - DSSATHelperMethods.firstPlantingDateFromMonth(thisMonthIndex + 1);

        // in leap years,

        // get the starting value for when we want to dump a bunch of rain if we are in a
        // zero->something situation
        ...
        // example: january gives me 31.0; then the floor is 31; then i subtract 1 to get to
        // index=30
        ...
        if (yearToUse % 4 == 0) {
          if (thisMonthIndex > 1) {
            ...
            //							dayOfTheMonthIndex = dayToUse -
            // DSSATHelperMethods.firstPlantingDateFromMonth(thisMonthIndex);
            dayOfTheMonthIndex--;
        ...
        if (dayOfTheMonthIndex == 0) {
          ...
          //						System.out.println(latitudeToPullFrom + "_" + longitudeToPullFrom + " INNER "+
          // nNewYearsSinceBeginningOfWeatherFile + "; year = " + yearToUse + " starting month index
          // " + thisMonthIndex + "; mDIFdeluge = " + magicDayIndexForDelugeThisMonth);
        }
        ...
        //					System.out.println("yearsAS=" + nNewYearsSinceBeginningOfWeatherFile + "; real=" +
        // yearToUse + "; day=" + dayToUse + "; monthI " +
        //							thisMonthIndex +
        //							"; swB=" + shortwaveBase + "; swF=" + shortwaveFuture + "; rB=" + rainBase + ";
        // rF=" + rainFuture +
        //							"; wFuture=" + FunTricks.fitInFiveCharacters(weightForFuture));

        // now we decide what to do. if it has already rained, we can just scale away similar to
        // sunshine.
        ...
        if (rainBase > magicTinyMonthlyRainEquivalentToZero) {
          ...
          if (rainHasFallenOnThisDayOrBefore) {
            ...
            // no daily rainfall yet, so we are working toward or are at the deluge on last day
            // scenario
            // PUT DELUGE HERE
            if (dayOfTheMonthIndex < magicDayIndexForDelugeThisMonth) {
              // we haven't gotten to the deluge day yet, so just write down zero or its close
              // equivalent being claimed for the day...
              rainToUse = rainHere;
            } else {
              // let it snow, let it snow, let it snow. or rain. whatever.
              //								System.out.println("DELUGE!!! (rainBase > small; meaning normal
              // situation)");
              //								System.out.println(latitudeToPullFrom + "_" + longitudeToPullFrom + " INNER
              // "+ nNewYearsSinceBeginningOfWeatherFile + "; year = " + yearToUse + " starting
              // month index " + thisMonthIndex + "; mDIFdeluge = " +
              // magicDayIndexForDelugeThisMonth);
          ...
          if (rainFuture <= magicTinyMonthlyRainEquivalentToZero) {
            ...
            // so, we want to do the "dump it all on the last day of the month" when there is no
            // rain

            // that is, deluge on the last day, no matter what. and just copy the other days over as
            // is.
            if (dayOfTheMonthIndex == magicDayIndexForDelugeThisMonth) {
              // it's the last day. whether the daily has rain or not, let's give it a little boost
              // let it snow, let it snow, let it snow. or rain. whatever.
              //								System.out.println("DELUGE!!! (rainBase < small; meaning zero to something
              // ugly case)");
              //								System.out.println(latitudeToPullFrom + "_" + longitudeToPullFrom + " INNER
              // "+ nNewYearsSinceBeginningOfWeatherFile + "; year = " + yearToUse + " starting
              // month index " + thisMonthIndex
              //									+ "; swB=" + shortwaveBase + "; swF=" + shortwaveFuture + "; rB=" +
              // rainBase + "; rF=" + rainFuture
              //									);
              rainToUse = weightForFuture * (rainFuture - rainBase) + rainHere;
            } else {
              // just copy it over...
        ...
        if (rainToUse > maximumAcceptableRain) {
          ...
          // added in, but after my big runs. negative rainfall should only happen in really bad
          // zero rainfall places
      ...
      dailyOutputAsArray[3] =
          upUntilTAV
              ...
              + " "
              + FunTricks.fitInNCharacters(temperatureFullAmplitude, 5)
              + afterAMP;
