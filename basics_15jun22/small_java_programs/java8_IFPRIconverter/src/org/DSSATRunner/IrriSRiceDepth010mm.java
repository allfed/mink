...
public class IrriSRiceDepth010mm implements IrrigationScheme {

  private boolean readyToCreateBlock = false;

  public IrriSRiceDepth010mm() {
  ...
  private double fixedFloodDepth =
      10.0; // mm; this is to try out different depths to see if we can get different water usages
  // and yields.
  ...
  public String buildIrrigationBlock() throws Exception {
    ...
    for (int yearIndex = 0; yearIndex < nYears; yearIndex++) {
      ...
      endOfSeason =
          DSSATHelperMethods.yyyyDDDaddDaysIgnoreLeap(plantingDate, 230 + yearlyDayOffset);
      ...
      irriBlock +=
          " 1 "
              + DSSATHelperMethods.padWithZeros(threeAfter, 5)
              + " IR011 "
              + fixedFloodDepthString
              + "\n";
      irriBlock += " 1 " + DSSATHelperMethods.padWithZeros(threeAfter, 5) + " IR009 100.0" + "\n";
      irriBlock +=
          " 1 "
              + DSSATHelperMethods.padWithZeros(sevenAfter, 5)
              + " IR011 "
              + fixedFloodDepthString
              + "\n";
