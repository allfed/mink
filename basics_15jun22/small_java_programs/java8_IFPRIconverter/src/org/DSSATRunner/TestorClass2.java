...
public class TestorClass2 {
  ...
  private static double[] monthlyWeatherAveragesDuringSeason(
      String[] originalAsArray, int actualPlantingDate, int numberOfMonthsToBeSummarized)
      throws Exception {
    // i want to develop a method that extracts monthly weather from the weather file based on the
    ...
    // monthly
    DescriptiveStatisticsUtility[] monthlySW =
        new DescriptiveStatisticsUtility[numberOfMonthsToBeSummarized];
    DescriptiveStatisticsUtility[] monthlyTmax =
        new DescriptiveStatisticsUtility[numberOfMonthsToBeSummarized];
    DescriptiveStatisticsUtility[] monthlyTmin =
        new DescriptiveStatisticsUtility[numberOfMonthsToBeSummarized];
    DescriptiveStatisticsUtility[] monthlyPrec =
        new DescriptiveStatisticsUtility[numberOfMonthsToBeSummarized];
    DescriptiveStatisticsUtility[] monthlyRainydays =
        new DescriptiveStatisticsUtility[numberOfMonthsToBeSummarized];
    ...
    // we will need to start at the beginning of the month that planting takes place in even if it
    ...
    int dateCodeForWhereWeWantToStartAsInteger =
        actualPlantingYearNumber * 1000 + firstDayOfMonthTheActualPlantingDateFallsIn;
    ...
    int firstDayOfMonthWeDoNotWant =
        DSSATHelperMethods.firstPlantingDateFromMonth(firstMonthWeDoNotWant);
    int dateCodeForWhereWeWantToStopAsInteger =
        firstYearWeDoNotWant * 1000 + firstDayOfMonthWeDoNotWant;

    // the weather generator pads with spaces instead of zeros. we could parse to integer and
    // compare, but that seems like more work than we want....
    // going manual....
    ...
    // "]");
    //	System.out.println("monthTheActualPlantingDateFallsIn = [" +
    // monthTheActualPlantingDateFallsIn + "]");
    //	System.out.println("firstDayOfMonthTheActualPlantingDateFallsIn = [" +
    // firstDayOfMonthTheActualPlantingDateFallsIn + "]");
    //	System.out.println("dateCodeForWhereWeWantToStartAsInteger = [" +
    // dateCodeForWhereWeWantToStartAsInteger + "]");
    //	System.out.println("dateCodeForWhereWeWantToStart = [" + dateCodeForWhereWeWantToStart +
    // "]");
    //	System.out.println("dateCodeForWhereWeWantToStopAsInteger = [" +
    // dateCodeForWhereWeWantToStopAsInteger + "]");
    ...
    for (int lineIndex = magicFirstLineToWorkOn; lineIndex < originalAsArray.length; lineIndex++) {
      ...
      if (originalAsArray[lineIndex]
          .substring(0, magicLengthOfDate)
          .equals(dateCodeForWhereWeWantToStart)) {
        firstLineOfPlantingMonth = lineIndex;
      }
      if (originalAsArray[lineIndex]
          .substring(0, magicLengthOfDate)
          .equals(dateCodeForWhereWeWantToStop)) {
    ...
    for (int lineIndex = firstLineOfPlantingMonth; lineIndex < firstLineWeDoNotWant; lineIndex++) {
      ...
      storageMonthIndex =
          monthIndexHere
              - monthIndexTheActualPlantingDateFallsIn
              + nMonths
                  * (yearInFile - actualPlantingYearNumber); // the month was recorded as a number

      //		System.out.println("monthIndexHere = " + monthIndexHere);
      //		System.out.println("monthIndexTheActualPlantingDateFallsIn = " +
      // monthIndexTheActualPlantingDateFallsIn);
  ...
  public static void main(String commandLineOptions[]) throws Exception {
    ...
    if (doThis) {
      ...
      int dateHere =
          (dayNumberYearPair[0] - (dayNumberYearPair[0] / 100) * 100) + 1000 * dayNumberYearPair[1];
    ...
    if (7 == 8) {
      ...
      outputStuff =
          monthlyWeatherAveragesDuringSeason(weatherFileContents, plantingDate, howManyToSummarize);
      ...
      System.out.println();
      System.out.println(
          "custom rice irrigation scheme, found this (hopefully) CSV: ["
      ...
      //	    String magicDSSATSummaryToReadPath =
      // "C:\\rdrobert\\breaking_DSSAT\\dssat_and_emissions\\Summary_OLD.OUT";
      String magicDSSATSummaryToReadPath =
          "C:\\rdrobert\\breaking_DSSAT\\dssat_and_emissions\\Summary_N2O.OUT";
      ...
      String[] splitUpNames =
          FunTricks.readLineToArrayOnArbitrarySpaceOrTab(
              candidateSummaryContents[magicDSSATSummaryLineIndexToRead - 1].substring(1));
      ...
      for (int nameIndex = 0; nameIndex < splitUpNames.length; nameIndex++) {
        endingIndicesForSplitUpNames[nameIndex] =
            candidateSummaryContents[magicDSSATSummaryLineIndexToRead - 1].indexOf(
                    splitUpNames[nameIndex], previousStartIndex)
        ...
        System.out.println(
            "- " + nameIndex + " - eP=" + endPrevious + "  eT=" + endThis + "  lT=" + lengthThis);

        testExtraction =
            candidateSummaryContents[magicDSSATSummaryLineIndexToRead - 1].substring(
                endPrevious, endThis);
        System.out.println(
            "sUN["
                + nameIndex
                + "] = ["
                + splitUpNames[nameIndex]
                + "] beginning at ["
                + endPrevious
                + "]");
    ...
    if (5 == 6) {
      ...
      System.out.println(nitrogenFertilizerScheme.buildNitrogenOnlyBlock(120, 140, 21));
        ...
        System.out.println(
            "start = ["
                ...
                + DSSATHelperMethods.padWithZeros(
                    DSSATHelperMethods.yyyyDDDaddDaysIgnoreLeap(startingDate, moreDays), 5)
                + "]");
      }
      ...
      String uglyLine =
          "       20      1  1  1  0 SB CRGRO045 RI                        GRID     RRRR.WTH"
              + " HN_GEN0010 2024155 2024244 2024286     -99     -99 2024303    56 *******       0 "
              + "      0       0     0  0.0000     0     0.0 0.000   0.0     0     0  1001   253   "
              + "  1   252   126   523     0     1     0     1     0    26    34     1     0   -99 "
              + "  -99   -99   -99   -99   -99   -99   -99     0  31890  31890      0      0 "
              + " 227848  227848***************************    -99.0     0.00     0.00     0.00  "
              + " -99.00    -99.0    -99.0    -99.0    -99.0    59  12.4   4.5  15.9 12.43  369.0 "
              + " 230.4  140.6";
      ...
      System.out.println(
          "-->[" + funnyValue + "] -> [" + FunTricks.fitInNCharactersOLD(funnyValue, 5) + "]");
      System.out.println(
          "-->[" + funnyValue + "] -> [" + FunTricks.fitInNCharacters(funnyValue, 5) + "] (new)");
      ...
      //				System.out.println("[" + funnyValue + "] -> [" +
      // FunTricks.fitInNCharactersNew(funnyValue, 5) + "] (new)");
      //			}
      //			funnyValue += stepsize;
      //		}
    ...
    if (false) {
      ...
      System.out.println(
          "# of decimals = "
              + nDecimals
              + "; value = ["
              + FunTricks.onlySomeDecimalPlaces(numberthing, nDecimals)
              + "]");
      ...
      DrawFromCDF drawObject =
          new DrawFromCDF(
              "D:\\rdrobert\\zambia_household_geographic\\text\\first_toy_elevation_quantiles.txt",
              ":");
