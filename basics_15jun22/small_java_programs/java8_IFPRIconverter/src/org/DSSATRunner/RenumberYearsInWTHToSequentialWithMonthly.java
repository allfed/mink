...
public class RenumberYearsInWTHToSequentialWithMonthly {
  ...
  // tingju's IMPACT Global Hydrologic Model in the funky format he needs. this necessitates peeling
  // off
  // the long-wave radiation numbers he needs but i do not... i don't really need to peel them off
  // (DSSAT
  ...
  private static String kickOutCleanAverage(
      DescriptiveStatisticsUtility[] monthlyAccumulator,
      int monthIndex,
      String missingValueString,
      int nDecimalsToReport) {
    String valueToRecord = null;
    // need the long cast in there because february is 28.24...
    if (monthlyAccumulator[monthIndex].getN()
        < (long) DSSATHelperMethods.averageDaysInMonth[monthIndex]) {
      valueToRecord = missingValueString;
    } else {
      valueToRecord =
          FunTricks.onlySomeDecimalPlaces(
              monthlyAccumulator[monthIndex].getMean(), nDecimalsToReport);
    }
  ...
  public static void main(String commandLineOptions[]) throws Exception {
    ...
    */
    ...
    for (int pixelIndex = 0; pixelIndex < rawPixelListLines.length; pixelIndex++) {
      pixelLineArray =
          FunTricks.parseRepeatedDelimiterToArray(rawPixelListLines[pixelIndex], delimiter);
    ...
    for (int pixelIndex = 0; pixelIndex < nValidPixels; pixelIndex++) {
      ...
      //			  RICK   -55.75   -67.25  -999   5.7   2.2  -999  -999
      //			                             "   0.1"
      // i want to check to see if TAV is zero or negative and then put it to an ever so slightly
      // positive value
      ...
      if (Double.parseDouble(
              originalAsArray[3].substring(magicTAVStartIndex, magicAMPStartIndex).trim())
          <= 0.0d) {
        // the value is negative, so we need to replace it with a suitable minimally positive value
        dailyOutputAsArray[3] =
            originalAsArray[3].substring(0, magicTAVStartIndex)
                + "   0.1"
                + originalAsArray[3].substring(magicAMPStartIndex);
      }
      // the value is positive,so carry on

      // build the monthly header...
      monthlyHeader =
          pixelIDNumberToPullFrom
              + delimiter
              + longitudeToPullFrom
              + delimiter
              + latitudeToPullFrom;
      ...
      // check for weirdness: we might start at the very end of a leap year in the data written
      // down, but
      ...
      for (int lineIndex = magicFirstLineToWorkOn;
        ...
        // Beware the MAGIC ASSUMPTION!!! we think DSSAT will interpret 00 as 2000 which *was* a
        // leap year
        // but 2100 will not be. grrr... two digit dates. so don't worry about century correction...

        // ok, so we need to decide when to turn the fake year over. that will happen when the day
        // to use
        // is bigger than the usual 365.
        if (dayToUse > 365) {
          // normal years need to bump up the year and reset the day while leap years should carry
          // on to 366.
          if (yearToUse % 4 != 0) {
            // normal year
        ...
        if (timeToDumpMonthlyDataForPrecedingYear) {
          ...
          for (int monthIndex = 0; monthIndex < nMonths; monthIndex++) {
            ...
            //						giantMonthlyShortwave   += delimiterToUse +
            // kickOutCleanAverage(monthlyShortwave,   monthIndex, missingValueString,
            // nDecimalsToReport);
            //						giantMonthlyTmax += delimiterToUse + kickOutCleanAverage(monthlyTmax,
            // monthIndex, missingValueString, nDecimalsToReport);
            //						giantMonthlyTmin += delimiterToUse + kickOutCleanAverage(monthlyTmin,
            // monthIndex, missingValueString, nDecimalsToReport);
            //						giantMonthlyPrec += delimiterToUse + kickOutCleanAverage(monthlyPrec,
            // monthIndex, missingValueString, nDecimalsToReport);
            //						giantMonthlyLongwave   += delimiterToUse + kickOutCleanAverage(monthlyLongwave,
            //  monthIndex, missingValueString, nDecimalsToReport);

            giantMonthlyShortwave +=
                delimiterToUse
                    + kickOutCleanAverage(
                        monthlyShortwave, monthIndex, missingValueString, nDecimalsToReport);
            giantMonthlyTmax +=
                delimiterToUse
                    + kickOutCleanAverage(
                        monthlyTmax, monthIndex, missingValueString, nDecimalsToReport);
            giantMonthlyTmin +=
                delimiterToUse
                    + kickOutCleanAverage(
                        monthlyTmin, monthIndex, missingValueString, nDecimalsToReport);
            giantMonthlyPrec +=
                delimiterToUse
                    + kickOutCleanAverage(
                        monthlyPrec, monthIndex, missingValueString, nDecimalsToReport);
            giantMonthlyLongwave +=
                delimiterToUse
                    + kickOutCleanAverage(
                        monthlyLongwave, monthIndex, missingValueString, nDecimalsToReport);
        ...
        //				System.out.println("file = " + thisInputFile + "; lineIndex = " + lineIndex + " [" +
        // originalAsArray[lineIndex] + "]");
        // grab the info
        try {
          shortwaveHere =
              Double.parseDouble(
        ...
        try {
          tmaxHere =
              Double.parseDouble(
                  originalAsArray[lineIndex].substring(shortwaveEnd, tmaxEnd).trim());
        } catch (NumberFormatException eee) {
        ...
        // moving the daily out put down here because i want to do the obvious checks for
        // reasonablity...
        // shortwave
      ...
      // check to see if we have gone through a partial year. if so, then we need to write out the
      // monthly stuff...
      // write down the info and reset the counters...
      timeToDumpMonthlyDataForPrecedingYear = false;
      for (int monthIndex = 0; monthIndex < nMonths; monthIndex++) {
