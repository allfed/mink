  ...
  public static void main(String commandLineOptions[]) throws Exception {
    ...
    for (int lineIndex = magicFirstLineToWorkOn; lineIndex < originalAsArray.length; lineIndex++) {
      ...
      if (yearInFile != previousYearInFile) {
        ...
        FunTricks.appendLineToTextFile(
            "approximateRelativeHumidityAtDawn = [" + approximateRelativeHumidityAtDawn + "]",
            particularProvenanceFilename,
            true); // append from there...
      }
    ...
    for (int fileIndex = 0; fileIndex < inputFiles.length; fileIndex++) {
      ...
      for (int lineIndex = magicFirstLineToWorkOn;
        ...
        if (previousYearInFile != initForPreviousYearInFile && previousYearInFile != yearInFile) {
          ...
          //		    particularOutputFilename = destinationFilename +
          // DSSATHelperMethods.padWithZeros(yearInFile, 2) + ".txt"; // this is the original ?wrong
          // way: i am writing it out under the wrong year number...
          // and also i was forgetting to finish out the last year under its own name after the loop
          // is done...

          particularOutputFilename =
              destinationFilename + DSSATHelperMethods.padWithZeros(previousYearInFile, 2) + ".txt";

          outputLine = latitude + "," + longitude; // + "," + notInAnyBin;

          // output the THI_min by month
          for (int monthIndex = 0; monthIndex < nMonths; monthIndex++) {
            // use a NaN since GRASS can interpret that (unlike DSSAT, hence my
            // onlySomeDecimalPlaces will return -0.01 for weird stuff)
              ...
              outputLine +=
                  ","
                      + FunTricks.onlySomeDecimalPlaces(
                          lowTHI[monthIndex].getMean(), nDecimalsToKeep);
            }
          }
          ...
          for (int monthIndex = 0; monthIndex < nMonths; monthIndex++) {
            ...
            if (highTHI[monthIndex].getN() == 0) {
              ...
              outputLine +=
                  ","
                      + FunTricks.onlySomeDecimalPlaces(
                          highTHI[monthIndex].getMean(), nDecimalsToKeep);
            }
          }
        ...
        // but, for the THI_min, we really want the daybreak THI and our (super simplistic)
        // assumption is that
        // it is roughly 100% humidity at dawn. not always true, but probably good enough when it is
        // warm and not completely dry
        ...
        if (tminHereInF >= lowestReasonableTemperatureForFiguringTHIinF) {
          approximateTHIAtDawn =
              tminHereInF
                  - (0.55 - 0.55 * approximateRelativeHumidityAtDawn / 100) * (tminHereInF - 58);
        } else {
      ...
      // output the THI_min by month
      for (int monthIndex = 0; monthIndex < nMonths; monthIndex++) {
        // use a NaN since GRASS can interpret that (unlike DSSAT, hence my onlySomeDecimalPlaces
        // will return -0.01 for weird stuff)
        if (lowTHI[monthIndex].getN() == 0) {
          outputLine += ",NaN";
        } else {
          outputLine +=
              "," + FunTricks.onlySomeDecimalPlaces(lowTHI[monthIndex].getMean(), nDecimalsToKeep);
        }
      }
      ...
      for (int monthIndex = 0; monthIndex < nMonths; monthIndex++) {
        ...
        if (highTHI[monthIndex].getN() == 0) {
          ...
          outputLine +=
              "," + FunTricks.onlySomeDecimalPlaces(highTHI[monthIndex].getMean(), nDecimalsToKeep);
        }
      }
