...
public class Mink0_droughtcompare {
...
//	private int cropToUseInt = -3;
	...
	private IrrigationScheme riceNonIrrigationScheme = null;
...
//	private boolean badThingsHappened = false;
	...
	public static String usageMessage() {
		...
		String usageMessage =
			"OLD and almost assuredly incorrect (as of 10may13)\n\n" +
...
//			throw new Exception();
		...
		if (irrigationSchemeToUse.equalsIgnoreCase(this.maizeString)) {
			...
			riceNonIrrigationScheme = new IrriSRiceUnfloodedPaddy();
...
//					System.out.println("  mf");
	...
	*/
	...
	private int parseIntegerResults(String[] stringArray,int start,int end) {
		int valueParsed = -9;
		try {
			valueParsed = Integer.parseInt(stringArray[magicDSSATSummaryLineIndexToRead].substring(start,end).trim());
		}	catch (NumberFormatException nfe) {
			String badVariableName = stringArray[magicDSSATSummaryLineIndexToRead - 1].substring(start,end).trim();
			if (stringArray[magicDSSATSummaryLineIndexToRead].substring(start,end).trim().contains("*")) {
				// ok, let's try this again, but we'll just skip over the offending value but keep the others from this line...
				System.out.println("REAL got *'s [" + badVariableName + "] as an integer; fail = ["
						+ stringArray[magicDSSATSummaryLineIndexToRead].substring(start,end).trim() + "]");
				valueParsed = Integer.MIN_VALUE;
			} else {
				nfe.printStackTrace();
				System.out.println("REAL had trouble reading [" + badVariableName + "] as an integer; fail = ["
						+ stringArray[magicDSSATSummaryLineIndexToRead].substring(start,end).trim() + "]");
				valueParsed = Integer.MIN_VALUE + 1;
			}
		}
	...
	private double parseDoubleResults(String[] stringArray,int start,int end) {
		double valueParsed = -9;
		try {
			valueParsed = Double.parseDouble(stringArray[magicDSSATSummaryLineIndexToRead].substring(start,end).trim());
		}	catch (NumberFormatException nfe) {
			String badVariableName = stringArray[magicDSSATSummaryLineIndexToRead - 1].substring(start,end).trim();
			if (stringArray[magicDSSATSummaryLineIndexToRead].substring(start,end).trim().contains("*")) {
				// ok, let's try this again, but we'll just skip over the offending value but keep the others from this line...
				System.out.println("REAL got *'s [" + badVariableName + "] as a double; fail = ["
						+ stringArray[magicDSSATSummaryLineIndexToRead].substring(start,end).trim() + "]");
				valueParsed = Double.NaN;
			} else {
				nfe.printStackTrace();
				System.out.println("REAL had trouble reading [" + badVariableName + "] as a double; fail = ["
						+ stringArray[magicDSSATSummaryLineIndexToRead].substring(start,end).trim() + "]");
				valueParsed = Double.NaN;
			}
		}
	...
	private void grabSingleYearRealResults(boolean useOldSummaryIndices) throws InterruptedException, Exception {
		// declarations
		int plantingDate = -1;
		int startingDate = -2;
		int daysSincePlantingForEvent = -3;
...
//		String[] hereextraNames;
		...
		// declarations with initializations
		final int nYears = 1; // Beware the MAGIC NUMBER!!! this is the single year definition...
		int nLinesToRead = nYears + magicDSSATSummaryLineIndexToRead;
		String[] candidateSummaryContents = new String[nLinesToRead];
		...
		// yield
		yieldToUse = parseIntegerResults(candidateSummaryContents,heremagicHarvestedWeightAtHarvestStartIndex,heremagicHarvestedWeightAtHarvestEndIndex);

		// planting / anthesis / maturity dates
		if (!useOldSummaryIndices) {
			emergenceToUse = parseIntegerResults(candidateSummaryContents,magicEmergenceDateStartIndex,magicEmergenceDateEndIndex);
		} else {
			emergenceToUse = -1;
		}
		startingDate  = parseIntegerResults(candidateSummaryContents,heremagicStartingDateStartIndex,heremagicStartingDateEndIndex);
		plantingDate  = parseIntegerResults(candidateSummaryContents,heremagicPlantingDateStartIndex,heremagicPlantingDateEndIndex);
		anthesisToUse = parseIntegerResults(candidateSummaryContents,heremagicAnthesisDateStartIndex,heremagicAnthesisDateEndIndex);
		maturityToUse = parseIntegerResults(candidateSummaryContents,heremagicMaturityDateStartIndex,heremagicMaturityDateEndIndex);
		if (maturityToUse < 0) {
			maturityToUse = parseIntegerResults(candidateSummaryContents,heremagicHarvestingDateStartIndex,heremagicHarvestingDateEndIndex);
		}

		// all the other goodies...
		for (int extraIndex = 0; extraIndex < hereextraStartIndices.length; extraIndex++) {
			extractedValues[extraIndex] = parseDoubleResults(candidateSummaryContents,hereextraStartIndices[extraIndex],hereextraEndIndices[extraIndex]);
			if (Double.isNaN(extractedValues[extraIndex])) { // Beware the MAGIC NUMBER!!! this is to catch any very small values used to indicate errors....
				realBadThingsCountsEntirePixel.useLongValue(1000000000); // assess that something bad happened...
			}
		}
...
//				System.out.println("  mf");
	...
	private int[] grabSingleYearHappyResults(boolean useOldSummaryIndices) throws InterruptedException, Exception {
		// declarations
...
//		String stringToParse = null;
		...
		int daysToAnthesis = -1;
		int daysToMaturity = -1;
			...
			// for the moment...
			if (anthesisToUse > 0 && plantingDate > 0) {
				daysToAnthesis = DSSATHelperMethods.yyyyDDDdifferencerIgnoreLeap(plantingDate, anthesisToUse);
			} else {
				daysToAnthesis = 0;
			}
			...
			if (maturityToUse > 0 && plantingDate > 0) {
				//					if (maturityToUse != magicMissingValue && plantingDate > 0) {
				daysToMaturity = DSSATHelperMethods.yyyyDDDdifferencerIgnoreLeap(plantingDate, maturityToUse);
			}
...
//		final int nFriends = 3;
		final int nFriends = 4; // adding in PRCM
		HistoStatisticsUtility ircmStatUtility = new HistoStatisticsUtility(nFriends); // three friends to keep track of...
		double IRCMnormal, PRCMnormal, yieldIRnormal, yieldRFnormal, yieldRFdrought;
		double[][] goofyQuantileThing = null;
		...
		// let the MAGIC begin...
		// we need to read in two X-files here: one RF and one IR
		// let us assume that we have been given the IR one,
		
		String templateXFileContents = FunTricks.readTextFileToString(templateXFile);
		String templateXFileContentsRF = FunTricks.readTextFileToString(templateXFile.replaceAll("IR", "RF"));
		...
		String irrigationBlockUnfloodedPaddyRice  = null;
...
//		String randomSeedCode = null;
		...
		String soilTypeStringDrought = null;
		...
		String XStuffToWriteRFnormal = null;
		String XStuffToWriteRFdrought = null;
...
//		final int nQuantiles = 5;
		final int nQuantiles = 21;
		
		System.out.println("BE AWARE: we are using " + nQuantiles + " quantiles");
...
//		nHappyYearsCode = DSSATHelperMethods.padWithZeros(nHappyPlantRunsForPhenology, 5);
		...
		SoilProfile soilProfilesDrought = new SoilProfile("HD" + ".SOL"); 
...
//		nitrogenPPMforBothNH4NO2);
		...
		String[] invariantsReplacedRF = this.buildUniqueInvariantsXTemplates(templateXFileContentsRF, this.randomSeedPlaceholder, nHappyYearsCode, co2ppmCode, dummyScheme, nYearsCode);
...
//		String XHappyInvariantsReplacedRF = invariantsReplacedRF[0];
		String XInvariantsReplacedRF      = invariantsReplacedRF[1];
		...
		for (int lineIndex = 0; lineIndex < nLinesInDataFile; lineIndex++) {
			...
			final int totalLengthForSoilType = 10;
			...
			soilTypeStringDrought = (magicSoilPrefix + FunTricks.padStringWithLeadingZeros(Integer.toString(soilType), totalLengthForSoilType - magicSoilPrefix.length())
					).replaceAll("HN", "HD");
...
//					theRunningErrorStream = theRunningProcess.getErrorStream();   theRunningErrorStream.close();
//					theRunningInputStream = theRunningProcess.getInputStream();   theRunningInputStream.close();
//					theRunningOutputStream = theRunningProcess.getOutputStream(); theRunningOutputStream.close();
...
//					phenologyInDays = this.grabUnifiedHappyResults(1, useOldSummaryIndices);
					phenologyInDays = this.grabSingleYearHappyResults(useOldSummaryIndices);
				...
				if (phenologyInDays[0] >= this.happyYieldThresholdToDoRealRuns && phenologyInDays[2] <= this.happyMaturityThresholdToDoRealRuns) {
					...
					for (int normalPlantRunIndex = 0; normalPlantRunIndex < this.nRandomSeedsToUse; normalPlantRunIndex++) {
						...
						if (irrigationSchemeToUse.equalsIgnoreCase(this.riceString)) {
							riceNonIrrigationScheme.initialize(Integer.parseInt(startingDayToPlantCode), 1);
							irrigationBlockUnfloodedPaddyRice = riceNonIrrigationScheme.buildIrrigationBlock();
						}

						// do the search and replace thing; the invariants have already been done above...
						XStuffToWrite = XInvariantsReplaced.replaceAll(soilPlaceholder , soilTypeString);
						XStuffToWriteRFnormal = XInvariantsReplacedRF.replaceAll(soilPlaceholder , soilTypeString);
						XStuffToWriteRFdrought = XInvariantsReplacedRF.replaceAll(soilPlaceholder , soilTypeStringDrought);
						// put in the random seed to use
						...
						// the normal rainfed one
						XStuffToWriteRFnormal = XStuffToWriteRFnormal.replaceAll(this.randomSeedPlaceholder,randomSeedActualValue);
						XStuffToWriteRFnormal =       XStuffToWriteRFnormal.replaceAll(initializationStartPlaceholder, initializationDayCode);
						XStuffToWriteRFnormal =       XStuffToWriteRFnormal.replaceAll(plantingDateStartPlaceholder  , startingDayToPlantCode);
						XStuffToWriteRFnormal =       XStuffToWriteRFnormal.replaceAll(plantingDateEndPlaceholder    , endingDayToPlantCode);
						XStuffToWriteRFnormal =       XStuffToWriteRFnormal.replaceAll(harvestingDatePlaceholder     , harvestDayCode);
						XStuffToWriteRFnormal =       XStuffToWriteRFnormal.replaceAll(fertilizerPlaceholder, fertilizerBlock);
						if (irrigationSchemeToUse.equalsIgnoreCase(this.riceString)) {
							XStuffToWriteRFnormal =       XStuffToWriteRFnormal.replaceAll(irrigationPlaceholder, irrigationBlockUnfloodedPaddyRice);
						} else {
							XStuffToWriteRFnormal =       XStuffToWriteRFnormal.replaceAll(irrigationPlaceholder, irrigationBlock);
						}
						...
						XStuffToWriteRFnormal = XStuffToWriteRFnormal.replaceAll(soilInitializationPlaceholder, initializationBlock);
						
						
						
						// the drought rainfed one
						XStuffToWriteRFdrought = XStuffToWriteRFdrought.replaceAll(this.randomSeedPlaceholder,randomSeedActualValue);
						XStuffToWriteRFdrought =       XStuffToWriteRFdrought.replaceAll(initializationStartPlaceholder, initializationDayCode);
						XStuffToWriteRFdrought =       XStuffToWriteRFdrought.replaceAll(plantingDateStartPlaceholder  , startingDayToPlantCode);
						XStuffToWriteRFdrought =       XStuffToWriteRFdrought.replaceAll(plantingDateEndPlaceholder    , endingDayToPlantCode);
						XStuffToWriteRFdrought =       XStuffToWriteRFdrought.replaceAll(harvestingDatePlaceholder     , harvestDayCode);
						XStuffToWriteRFdrought =       XStuffToWriteRFdrought.replaceAll(fertilizerPlaceholder, fertilizerBlock);
						if (irrigationSchemeToUse.equalsIgnoreCase(this.riceString)) {
							XStuffToWriteRFdrought =       XStuffToWriteRFdrought.replaceAll(irrigationPlaceholder, irrigationBlockUnfloodedPaddyRice);
						} else {
							XStuffToWriteRFdrought =       XStuffToWriteRFdrought.replaceAll(irrigationPlaceholder, irrigationBlock);
						}
...
//						XStuffToWriteRFdrought =       XStuffToWriteRFdrought.replaceAll(irrigationPlaceholder, irrigationBlock);
						...
						initializationBlock = soilProfilesDrought.makeInitializationAndSoilAnalysisBlock(
								soilTypeStringDrought,
								fractionBetweenLowerLimitAndDrainedUpperLimit,
								initializationDayCode,
								totalInitialNitrogenKgPerHa,
								depthForNitrogen,
						...
						initializationBlock = "! " + soilTypeStringDrought + " " +
						fractionBetweenLowerLimitAndDrainedUpperLimit + " " +
						initializationDayCode + " " +
						totalInitialNitrogenKgPerHa + " " +
						depthForNitrogen + " " +
						...
						XStuffToWriteRFdrought = XStuffToWriteRFdrought.replaceAll(soilInitializationPlaceholder, initializationBlock);
						...
						// attempt to read
						candidateSummaryContents = FunTricks.readSomeLinesOfTextFileToArray(magicDSSATSummaryToReadPath,nLinesToRead);
						yieldIRnormal = this.parseDoubleResults(candidateSummaryContents, magicHarvestedWeightAtHarvestStartIndex, magicHarvestedWeightAtHarvestEndIndex);
						   IRCMnormal = this.parseDoubleResults(candidateSummaryContents, extraStartIndices[12], extraEndIndices[12]); // Beware the MAGIC NUMBER!!! 12 should be IRCM
						   PRCMnormal = this.parseDoubleResults(candidateSummaryContents, extraStartIndices[13], extraEndIndices[13]); // Beware the MAGIC NUMBER!!! 12 should be IRCM
						...
						// RF normal
						FunTricks.writeStringToFile(XStuffToWriteRFnormal, fullTempXFile);
						realTimer.tic();
						theRunningProcess = Runtime.getRuntime().exec(dssatExecutionCommand , null , pathToDSSATDirectoryAsFile);
						...
						// attempt to read
						candidateSummaryContents = FunTricks.readSomeLinesOfTextFileToArray(magicDSSATSummaryToReadPath,nLinesToRead);
						yieldRFnormal = this.parseDoubleResults(candidateSummaryContents, magicHarvestedWeightAtHarvestStartIndex, magicHarvestedWeightAtHarvestEndIndex);
						...
						// RF drought
						FunTricks.writeStringToFile(XStuffToWriteRFdrought, fullTempXFile);
						realTimer.tic();
						theRunningProcess = Runtime.getRuntime().exec(dssatExecutionCommand , null , pathToDSSATDirectoryAsFile);
						...
						// attempt to read
						candidateSummaryContents = FunTricks.readSomeLinesOfTextFileToArray(magicDSSATSummaryToReadPath,nLinesToRead);
						yieldRFdrought = this.parseDoubleResults(candidateSummaryContents, magicHarvestedWeightAtHarvestStartIndex, magicHarvestedWeightAtHarvestEndIndex);
						...
						ircmStatUtility.useDoublePair(IRCMnormal, new double[] {PRCMnormal,yieldIRnormal,yieldRFnormal,yieldRFdrought} );
			...
			// a little brute force MAGIC!!!
			statisticsOutLine =
				ircmStatUtility.getN() + delimiter
				+ happyYieldsEntirePixel.getMean() + delimiter 
				+ happyMaturityEntirePixel.getMean() + delimiter
				+ ircmStatUtility.getMeans()[0] + delimiter
				+ ircmStatUtility.getMeans()[1] + delimiter
				+ ircmStatUtility.getMeans()[2] + delimiter
				+ ircmStatUtility.getMeans()[3] + delimiter
				+ ircmStatUtility.getMeans()[4] + delimiter;
//				+ ircmStatUtility.getMeans()[3] + delimiter;
			
			goofyQuantileThing = ircmStatUtility.getGoofyMediansWithinQuantileThing(nQuantiles);
			for (int colIndex = 0; colIndex < nFriends + 1; colIndex++) {
				for (int quantileIndex = 0; quantileIndex < nQuantiles; quantileIndex++) {
					statisticsOutLine += goofyQuantileThing[quantileIndex][colIndex] + delimiter;
				}
			}
...
//				averageTimePerEvent = timeForWeather.getMean() + timeForORYZA.getMean();
		...
		String columnList = "n_valid" + delimiter 
		+ "happy_yield_mean" + delimiter + "happy_maturity_mean" + delimiter 
		+ "IRCM" + delimiter + "PRCM" + delimiter + "IRnormal_yield_mean" + delimiter 
		+ "RFnormal_yield_mean" + delimiter + "RFdrought_yield_mean" + delimiter;

		for (int quantileIndex = 0; quantileIndex < nQuantiles; quantileIndex++) {
			columnList += "IRCM_" + quantileIndex + delimiter;
		}
		for (int quantileIndex = 0; quantileIndex < nQuantiles; quantileIndex++) {
			columnList += "PRCM_" + quantileIndex + delimiter;
		}
		for (int quantileIndex = 0; quantileIndex < nQuantiles; quantileIndex++) {
			columnList += "IRnormal_yield_" + quantileIndex + delimiter;
		}
		for (int quantileIndex = 0; quantileIndex < nQuantiles; quantileIndex++) {
			columnList += "RFnormal_yield_" + quantileIndex + delimiter;
		}
		for (int quantileIndex = 0; quantileIndex < nQuantiles; quantileIndex++) {
			columnList += "RFdrought_yield_" + quantileIndex + delimiter;
		}
		...
		// Beware the MAGIC NUMBER!!!
//		int nCols = 7 + nQuantiles * 4;
		int nCols = 8 + nQuantiles * 5;
	...
	public void doSimulationsUniqueSeedsAllClimateSuppliedOriginal(boolean useOldSummaryIndices) throws Exception {
...
//					phenologyInDays = this.grabUnifiedHappyResults(1, useOldSummaryIndices);
				...
				if (phenologyInDays[0] >= this.happyYieldThresholdToDoRealRuns && phenologyInDays[2] <= this.happyMaturityThresholdToDoRealRuns) {
					...
					for (int normalPlantRunIndex = 0; normalPlantRunIndex < this.nRandomSeedsToUse; normalPlantRunIndex++) {
						...
						this.grabSingleYearRealResults(useOldSummaryIndices);
