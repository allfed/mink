...
public class Mink3p2daily {
  ...
  // originally, we "deleteme.SNX" here, but this failed when trying to use CROPSIM wheat; now, we
  // will try "./deleteme.SNX"
  // and hopefully, that will be sufficiently robust for a while
  //      public static final String tempXFileName                    = "deleteme.SNX";
  public static final String tempXFileName = "./deleteme.SNX";
  // and down here, we have to get rid of two spaces (now occupied by the ./) so that the
  // fixed-spacing is still satisfied.
  ...
  // Beware the MAGIC ASSUMPTION!!! assuming soil codes must fit into 10 characters padded with
  // zeros
  ...
  //  private static int magicNumberOfOutputsExpected = magicOffsetForExtraNamesOutputIndex +
  // extraNames.length;
  ...
  private static final String middleHeavyThreeSplitWithFloweringString =
      "middleHeavyThreeSplitWithFlowering";
  ...
  private DescriptiveStatisticsUtility realYieldsExactlyZeroEntirePixel =
      new DescriptiveStatisticsUtility(false);
  private DescriptiveStatisticsUtility happyYieldsEntirePixel =
      new DescriptiveStatisticsUtility(false);

  private DescriptiveStatisticsUtility realTimeToPlantingEntirePixel =
      new DescriptiveStatisticsUtility(false);
  private DescriptiveStatisticsUtility realNoPlantingEntirePixel =
      new DescriptiveStatisticsUtility(false);
  ...
  private static final long yieldForMissingWeatherFile = -37L;
  ...
  private static final String[] beansCleanedGrowthStages = {
  ...
  private static final String[] groundnutsCleanedGrowthStages = {
    magicExtraPrefixForGrowthStages + "_" + "Emergence__First_Flower",
    magicExtraPrefixForGrowthStages + "_" + "First_Flower_First_Seed",
    magicExtraPrefixForGrowthStages + "_" + "First_Seed___Phys__Mat_",
    magicExtraPrefixForGrowthStages + "_" + "Emergence____Phys__Mat_",
  ...
  private static final String[] sorghumCleanedGrowthStages = {
    magicExtraPrefixForGrowthStages + "_" + "Emergence_End_Juvenile",
  ...
  private static final String[] soybeansCleanedGrowthStages = {
    magicExtraPrefixForGrowthStages + "_" + "Emergence__First_Flower",
    magicExtraPrefixForGrowthStages + "_" + "First_Flower_First_Seed",
    magicExtraPrefixForGrowthStages + "_" + "First_Seed___Phys__Mat_",
    magicExtraPrefixForGrowthStages + "_" + "Emergence____Phys__Mat_",
  ...
  private static final String[] chickpeasCleanedGrowthStages = {
    magicExtraPrefixForGrowthStages + "_" + "Emergence__First_Flower",
    magicExtraPrefixForGrowthStages + "_" + "First_Flower_First_Seed",
    magicExtraPrefixForGrowthStages + "_" + "First_Seed___Phys__Mat_",
    magicExtraPrefixForGrowthStages + "_" + "Emergence____Phys__Mat_",
  ...
  private String[] findGrowthStageNames(String xFileContents) throws Exception {
    ...
    switch (cropTwoLetterCode) {
      case "MZ":
        return maizeCleanedGrowthStages;
      case "BN":
        return beansCleanedGrowthStages;
      case "PN":
        return groundnutsCleanedGrowthStages;
      case "PT":
        return potatoesCleanedGrowthStages;
      case "RI":
        return riceCleanedGrowthStages;
      case "SG":
        return sorghumCleanedGrowthStages;
      case "SB":
        return soybeansCleanedGrowthStages;
      case "WH":
        return wheatCleanedGrowthStages;
      case "CH":
        return chickpeasCleanedGrowthStages;
      default:
        System.out.println("unknown crop code: [" + cropTwoLetterCode + "]");
        throw new Exception();
    }
  }
  ...
  public void readInitFile() throws IOException, Exception {
    ...
    if (cropFertilizerSchemeToUse.equalsIgnoreCase(maizeString)) {
    ...
    } else if (cropFertilizerSchemeToUse.equalsIgnoreCase(
        middleHeavyThreeSplitWithFloweringString)) {
      ...
      nitrogenFertilizerScheme = new FSAllAtPlanting();
      System.out.println(
          "CASSAVA fertilizer scheme doubles as meaning ignore emergence and maturity!");
    ...
    if (irrigationSchemeToUse.equalsIgnoreCase(maizeString)) {
      ...
      String csvPairsOfDaysAndDepths =
          irrigationSchemeToUse.replaceFirst(constantFloodRiceStartString, "");

      System.out.println();
      System.out.println(
          "constant flood rice irrigation scheme, found this (hopefully) CSV: ["
              + csvPairsOfDaysAndDepths
              + "]");
      System.out.println();
      ...
      for (int pairIndex = 0; pairIndex < nPairs; pairIndex++) {
        ...
        System.out.println(
            "on day "
                + dayValue
                + ", change the constant flood height to a "
                + depthValue
                + " mm flood");
      ...
      String csvPairsOfDaysAndDepths =
          irrigationSchemeToUse.replaceFirst(definedFloodRiceStartString, "");

      System.out.println();
      System.out.println(
          "defined flood rice irrigation scheme, found this (hopefully) CSV: ["
              + csvPairsOfDaysAndDepths
              + "]");
      System.out.println();
    ...
    // we may not always call for these extra things, but the overhead is so small that the
    // confusion with
    ...
    //      allExtraToRecordIndices = new int[0]; // don't forget to initialize like we already
    // forgot to
    //  } else {
    //      // we think we have stuff to keep track of...
    //      allExtraToRecordIndices = new int[allExtraToRecordNames.length]; // don't forget to
    // initialize like we already forgot to
    //      for (int extraToRecord = 0; extraToRecord < allExtraToRecordNames.length;
    // extraToRecord++) {
    //      // run through the official list to see if there is a match. if not, throw a hissy
    // fit...
    //      System.out.println("    looking for index: string = [" +
    // allExtraToRecordNames[extraToRecord] + "]; extraToRecordIndex = " + extraToRecord);
    //      foundMatchHere = false;
    //      for (int officialExtraIndex = 0; officialExtraIndex < extraNames.length;
    // officialExtraIndex++) {
    //          if
    // (allExtraToRecordNames[extraToRecord].equalsIgnoreCase(extraNames[officialExtraIndex])) {
    ...
    //      if (!foundMatchHere) {
    //          System.out.println("An extra quantity was requested for yearly recording but is
    // unknown [" + allExtraToRecordNames[extraToRecord] + "]");
    //          throw new Exception();
    //      }
    //      } // looking for the indices of the requested extra quantities
    //  } // end if to check on whether we really have something or not....
    //  // if we manage to get here without an exception getting thrown, then we have a good set of
    // requests
  ...
  private void writeProvenance(String filename)
    ...
    //      provenanceOut.print("nHappyPlantRunsForPhenology:\t"        +
    // nHappyPlantRunsForPhenology + "\n");
    provenanceOut.print(
    ...
    provenanceOut.print(
        "clayLoamSandStableCarbonRatesFilename:\t" + clayLoamSandStableCarbonRatesFilename + "\n");

    provenanceOut.print("optionalHarvestInterval:\t" + optionalHarvestInterval + "\n");
    provenanceOut.print(
        "plantingDateInMonthShiftInDays:\t" + plantingDateInMonthShiftInDays + "\n");
    ...
    provenanceOut.print(
        "nYearsOrRandomSeedsPlaceholder =\t\t\t" + nYearsOrRandomSeedsPlaceholder + "\n");
    ...
    provenanceOut.print(
        "soilInitializationPlaceholder =\t\t\t" + soilInitializationPlaceholder + "\n");

    provenanceOut.print(
        "latitudeSpacingOfWeatherFiles =\t\t\t" + latitudeSpacingOfWeatherFiles + "\n");
    provenanceOut.print(
        "longitudeSpacingOfWeatherFiles =\t\t\t" + longitudeSpacingOfWeatherFiles + "\n");
    ...
    String carbonTableAsString =
        FunTricks.readTextFileToString(clayLoamSandStableCarbonRatesFilename);
    provenanceOut.print(
        "clayLoamSandStableCarbonRates table format is tab delimited: depth / clay fraction stable"
            + " / loam fraction stable / sand fraction stable\n");
    provenanceOut.print(
        "--- begin copy of clayLoamSandStableCarbonRates table file ["
            + clayLoamSandStableCarbonRatesFilename
            + "] ---"
            + "\n");
    provenanceOut.print(carbonTableAsString);
    provenanceOut.print(
        "---- end copy of clayLoamSandStableCarbonRates table file ["
            + clayLoamSandStableCarbonRatesFilename
            + "] ----"
            + "\n");
    provenanceOut.print("\n");
    ...
    for (int layerIndex = 0; layerIndex < standardDepths.length; layerIndex++) {
      provenanceOut.print(
          standardDepths[layerIndex]
              + "\t"
              + clayStableFractionAbove[layerIndex]
              + "\t"
              + loamStableFractionAbove[layerIndex]
              + "\t"
              + sandStableFractionAbove[layerIndex]
              + "\n");
    }
  ...
  public static String happyPlantX(String templateXFile) throws Exception {
    ...
    ////////// ok, and we need to set the planting option to "recorded date" for happy plant
    // purposes //////////////
  ...
  private static Integer[] stressStartingIndices(String[] overviewFileArray) {
  ...
  private static DescriptiveStatisticsUtility[][] extractStressIndicesWpsWgNpsNg(
    ...
    while (keepLookingForRuns && startLineIndex < overviewFileArray.length) {
      ...
      //      System.out.println("top of while: [" + startLineIndex + "] = [" +
      // overviewFileArray[startLineIndex] + "]");
      ...
      for (int lineIndex = labelIsInThisIndex + 1;
        ...
        //          && !overviewFileArray[lineIndex].startsWith(magicGiantBar)
          ...
          // System.out.println("overviewFileArray[" + lineIndex + "] = [" +
        ...
        //          || overviewFileArray[lineIndex].startsWith(magicGiantBar)
          ...
          //          System.out.println("      ____resetting because we think we found all the
          // growth stages_____");
          foundFirstGiantBar = false;
          startLineIndex = lineIndex + 1;
          growthStageWithinBlockLineIndex = 0;

          //          System.out.println("      ====about to break: startLineIndex=" +
          // startLineIndex + " ====");
  ...
  private String[] dynamicallyBuildExtraNamesAndFirstIndexToKeepForExtras(String firstExtraName)
      throws Exception {
  ...
  private Object[] findSummarySpacing() throws Exception {
    ...
    // we only need to read the first few lines, specifically, we want to get to the real header
    // line:
    ...
    try {
      ...
      if (errorAsFileObject.exists()) {
        System.out.println(
            "count spacing: file not found... [" + errorAsFileObject + "] exists...");
        throw fnfe;
      }
      System.out.println("count spacing: file not found...  (no error file)");
    }

    // ok, so we want to look at the header line and go through and find the beginnings of all the
    // key words...
    ...
    try {
      splitUpNames =
          FunTricks.readLineToArrayOnArbitrarySpaceOrTab(
              candidateSummaryContents[lineToGetNamesFrom]);
      ...
      for (int dumpIndex = 0; dumpIndex < candidateSummaryContents.length; dumpIndex++) {
        System.out.println(
            "candidateSummaryContents["
                + dumpIndex
                + "] = ["
                + candidateSummaryContents[dumpIndex]
                + "]");
      }
    }
    ...
    for (int nameIndex = 0; nameIndex < splitUpNames.length; nameIndex++) {
      endingIndicesForSplitUpNames[nameIndex] =
          candidateSummaryContents[magicDSSATSummaryLineIndexToRead - 1].indexOf(
                  splitUpNames[nameIndex], previousEndIndex)
              + splitUpNames[nameIndex].length();
    }
    return new Object[] {splitUpNames, endingIndicesForSplitUpNames};
  }

  private static double pullValueUsingName(
      String summaryLine,
      String columnName,
      String[] splitUpNames,
      int[] endingIndicesForSplitUpNames) {
    ...
    // if we want anything besides the first one, we need to look up what the ending index of the
    // previous one is
    if (nameIndex > 0) {
  ...
  private static String pullStringValueUsingName(
      String summaryLine,
      String columnName,
      String[] splitUpNames,
      int[] endingIndicesForSplitUpNames) {
  ...
  private int[] grabHappyResultsByName(
    ...
    for (int fakeYearIndex = 0; fakeYearIndex < nYears; fakeYearIndex++) {
      ...
      if (candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead] == null) {
        // i am sick of seeing all manner of these error messages, so we'll write it down the first
        // time
        if (!somethingWasNullInYearlyPreviously) {
          // this is taking up way too much space and i never check up on it
          System.out.println(
              "Happy: funny business; skip and hope for the best; this is only the first, skipping"
  ...
  private void grabRealResultsByName(
    ...
    int plantingDate = -1,
        startingDate = -2,
        daysSincePlantingForEvent = -3,
        daysSincePlantingForMaturity = -9;
    ...
    for (int fakeYearIndex = 0; fakeYearIndex < nYears; fakeYearIndex++) {
        ...
        if (false) {
          ...
          System.out.println(
              "   E: fakeYearIndex = ["
                  ...
                  + "]");
          System.out.println(
              "   E: a null pointer exception came up when trying to read the REAL summary: guess"
                  + " is: ["
                  + candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                  + "]");
          System.out.println(
              "   E: skipping the rest of these errors (lineindex = "
                  + lineIndex
                  + " fakeYearIndex = "
                  + fakeYearIndex
                  + ")...");
      ...
      try {
        ...
        if (plantingDate > 0) {
          daysSincePlantingForEvent =
              DSSATHelperMethods.yyyyDDDdifferencerIgnoreLeap(startingDate, plantingDate);
        ...
        if (maturityToUse > 0 && plantingDate > 0) {
          ...
          if (daysSincePlantingForEvent > 0) {
            ...
            // System.out.println("grab real: pd = " + plantingDate + " ; mat = " + maturityToUse +
            // " ; diff = " + daysSincePlantingForEvent);
        ...
        // now, since we are keeping everything (this is the true yearly results, after all), we
        // need
        // to keep even the -99's as crop failures. perhaps, we should encode them as a very very
        // small yield rather
        // than zero, just so we can see them, but so they don't move the averages too far? but then
        // that gives us the
        ...
        if (yieldToUse < 0) {
          // System.out.println("    REAL: [" + yieldToUse + "], -> " +
          // magicValueToUseWithNegativeYields + "; fakeYearIndex=" + fakeYearIndex);
          yieldToUse = magicValueToUseWithNegativeYields;
        }

        // also, i want to see if the maturity length is like way crazy. if so, again: write down
        // something negative
        if (daysSincePlantingForMaturity <= 0
            || daysSincePlantingForMaturity > happyMaturityThresholdToDoRealRuns) {
        ...
        // hopefully this spacey thing will fix the stars problem; but it just creates new ones when
        // values are
        ...
        // i am trying to simplify the handling of problems. it may not be quite as robust, but i am
        // hoping....
        // basically, the "fundamentals" have one set of try/catch-es and the "extras" have
        // another...
        //      System.out.println("extraNames.length = " + extraNames.length + ";
        // allExtraToRecordNames.length = " + allExtraToRecordNames.length + "
        // extractedValues.length=" + extractedValues.length);

        for (int extraIndex = 0; extraIndex < extraNames.length; extraIndex++) {

          try {
            // why don't we skip over nulls here instead of relying on the try/catch?
            extractedValues[extraIndex] =
                pullValueUsingName(
                    candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead],
                    extraNames[extraIndex],
                    splitUpNamesHere,
                    endingIndicesForSplitUpNamesHere);

            // the line we are trying to parse...
            //              System.out.println(candidateSummaryContents[fakeYearIndex +
            // magicDSSATSummaryLineIndexToRead]);
            //              System.out.println("SSS: fYI=" + fakeYearIndex + " name[" + extraIndex +
            // "]=[" + extraNames[extraIndex] + "] value found=" + extractedValues[extraIndex]);
            ...
            // only report the error if it isn't a null because null can mean we didn't get the
            // whole file finished before having to bail out
            nfeExtras.printStackTrace();
            System.out.println(
                "REAL: had trouble reading #"
                    + extraIndex
                    + " "
                    + extraNames[extraIndex]
                    + ": ["
          ...
          // QQQ          System.out.println(candidateSummaryContents[fakeYearIndex +
          // magicDSSATSummaryLineIndexToRead]);
          // QQQ          System.out.println("PPP: extractedValues[" + extraIndex +"]" +
          // extractedValues[extraIndex] + "for " + allExtraToRecordNames[extraIndex]);
        ...
        System.out.println(
            "REAL: had trouble reading one of the following (data line "
                + lineIndex
                + "; fakeYear "
                + fakeYearIndex
                + " planting window index "
        ...
        //              throw new Exception();
        System.out.println(
            "     -> something bad happened with the essentials, skipping this line (index = "
                + lineIndex
                + ")... <-");
      } catch (NullPointerException npe) {
        npe.printStackTrace();
        // newly added on 19mar12 to try to deal with sorghum's funny business where sometimes it
        // seems like
        ...
        System.out.println(
            "   fakeYearIndex = ["
                ...
                + "]");
        System.out.println(
            "   a null pointer exception came up when trying to read the REAL summary: guess is: ["
                + candidateSummaryContents[fakeYearIndex + magicDSSATSummaryLineIndexToRead]
                + "]");
      ...
      for (int wantThisIndex = 0; wantThisIndex < allExtraToRecordIndices.length; wantThisIndex++) {
        ...
        //      System.out.println("       --- wantThisIndex=" + wantThisIndex + " ;
        // allExtraToRecordIndices[wantThisIndex]=" + allExtraToRecordIndices[wantThisIndex] + ";
        // everythingIsValid[<-]" + everythingIsValid[allExtraToRecordIndices[wantThisIndex]]);

        if (everythingIsValid[allExtraToRecordIndices[wantThisIndex]]) {
          //          System.out.println("RECORDING AN EXTRA!!!! --> " +
          // extractedValues[allExtraToRecordIndices[wantThisIndex]]);
          ...
          extraYearly[wantThisIndex][fakeYearIndex].useDoubleValue(
              extractedValues[allExtraToRecordIndices[wantThisIndex]]);
        } else {
          // it does not appear to be valid
          //          System.out.println("recording AN UNAVAILABLE!!!! --> " +
          // extractedValues[allExtraToRecordIndices[wantThisIndex]]);
  ...
  public void doSimulationsOnExistingWeather() throws Exception {
    ...
    //      int[] nonClimateInfo = null; // {(int)soilType, (int)elevation, (int)plantingMonth,
    // firstPlantingDay};
    ...
    double totalInitialNitrogenKgPerHa = -5,
        rootWeight = -6,
        surfaceResidueWeight = -7,
        latitude,
        longitude,
        latitudeToUse,
        longitudeToUse,
    ...
    String[] invariantsReplaced =
        this.buildUniqueInvariantsXTemplates(
            templateXFileContents,
    ...
    for (int lineIndex = 0; lineIndex < nLinesInDataFile; lineIndex++) {
      ...
      soilTypeString =
          magicSoilPrefix
              + FunTricks.padStringWithLeadingZeros(
                  Integer.toString(soilType), totalLengthForSoilType - magicSoilPrefix.length());
      ...
      // ok, so what i need to do is find the nearest of the pre-existing weather data files. but,
      // the
      // latitude and longitude might not be exactly on. so, i need to find the nearest one.

      // how will we do this? well, we will need to know the latitudeSpacingOfWeatherFiles and
      // longitudeSpacingOfWeatherFiles
      // then we need to figure out which "box" it falls into and then what the middle of that box
      // is...
      ...
      latitudeToUse =
          positiveNegativeMultiplier * latitudeSpacingOfWeatherFiles / 2
              + boxIndex * latitudeSpacingOfWeatherFiles;
      ...
      longitudeToUse =
          positiveNegativeMultiplier * longitudeSpacingOfWeatherFiles / 2
              + boxIndex * longitudeSpacingOfWeatherFiles;
      ...
      if (!originalWeatherFile.exists()) {
        ...
        // Beware the MAGIC NUMBER!!! the warning that something funny is happening... that is, the
        // weather file went missing
        ...
        for (int plantingWindowIndex = 0;
          ...
          startingDateAsInt =
              DSSATHelperMethods.yyyyDDDaddDaysIgnoreLeap(
                  fakePlantingYear * 1000 + startingDayToPlantForThisWindow,
                  plantingDateInMonthShiftInDays);
          startingDayToPlantCode = DSSATHelperMethods.padWithZeros(startingDateAsInt, 5);

          // pick the ending day/etc for this window
          endingDayToPlantForThisWindow =
              DSSATHelperMethods.yyyyDDDaddDaysIgnoreLeap(
                  startingDateAsInt, plantingWindowLengthDays);
          ...
          initializationDayCode =
              DSSATHelperMethods.padWithZeros(
                  DSSATHelperMethods.yyyyDDDaddDaysIgnoreLeap(startingDateAsInt, -spinUpTimeDays),
                  5);

          harvestDayCode =
              DSSATHelperMethods.padWithZeros(
                  DSSATHelperMethods.yyyyDDDaddDaysIgnoreLeap(
                      startingDateAsInt, optionalHarvestInterval),
                  5);
          ...
          XHappyStuffToWrite =
              XHappyInvariantsReplaced.replaceAll(
                  soilInitializationPlaceholder, dummyInitializationBlock);
          ...
          XHappyStuffToWrite =
              XHappyStuffToWrite.replaceAll(harvestingDatePlaceholder, harvestDayCode);
          // System.out.println("harvestdaycode");
          // System.out.println("XHappyStuffToWrite");
          // System.out.println(XHappyStuffToWrite);

          XHappyStuffToWrite =
              XHappyStuffToWrite.replaceAll(
          ...
          // ok, i've had trouble with unstable ORYZA-in-DSSAT not finishing when run under here,
          // but it does
          // just fine manually. go figure. so first, i'm going to run it with a timer. if i get
          // that working,
          ...
          // happyRunnerThing.setup(dssatExecutionCommand, pathToDSSATDirectoryAsFile,
          ...
          // i am going to add an arbitrary tiny wait in here in case the filesystem is bogged
          // down...
          ...
          //     System.out.println("     +++ happy timed out #" + rerunIndex + "(max = " +
          // (rerunAttemptsMax - 1) + ") line=" + lineIndex + " plantingWindow=" +
          // plantingWindowIndex
          //         + " lines in Summary.OUT = " +
          // FunTricks.nLinesInTextFile(magicDSSATSummaryToRead) + " goal = " + (nFakeYears +
          // magicDSSATSummaryLineIndexToRead) );
          //     // check one more time to see if the length goal has been met because sometimes it
          // is by the time we get to here...
          //     if (FunTricks.nLinesInTextFile(magicDSSATSummaryToRead) == (nFakeYears +
          // magicDSSATSummaryLineIndexToRead)) {
          //         System.out.println("  apparently, the goal was met by the time we got here,
          // moving on...");
          //         break;
          //     }
          //     } else {
          ...
          // (rerunAttemptsMax - 1) + ") line=" + lineIndex + " plantingWindow=" +
          // plantingWindowIndex
          //         + " lines in Summary.OUT = " + 0 + " goal = " + (nFakeYears +
          // magicDSSATSummaryLineIndexToRead) );
          //     }
          // } else {
          ...
          //              phenologyInDays = this.grabOLDHappyResults(1); //
          // nHappyPlantRunsForPhenology);
          //              } else {
          //              phenologyInDays = this.grabNewHappyResults(1); //
          // nHappyPlantRunsForPhenology);
          //              }
          ...
          //  but then the problem becomes what to do if we don't get a maturity length or
          // something...
          ...
          // on the very first line and very first planting window, we'll need to be clever and set
          // everything up. after that, we'll just re-use...
          ...
          if (!namesAndStuffHaveBeenSetUp) {
            ...
            extraNames =
                dynamicallyBuildExtraNamesAndFirstIndexToKeepForExtras(
                    magicFirstSummaryColumnNameToKeepForExtras);

            // switching this to consider life as floats rather than as integers...
            //          extraSummaryAccumulators = new
            // DescriptiveStatisticsUtility[extraNames.length];
            //          for (int extraIndex = 0; extraIndex < extraNames.length; extraIndex++) {
            extraSummaryAccumulators =
                new DescriptiveStatisticsUtility[allExtraToRecordNames.length];
            for (int extraIndex = 0; extraIndex < allExtraToRecordNames.length; extraIndex++) {
              extraSummaryAccumulators[extraIndex] = new DescriptiveStatisticsUtility(true);
              ...
              System.out.println(
                  "["
                      + extraIndex
                      + "] sUNH=["
                      + splitUpNamesHere[extraIndex]
                      + "] aETRN=["
                      + allExtraToRecordNames[extraIndex]);
            }
            ...
            //          // first, let's split it up on the comma so we can get the different
            // names...
            //          allExtraToRecordNames = allExtraToRecordCSV.split(",");

            // now, let's look through the list of official extra names and figure out the
            // appropriate
            // index. brute force, because we just don't care...
            // Beware the MAGIC ASSUMPTION!!! there is no idiot checking for uniqueness. somebody
            // could
            // ask for IRCM five times and we'll give it to them...
            boolean foundMatchHere = false;
            ...
            // to zero length and move on...
            //          if (allExtraToRecordNames[0].equalsIgnoreCase("") ||
            // allExtraToRecordNames.length == 0) {
            if (allExtraToRecordNames.length == 0) {
              // we find ourselves with nothing worthwhile
              allExtraToRecordIndices =
                  new int[0]; // don't forget to initialize like we already forgot to
            } else {
              // we think we have stuff to keep track of...
              allExtraToRecordIndices =
                  new int
                      [allExtraToRecordNames
                          .length]; // don't forget to initialize like we already forgot to
              for (int extraToRecord = 0;
                  extraToRecord < allExtraToRecordNames.length;
                  extraToRecord++) {
                // run through the official list to see if there is a match. if not, throw a hissy
                // fit...
                // QQQ              System.out.println("    looking for index: string = [" +
                // allExtraToRecordNames[extraToRecord] + "]; extraToRecordIndex = " +
                // extraToRecord);
                foundMatchHere = false;
                for (int officialExtraIndex = 0;
                    officialExtraIndex < extraNames.length;
                    officialExtraIndex++) {
                  //              for (int officialExtraIndex = 0; officialExtraIndex <
                  // allExtraToRecordNames.length; officialExtraIndex++) {
                  // QQQ                  System.out.println("testing index " + officialExtraIndex +
                  // " [" + allExtraToRecordNames[extraToRecord] + "] ?= [" +
                  // extraNames[officialExtraIndex] + "]");
                  if (allExtraToRecordNames[extraToRecord].equalsIgnoreCase(
                      extraNames[officialExtraIndex])) {
                    // we have a match, so write the number down and break out
                    allExtraToRecordIndices[extraToRecord] = officialExtraIndex;
                    foundMatchHere = true;
                ...
                if (!foundMatchHere) {
                  System.out.println(
                      "An extra quantity was requested for yearly recording but is unknown ["
                          + allExtraToRecordNames[extraToRecord]
                          + "]");
            ...
            // if we manage to get here without an exception getting thrown, then we have a good set
            // of requests
            // so, let's set up the accumulators...
            extraYearly =
                new DescriptiveStatisticsUtility[allExtraToRecordIndices.length][nFakeYears];
            for (int extraIndex = 0; extraIndex < allExtraToRecordIndices.length; extraIndex++) {
              for (int yearIndex = 0; yearIndex < nFakeYears; yearIndex++) {
                extraYearly[extraIndex][yearIndex] = new DescriptiveStatisticsUtility(true);
              }
          ...
          if (weShouldBotherParsingOverviewDotOut) {
            overviewTestFile
                .delete(); // clear it out after reading it? to make sure they don't get confused...
          }
          ...
          //                  irrigationScheme.initialize(Integer.parseInt(startingDayToPlantCode),
          // nRandomSeedsToUse);
          ...
          // for (int rerunIndex = 0; rerunIndex < rerunAttemptsMax ; rerunIndex++) {
          // realRunnerThing.setup(dssatExecutionCommand, pathToDSSATDirectoryAsFile,
          // (int)Math.ceil(maxRunTime * Math.pow(bumpUpMultiplier, rerunIndex)),
          // testIntervalToUse);
          // realRunnerThing.run();
          ...
          if (weShouldBotherParsingOverviewDotOut) {
            ...
            // the correct filename should have been ascertained; we should not get here if the file
            // does not exist
            overviewAsArray = FunTricks.readTextFileToArray(overviewToTry);

            // ok, now let's try to accumulate...
            // but i need to modify to match up growth stage names rather than just looking for
            // them...
            //          System.out.println("about to extract stresses: lineIndex = " + lineIndex +
            // "; planting window = " + plantingWindowIndex);
            stressAccumulators =
      ...
      //      System.out.println("DEBUG2: n=" + realYieldsEntirePixel.getN() + " yieldmean=" +
      // realYieldsEntirePixel.getMean());
      if (realYieldsEntirePixel.getN() == 0
          || realYieldsEntirePixel.getMean() == yieldForMissingWeatherFile) {
        ...
        statisticsOutLine =
            (-1)
                ...
                + FunTricks.onlySomeDecimalPlaces(
                    happyYieldsEntirePixel.getMean(), nDecimalsInOutput)
                + delimiter
                + FunTricks.onlySomeDecimalPlaces(
                    happyMaturityEntirePixel.getMean(), nDecimalsInOutput);
        ...
        // "time_to_planting" + delimiter + "n_no_planting" + delimiter + "n_real_exactly_zero" +
        // delimiter + "n_contributing_real" + delimiter + "n_contributing_happy";
        statisticsOutLine +=
            delimiter
                + (-this.spinUpTimeDays)
                + delimiter
                ...
                + (0)
                + delimiter
                + realYieldsEntirePixel.getN()
                + delimiter
                + happyYieldsEntirePixel.getN();

        // and the stress indices...
        if (weShouldBotherParsingOverviewDotOut) {
          for (int growthStageIndex = 0;
              growthStageIndex < growthStageNames.length;
              growthStageIndex++) {
            for (int stressIndexIndex = 0; stressIndexIndex < 4; stressIndexIndex++) {
              //              statisticsOutLine += delimiter +
              // stressAccumulators[growthStageIndex][stressIndexIndex].getMean();
        ...
        if (allFlag) {
          // yes, we could probably just put zeros in here or something, but they should come out as
          // zeros...
          for (int fakeYearIndex = 0; fakeYearIndex < this.nFakeYears; fakeYearIndex++) {
            statisticsOutLine +=
                delimiter
                    + FunTricks.onlySomeDecimalPlaces(
                        realYearlyYields[fakeYearIndex].getMean(), nDecimalsInOutput);
          }
          ...
          if (keepHappyYields) {
            for (int fakeYearIndex = 0; fakeYearIndex < this.nFakeYears; fakeYearIndex++) {
              statisticsOutLine +=
                  delimiter
                      + FunTricks.onlySomeDecimalPlaces(
                          happyYearlyYields[fakeYearIndex].getMean(), nDecimalsInOutput);
            } // fakeYearIndex
          } // keepHappyYields
          ...
          if (keepRealDaysToEmergence) {
            for (int fakeYearIndex = 0; fakeYearIndex < this.nFakeYears; fakeYearIndex++) {
              statisticsOutLine +=
                  delimiter
                      + FunTricks.onlySomeDecimalPlaces(
                          realYearlyEmergence[fakeYearIndex].getMean(), nDecimalsInOutput);
            } // fakeYearIndex
          } // keepRealDaysToEmergence
          ...
          if (keepRealDaysToAnthesis) {
            for (int fakeYearIndex = 0; fakeYearIndex < this.nFakeYears; fakeYearIndex++) {
              statisticsOutLine +=
                  delimiter
                      + FunTricks.onlySomeDecimalPlaces(
                          realYearlyAnthesis[fakeYearIndex].getMean(), nDecimalsInOutput);
            } // fakeYearIndex
          } // keepRealDaysToAnthesis
          ...
          if (keepRealDaysToMaturity) {
            for (int fakeYearIndex = 0; fakeYearIndex < this.nFakeYears; fakeYearIndex++) {
              statisticsOutLine +=
                  delimiter
                      + FunTricks.onlySomeDecimalPlaces(
                          realYearlyMaturity[fakeYearIndex].getMean(), nDecimalsInOutput);
            } // fakeYearIndex
          } // keepRealDaysToMaturity

          for (int wantThisIndex = 0;
              wantThisIndex < allExtraToRecordNames.length;
              wantThisIndex++) {
            for (int fakeYearIndex = 0; fakeYearIndex < this.nFakeYears; fakeYearIndex++) {
              statisticsOutLine +=
                  delimiter + FunTricks.onlySomeDecimalPlaces(-8, nDecimalsInOutput);
            } // fakeYearIndex
          } // wantThisIndex
        } // allFlag
        ...
        statisticsOutLine =
            realBadThingsCountsEntirePixel.getTotalLong()
                + delimiter
                ...
                + delimiter
                + FunTricks.onlySomeDecimalPlaces(
                    realYieldsEntirePixel.getMean(), nDecimalsInOutput)
                + delimiter
                + FunTricks.onlySomeDecimalPlaces(realYieldsEntirePixel.getStd(), nDecimalsInOutput)
                + delimiter
                + FunTricks.onlySomeDecimalPlaces(
                    realEmergenceEntirePixel.getMean(), nDecimalsInOutput)
                + delimiter
                + FunTricks.onlySomeDecimalPlaces(
                    realEmergenceEntirePixel.getStd(), nDecimalsInOutput)
                + delimiter
                + FunTricks.onlySomeDecimalPlaces(
                    realAnthesisEntirePixel.getMean(), nDecimalsInOutput)
                + delimiter
                + FunTricks.onlySomeDecimalPlaces(
                    realAnthesisEntirePixel.getStd(), nDecimalsInOutput)
                + delimiter
                + FunTricks.onlySomeDecimalPlaces(
                    realMaturityEntirePixel.getMean(), nDecimalsInOutput)
                + delimiter
                + FunTricks.onlySomeDecimalPlaces(
                    realMaturityEntirePixel.getStd(), nDecimalsInOutput)
                + delimiter
                + FunTricks.onlySomeDecimalPlaces(
                    happyYieldsEntirePixel.getMean(), nDecimalsInOutput)
                + delimiter
        ...
        if (!namesAndStuffHaveBeenSetUp) {
          ...
          for (int extraIndex = 0; extraIndex < allExtraToRecordNames.length; extraIndex++) {
            statisticsOutLine +=
                delimiter
                    + FunTricks.onlySomeDecimalPlaces(
                        extraSummaryAccumulators[extraIndex].getMean(), nDecimalsInOutput);
          }
        }
        ...
        statisticsOutLine +=
            delimiter
                + FunTricks.onlySomeDecimalPlaces(
                    (realTimeToPlantingEntirePixel.getMean() - this.spinUpTimeDays),
                    nDecimalsInOutput)
                + delimiter
                + realNoPlantingEntirePixel.getN()
                + delimiter
                + realYieldsExactlyZeroEntirePixel.getN()
                + delimiter
                + realYieldsEntirePixel.getN()
                + delimiter
                + happyYieldsEntirePixel.getN();
          ...
          for (int wantThisIndex = 0;
            ...
            for (int fakeYearIndex = 0; fakeYearIndex < this.nFakeYears; fakeYearIndex++) {
              statisticsOutLine +=
                  delimiter
                      + FunTricks.onlySomeDecimalPlaces(
                          extraYearly[wantThisIndex][fakeYearIndex].getMean(), nDecimalsInOutput);
            } // fakeYearIndex
          } // wantThisIndex
        } // end if(allFlag)
    ...
    if (nLinesInDataFile == 0) {
      ...
      System.out.println("");
      // We have no pixels in the raster we're processing. We should skip the rest of this and
      // return from the function. Any missing rasters as a result should be ignored.
      return;
    }
    ...
    // Beware the MAGIC NUMBER!!!
    int nCols = -5;
    //      nCols = 13 + extraNames.length + 2 + 2; // min / max/ mean / std / bad / happy mean /
    // happy std / real anthesis mean / real anthesis std / real maturity mean / real maturity std /
    // happy maturity mean / happy maturity std
    //  nCols = 13 + extraNames.length + 2 + 1 + 2; // min / max/ mean / std / bad / happy mean /
    // happy std / real anthesis mean / real anthesis std / real maturity mean / real maturity std /
    // happy maturity mean / happy maturity std
    nCols =
        13
            + allExtraToRecordNames.length
            + 2
            + 1
