    ...
    for (int pixelIndex = 0; pixelIndex < nValidPixels; pixelIndex++) {
      ...
      for (int lineIndex = magicFirstLineToWorkOn;
        ...
        linearMixingCoefficient =
            (nYearsForTransition
                    - ((double) yearToAssumeAfterSpinup
                        + (nNewYearsSinceBeginningOfWeatherFile - nJanuariesForSpinup - 1)
                        - baselineClimateYear))
                / nYearsForTransition;
        ...
        if (shortwaveToUse > maximumAcceptableShortwave) {
          ...
          // added in, but after my big runs. negative sunshine should only happen in really bad
          // zero sunshine places
        ...
        // and double check that max is bigger than min. otherwise, swap them in the ugly SIMMETEO
        // manner...
