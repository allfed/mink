...
public class IrriSRiceSomeDrySpellsB implements IrrigationScheme {

	private boolean	readyToCreateBlock	= false;

	public IrriSRiceSomeDrySpellsB() {
...
//	    int[][] daysColOneDepthColTwoFloodDefinitions = (int[][])inputObjectArray[0];
		...
		if (day <= previousDay) {
		    System.out.println("flexible rice IR scheme day out of order: event index = " + eventIndex + "; day = " + day + "; previousDay = " + previousDay);
		    throw new Exception();
		}
		
		if (day < oneAfterShift) {
		    System.out.println("flexible rice IR scheme day too early: event index = " + eventIndex + "; day = " + day + "; must be on or after  " + oneAfterShift);
		    throw new Exception();
		}

		if (day >= endOfSeasonShift) {
		    System.out.println("flexible rice IR scheme day too late: event index = " + eventIndex + "; day = " + day + "; must be before  " + endOfSeasonShift);
		    throw new Exception();
		}

		if (depth < 0) {
		    System.out.println("flexible rice IR scheme depth too low: event index = " + eventIndex + "; depth = " + depth + "; must be non-negative");
		    throw new Exception();
		}
...
//	private double fixedFloodDepth = 1.0; // mm; this is to try out different depths to see if we can get different water usages and yields.
...
//		int sevenAfter  = -3;
		...
		double percolationRate = 0.5; // IR008 mm/day; originally, i used 2mm, but i think it should be lower, so stealing 0.5 from a standard X file
		double plowpanPuddlingDepth = 15; // IR010; dunno if this is actually read or not
		double initialBundHeight = 100; // 20.0; // IR009; mm
		double initialFloodDepth = 0.0; // 20.0; // IR003; mm
		double finalBundHeight = 150.0; // IR009; mm
		for (int yearIndex = 0; yearIndex < nYears; yearIndex++) {

			yearlyDayOffset = yearIndex * 365;

			dayAfter = DSSATHelperMethods.yyyyDDDaddDaysIgnoreLeap(plantingDate, oneAfterShift + yearlyDayOffset);
//			threeBefore = DSSATHelperMethods.yyyyDDDaddDaysIgnoreLeap(plantingDate, threeBeforeShift + yearlyDayOffset);
//			threeAfter  = DSSATHelperMethods.yyyyDDDaddDaysIgnoreLeap(plantingDate, threeAfterShift + yearlyDayOffset);
//			sevenAfter  = DSSATHelperMethods.yyyyDDDaddDaysIgnoreLeap(plantingDate, sevenAfterShift + yearlyDayOffset);
			endOfSeason = DSSATHelperMethods.yyyyDDDaddDaysIgnoreLeap(plantingDate, endOfSeasonShift + yearlyDayOffset);

			// the "unflooded" setup; similar, but not the same as what i am using for real
			irriBlock += " 1 " + DSSATHelperMethods.padWithZeros(plantingDate,5) + " IR009 " + DSSATHelperMethods.padWithZeros(initialBundHeight, 5) + "\n";
			irriBlock += " 1 " + DSSATHelperMethods.padWithZeros(plantingDate,5) + " IR003 " + DSSATHelperMethods.padWithZeros(initialFloodDepth, 5) + "\n";
			irriBlock += " 1 " + DSSATHelperMethods.padWithZeros(plantingDate,5) + " IR010 " + DSSATHelperMethods.padWithZeros(plowpanPuddlingDepth, 5) + "\n";
			irriBlock += " 1 " + DSSATHelperMethods.padWithZeros(plantingDate,5) + " IR008 " + DSSATHelperMethods.padWithZeros(percolationRate, 5) + "\n";
			irriBlock += " 1 " + DSSATHelperMethods.padWithZeros(dayAfter,5)     + " IR009 " + DSSATHelperMethods.padWithZeros(finalBundHeight, 5) + "\n";

			//			irriBlock += " 1 " + DSSATHelperMethods.padWithZeros(dayAfter,5)     + " IR009   150" + "\n";

			// the logic here is that we have enough water to soak the paddy for a few days around planting
...
//			irriBlock += " 1 " + DSSATHelperMethods.padWithZeros(threeAfter,5)  + " IR009 100.0" + "\n";
			...
			for (int eventIndex = 0; eventIndex < daysColOneDepthColTwoFloodArray.length; eventIndex++) {
			    ...
			    irriBlock += " 1 " + DSSATHelperMethods.padWithZeros(eventDay,5)  + " IR003 " + floodDepthString  + "\n";

//			    System.out.println("[" + eventIndex + "][0] = " +  daysColOneDepthColTwoFloodArray[eventIndex][0]);
//			    System.out.println("[" + eventIndex + "][1] = " +  daysColOneDepthColTwoFloodArray[eventIndex][1]);
			}

//			irriBlock += " 1 " + DSSATHelperMethods.padWithZeros(sevenAfter,5)  + " IR011 " + floodDepthString  + "\n";
