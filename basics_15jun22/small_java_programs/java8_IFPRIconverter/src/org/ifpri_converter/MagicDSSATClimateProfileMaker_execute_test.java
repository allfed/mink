...
public class MagicDSSATClimateProfileMaker_execute_test {
  ...
  public static void runCommand(String cmd) throws Exception {
    ...
    //		Process proc = Runtime.getRuntime().exec(cmd); // Execute a command and get its process
    // handle
    Runtime.getRuntime().exec(cmd); // Execute a command and get its process handle
    Runtime.getRuntime().exec("pwd");

    //		InputStream istr = proc.getInputStream(); // Get the handle for the processes [InputStream]
    ...
    //
    //		try { proc.waitFor(); } // Wait for process to terminate and catch any Exceptions.
  ...
  /*
  	...
  	return (String[])list.toArray(new String[0]); // Convert the list to a string and return
        ...
        public static void funny(String args[]) throws IOException // Actual execution starts here
        {
            try
            {
  ...
  */
  ...
  public static void main(String commandLineOptions[]) throws Exception {
    ...
    if (commandLineOptions.length == 0) {
      System.out.println(
          "Usage: org.ifpri_converter.MagicDSSATClimateProfileMaker input_base_name"
              + " output_base_name SW_multiplier delimiter\n"
              + "\n"
              + "This program will take a magically produced text table of climate/elevation/soil"
              + " and create DSSAT style *.CLI files.\n"
              + "Everything is brute force and magical. This will create a CLI file for each line"
              + " in the table. They will be systematically\n"
              + "named, but not directly useful in DSSAT. First, you will have to rename/copy/move"
              + " it to a four letter name.\n"
              + "\n"
    ...
    */
    ...
    System.out.println(
        "-- all done at "
            + new Date()
            + " = "
            + (float) ((endTime - startTime) / 1000.0d)
            + " s "
            + (float) ((endTime - startTime) / (60 * 1000.0d))
            + " m");
  } // main
}
