...
public class MagicSoilCodeSplitterWithIrrigation {
  ...
  public static void main(String commandLineOptions[]) throws Exception {
    ...
    if (commandLineOptions.length == 0) {
      System.out.println(
          "Usage: org.ifpri_converter.MagicSoilCodeSplitterWithIrrigation input_file soil_column"
              + " zero_columns_csv soil_code_table_file output_directory_with_slash"
              + " output_base_name delimiter csv_columns_to_keep\n"
              + "\n"
              + "The point of it is to take the output of Jawoo's runs and split them up into"
              + " separate files for each \n"
              + "soil code. \n"
              + "\n"
              + "We also want to give a list of columns which, when any is non-zero, should go into"
              + " a separate file from the zeros.\n"
              + "\n"
              + " The column with the soil code number needs to be specified. We also specify the"
              + " \n"
              + "columns to keep using a CSV list. The columns will be outputted in the order that"
              + " they appear \n"
              + "in the list. Counting starts at zero.\n"
              + "The soil code table file should have two comma separated columns per line. The"
              + " first column is the \n"
              + "soil code number while the second is the three letter code name. The codes are"
              + " assumed to be sequential starting at 1.\n"
              + "\n"
    ...
    String[] zeroColumnsCSVSplit =
        zeroColumnsCSV.split(","); // Beware the MAGIC NUMBER!!! comma for CSV
    ...
    for (int colIndex = 0; colIndex < nZeroColumns; colIndex++) {
      ...
      System.out.println(
          "  actual col " + columnsToTestForZeros[colIndex] + " will be tested for zero-ness");
    }

    // build a more easily processed list of columns
    String[] csvColumnsToKeepSplit =
        csvColumnsToKeep.split(","); // Beware the MAGIC NUMBER!!! comma for CSV
    ...
    for (int colIndex = 0; colIndex < nColumnsToKeep; colIndex++) {
      ...
      System.out.println(
          "actual col " + columnsToKeepList[colIndex] + " is being mapped to " + colIndex);
    }
    ...
    String[] soilCodeList =
        new String[nSoilCodes]; // dealing with numbers, not indices, unfortunately
    ...
    for (int soilIndex = 0; soilIndex < nSoilCodes; soilIndex++) {
      outputOutName =
          outputDirectory + soilCodeList[soilIndex] + "_" + outputBaseName + "_nonzeros.txt";
      ...
      outputOutName =
          outputDirectory + soilCodeList[soilIndex] + "_" + outputBaseName + "_zeros.txt";
    ...
    while (lineContents != null) {
      ...
      for (int zeroIndex = 0; zeroIndex < nZeroColumns; zeroIndex++) {
        zeroColumnContents[zeroIndex] =
            Double.parseDouble(lineSplit[columnsToTestForZeros[zeroIndex]]);
    ...
    for (int soilIndex = 0; soilIndex < nSoilCodes; soilIndex++) {
      ...
      System.out.println(
          "zeros: index "
              + soilIndex
              + "/"
              + soilCodeList[soilIndex]
              + " -> "
              + nLinesWrittenZero[soilIndex]
              + " lines written");
      ...
      infoFileToWrite =
          new File(
              outputDirectory + soilCodeList[soilIndex] + "_" + outputBaseName + "_zeros.info.txt");
      ...
      outInfoWriterObject.print(
          (nLinesWrittenZero[soilIndex] * nColumnsToKeep) + "\t = Total Number of Elements\n");
      ...
      infoFileToWrite =
          new File(
              outputDirectory + soilCodeList[soilIndex] + "_" + outputBaseName + "_zeros.cols.txt");
      outInfoStream = new FileWriter(infoFileToWrite);
      outInfoWriterObject = new PrintWriter(outInfoStream);
      ...
      System.out.println(
          "non-zeros: index "
              ...
              + soilCodeList[soilIndex]
              + " -> "
              + nLinesWrittenNonZero[soilIndex]
              + " lines written");
      ...
      outInfoWriterObject.print(
          (nLinesWrittenNonZero[soilIndex] * (nColumnsToKeep + nZeroColumns))
