...
public class MagicHCgridMatcherForSoilProfiles {
  ...
  public static void main(String commandLineOptions[]) throws Exception {
    ...
    if (commandLineOptions.length == 0) {
      System.out.println(
          "Usage: org.ifpri_converter.MagicHCgridMatcherForSoilProfiles attributes_file"
              + " geography_base_name output_file header_line_flag\n"
              + "\n"
              ...
              + " renamed).\n"
              + "\n"
              + "The logic is that we will take a sorted list of id numbers which have other"
              + " attributes associated\n"
              + "with them in a tab delimited format and match these up against a set of tables"
              + " created by doing\n"
              + "g.region rast=proper_id_grid ; r.out.new input=proper_id_grid"
              + " output=place_to_put_them\n"
              + "We will then pull out only the bits of geographic information needed for the pixel"
              + " ids in the table ofattributes and make a new set of text files that can be"
              + " imported via r.in.new.\n"
    ...
    for (long attributeLineIndex = 0; attributeLineIndex < nAttributeLines; attributeLineIndex++) {
      ...
      if (atLineContents.isEmpty()) {
        ...
        while (pixelToFind != pixelInGeography) {
          ...
          //					System.out.println("  " + nDataLinesRead + ": dLC = [" + dataLineContents + "]; gLC
          // = [" + geogLineContents + "]");
          /*
