...
public class MagicCropLifeReader1 {
  ...
  public static void main(String commandLineOptions[]) throws Exception {
    ...
    // my plan is the step through the cellIDNumberFile and then match up with the data file to keep
    // everything in order
    ...
    // ok, so we will have a bunch of cases which will end up as prefixes, e.g.,
    // cultivar_climate_technology_*
    // then we have a bunch of characteristics to aggregate for those (i.e., all the other columns);
    // e.g., *_HWAH or *_NICM
    ...
    DescriptiveStatisticsUtility[] overallAverages =
        new DescriptiveStatisticsUtility[nAggregateQuantities];
    DescriptiveStatisticsUtility[][] yearlyAverages =
        new DescriptiveStatisticsUtility[nAggregateQuantities][PYEARn];
    ...
    MultiFormatMatrix cellID =
        MatrixOperations.read2DMFMfromTextForceFormat(
            cellIDNumberFile + "_data", 1); // Beware the MAGIC NUMBER!!! in memory...

    // grab the cell id geog as a string array
    String[] geogStringArrayWholeLines =
        FunTricks.readTextFileToArray(cellIDNumberFile + "_geog.txt");
    ...
    for (int lineIndex = 0; lineIndex < nDataFileLines; lineIndex++) {
      ...
      if (lineIndex > 0) {
        if (SoilID[lineIndex] != SoilID[0]) {
          System.out.println(
              "SoilID[" + lineIndex + "] != SoilID[0]; " + SoilID[lineIndex] + " != " + SoilID[0]);
          throw new Exception();
    ...
    for (int pixelIndex = 0; pixelIndex < nPixels; pixelIndex++) {
      ...
      //			System.out.println("[[[ we have data starting on line index " +
      // firstLineNumberInDataFileForThisPixelID + " for pixel " + thisPixelID);
      ...
      while (!doneWithThisBlockInDataFile) {
        ...
        // now, sometimes we get a weird early planting problem, so we'll fix it, but note it in the
        // screen dump
        if (computedYearIndex < 0) {
          System.out.print(
              dataLineToPullFrom
                  + " <- line with possible early planting. resetting computedYearIndex from "
                  + computedYearIndex
                  + " to ");
          computedYearIndex = 0;
          ...
          System.out.print(
              dataLineToPullFrom
                  + " <- line with possible LATE thing. resetting computedYearIndex from "
                  + computedYearIndex
                  + " to ");
      ...
      //			System.out.println("nValidPixels (index) = " + nValidPixels + " bailing prior to " +
      // nextLineIndex);
      ...
      if (soilShareCounter.getMinAsDouble() != soilShareCounter.getMaxAsDouble()) {
        System.out.println(
            "soilShareCounter.getMinAsDouble() != soilShareCounter.getMaxAsDouble()"
                ...
                + dataLineToPullFrom
                + " "
                + soilShareCounter.getMinAsDouble()
                + " != "
                + soilShareCounter.getMaxAsDouble());
        throw new Exception();
      }
    ...
    FunTricks.writeInfoFile(
        outputBaseName + "_data", nValidPixels, 6 * (1 + PYEARn) * 1 + 1 + 1, delimiter);

    FunTricks.writeInfoFile(
        outputBaseName + "_geog",
        nValidPixels,
