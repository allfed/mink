...
public class MagicSoilCodeSplitter {
  ...
  public static void main(String commandLineOptions[]) throws Exception {
    ...
    if (commandLineOptions.length == 0) {
      System.out.println(
          "Usage: org.ifpri_converter.MagicSoilCodeSplitter input_file soil_column"
              + " soil_code_table_file output_directory_with_slash output_base_name delimiter"
              + " csv_columns_to_keep\n"
              + "\n"
              + "The point of it is to take the output of Jawoo's runs and split them up into"
              + " separate files for each \n"
              + "soil code. The column with the soil code number needs to be specified. We also"
              + " specify the \n"
    ...
    for (int soilIndex = 0; soilIndex < nSoilCodes; soilIndex++) {
      ...
      infoFileToWrite =
          new File(outputDirectory + soilCodeList[soilIndex] + "_" + outputBaseName + ".info.txt");
      ...
      outInfoWriterObject.print(
          (nLinesWritten[soilIndex] * nColumnsToKeep) + "\t = Total Number of Elements\n");
      ...
      infoFileToWrite =
          new File(outputDirectory + soilCodeList[soilIndex] + "_" + outputBaseName + ".cols.txt");
      outInfoStream = new FileWriter(infoFileToWrite);
      outInfoWriterObject = new PrintWriter(outInfoStream);
