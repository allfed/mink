...
public class MagicCropLifeReader0 {
...
//		soilCodeUtility = new org.R2Useful.StringToUniqueCode();
		for (int pixelIndex = 0; pixelIndex < nPixels; pixelIndex++) {
			...
			for (int repIndex = 0; repIndex < nRepetitions; repIndex++) {
				for (int yearIndex = 0; yearIndex < PYEARn; yearIndex++) {
					dataLineToPullFrom = firstLineNumberInDataFileForThisPixelID + repIndex * PYEARn + yearIndex;
//					String[] aggregateQuantityNames = {"NICM", "IRCM", "PRCM", "GRDAYS", "CWAM", "HWAH"};
					...
					yearlyAverages[0][yearIndex].useLongValue(NICM  [dataLineToPullFrom]);
					yearlyAverages[1][yearIndex].useLongValue(IRCM  [dataLineToPullFrom]);
					yearlyAverages[2][yearIndex].useLongValue(PRCM  [dataLineToPullFrom]);
					yearlyAverages[3][yearIndex].useLongValue(GRDAYS[dataLineToPullFrom]);
					yearlyAverages[4][yearIndex].useLongValue(CWAM  [dataLineToPullFrom]);
					yearlyAverages[5][yearIndex].useLongValue(HWAH  [dataLineToPullFrom]);
				} // yearIndex
			} // repIndex
			...
			outputLine += overallAverages[0].getN() + delimiter;
			...
			outputLine += delimiter + overallAverages[5].getMean() + delimiter;
			...
			// we need to strip the last comma off...
			outData.println(outputLine.substring(1));
		...
		outCols.println(columnsLine.substring(1));
		...
		FunTricks.writeInfoFile(outputBaseName  + "_data.txt",
				nValidPixels, 
				6 * (1 + PYEARn) + 1 + 1, 
				delimiter);

		FunTricks.writeInfoFile(outputBaseName  + "_geog.txt",
				nValidPixels, 
