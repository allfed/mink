...
public class MagicGeogCreatorThornton {
  ...
  private static String[] splitUpLine(String inLine, int[] columnWidth) {
    ...
    for (int colIndex = 0; colIndex < nColumns; colIndex++) {
      ...
      if (endIndex < inLine.length() - 1) {
        outArray[colIndex] =
            inLine.substring(currentIndex, currentIndex + columnWidth[colIndex] + 1).trim();
      } else {
  ...
  public static void main(String commandLineOptions[]) throws Exception {
    ...
    if (commandLineOptions.length == 0) {
      System.out.println(
          "Usage: org.ifpri_converter.MagicConverterThornton filename output_base_filename\n ");
      System.exit(1);
    }
    ...
    // raw columns are: n,lat,long,elev,col,row,tt,amp,sry,txy,tny,rt,srad,tx,tn,rain,rdays (72
    // total)
    ...
    outArcWriterObject.print(
        "xllcorner     " + (minLongitude - cellSize / 2.0 + 0 * cellSize) + "\n");
    outArcWriterObject.print(
        "yllcorner     " + (minLatitude - cellSize / 2.0 + 1 * cellSize) + "\n");
    ...
    MultiFormatFloat geogMatrix =
        new MultiFormatFloat(1, new long[] {nLinesTotal, nGeographyColumns});
    ...
    // now comes the tricky part. we need to put everything in the opposite order by row number, but
    // keep the
    // column numbers the same...

    MultiFormatFloat dataMatrixFinal =
        new MultiFormatFloat(1, new long[] {nLinesTotal, nRawColumns});
    MultiFormatFloat geogMatrixFinal =
        new MultiFormatFloat(1, new long[] {nLinesTotal, nGeographyColumns});
    ...
    for (int readRowIndex = nLinesTotal - 1; readRowIndex >= 0; readRowIndex--) {
      ...
      //    	System.out.println(readRowIndex + ": current considered = " +
      // currentGeogRowBeingConsidered + " ; old = " + oldGeogRowBeingConsidered);
      ...
      if (currentGeogRowBeingConsidered != oldGeogRowBeingConsidered || readRowIndex == 0) {
        ...
        for (int copyRowIndex = 0; copyRowIndex < nRowsToCopy; copyRowIndex++) {
          ...
          for (int colIndex = 0; colIndex < nGeographyColumns; colIndex++) {
            ...
            geogMatrixFinal.setValue(destinationRow, colIndex, tempValue);
          }
          ...
          for (int colIndex = 0; colIndex < nRawColumns; colIndex++) {
            ...
            dataMatrixFinal.setValue(destinationRow, colIndex, tempValue);
          }
        }
