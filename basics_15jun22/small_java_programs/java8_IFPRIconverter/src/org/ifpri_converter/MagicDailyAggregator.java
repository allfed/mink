...
//				System.out.println("nfe = " + nfe);
	...
	public static void main(String commandLineOptions[]) throws Exception {
		...
		if (commandLineOptions.length == 0) {
			System.out.println("Usage: org.ifpri_converter.MagicDailyAggregator input_file output_file\n" +
					"\n" +
					"The point of it is to take some data from Jawoo's downloading of the NASA-POWER daily stuff and try to aggregate " +
					"it up to monthly. But we have to check for double counting and all sorts of good things...." +
					"\n" +
...
//		MultiFormatFloat dailySolarAll   = new MultiFormatFloat(3,new long[] {nCellCodes,magicNYears,magicDaysInYear},Float.NaN);
...
//		MultiFormatFloat totalSolarRadiationThatIsValid   = new MultiFormatFloat(3,new long[] {nCellCodes,12});
...
//			lineIndex ++;
			if (lineIndex % 1000000 == 0) {
				System.out.println("reading line " + (lineIndex + 1) + " of " + nLinesTotal + " " + (((float)lineIndex + 1) / nLinesTotal));
			}
...
//				System.out.println("new: " + minCellCodeFound + "/" + maxCellCodeFound + " | " + cellID + "," + yearIndexToUse + "," + dayIndexToUse + "," + solarRadiation + "," + rainFall);
...
//				System.out.println("old: " + minCellCodeFound + "/" + maxCellCodeFound + " | " + cellID + "," + yearIndexToUse + "," + dayIndexToUse + "," + solarRadiation + "," + rainFall);
			}
...
//						thisYearsSolar[monthToConsider] += dailySolarAll.getValue(cellIndex,yearIndex,dayIndex);
						thisYearsCounts[monthToConsider]++;
						if (dailyIsRainyAll[cellIndex][yearIndex][dayIndex]) {
							thisYearsNRainy[monthToConsider]++;
						}
						debugOut.println(cellIndex + magicDelimiter + yearIndex + magicDelimiter + dayIndex + magicDelimiter + monthToConsider +
								magicDelimiter + thisYearsSolar[monthToConsider] + magicDelimiter + thisYearsCounts[monthToConsider] +
								magicDelimiter + thisYearsNRainy[monthToConsider]);
					}
				...
				for (int monthNumber = 1; monthNumber <= 12; monthNumber++) {
					...
					if (thisYearsCounts[monthNumber] == daysInThisMonth) {
						...
						totalSolarRadiationThatIsValid[cellIndex][monthNumber-1] += thisYearsSolar[monthNumber];
//						totalSolarRadiationThatIsValid.setValue(cellIndex,monthNumber-1,
//								totalSolarRadiationThatIsValid.getValue(cellIndex,monthNumber-1) + thisYearsSolar[monthNumber]);
					}
				} // end monthIndex
