...
public class MagicConverterSeveralCodeNonNumerics {
  ...
  public static void main(String commandLineOptions[]) throws Exception {
    ...
    if (commandLineOptions.length == 0) {
      System.out.println(
          "Usage: org.ifpri_converter.MagicConverter filename output_base_filename delimiter"
              + " pixelField non_numeric_column\n"
              + " ");
      System.exit(1);
    }
    ...
    for (int lineNumber = 0; lineNumber < nLinesTotal; lineNumber++) {
      ...
      for (int fieldIndex = 0; fieldIndex < nValueFields; fieldIndex++) {
        ...
        //        valueList[lineNumber][fieldIndex] =
        // Double.parseDouble(splitLine[valueFieldToConsider].trim());

        // we're gonna do this using the non-numerics coding thing...
        valueList[lineNumber][fieldIndex] =
            encoderObject.getCode(splitLine[valueFieldToConsider].trim());
      }
    ...
    for (int fieldIndex = 0; fieldIndex < nValueFields; fieldIndex++) {
      ...
      //      outputFile = filename.substring(0,filename.lastIndexOf(".")) + "_" + fieldIndex +
      // ".asc";
