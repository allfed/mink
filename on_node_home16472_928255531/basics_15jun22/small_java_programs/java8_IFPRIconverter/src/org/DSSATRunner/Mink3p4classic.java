...
//    static final private String magicOverviewRunBreak = "**************************************************************************************************************";
    ...
    public Mink3p4classic(String initFileNameToUse) throws IOException, Exception {
...
//	int indexOfFirstToKeep = findIndexForColumnName(firstExtraName, splitUpNamesHere);
    ...
    private void grabRealResultsByName(int nYears, long lineIndex, int plantingWindowIndex, String[] splitUpNamesHere, int[] endingIndicesForSplitUpNamesHere, String[] weatherFileContents, int howManyToSummarize)
...
//		startingDateAsInt = Integer.parseInt(startingDayToPlantCode);
		...
		readingTimer.tic();
		phenologyInDays = this.grabHappyResultsByName(nHappyPlantRunsForPhenology,splitUpNamesHere, endingIndicesForSplitUpNamesHere);
...
//		if (lineIndex == 0 && plantingWindowIndex == 0) {
		...
		if (happyIsGoodEnoughToProceedWithReal) {
		    ...
		    grabRealResultsByName(nFakeYears, lineIndex, plantingWindowIndex, splitUpNamesHere, endingIndicesForSplitUpNamesHere, weatherFileContents, howManyMonthsOfWeatherToSummarize);
...
//		    elseTimerStats.useDoubleValue(elseTimer.tocMillis());
		...
		if (realYieldsEntirePixel.getN() == 0) {
			...
			for (int weatherTypeIndex = 0; weatherTypeIndex < weatherAveragesNames.length; weatherTypeIndex++ ) {
			    for (int monthToAverageIndex = 0; monthToAverageIndex < howManyMonthsOfWeatherToSummarize; monthToAverageIndex++) {
				for (int fakeYearIndex = 0; fakeYearIndex < nFakeYears; fakeYearIndex++) {
				    statisticsOutLine += delimiter + FunTricks.onlySomeDecimalPlaces(monthlyAveragesAfterPlanting[fakeYearIndex][weatherTypeIndex*howManyMonthsOfWeatherToSummarize + monthToAverageIndex],nDecimalsInOutput);
				} // fakeYearIndex
			    } // monthToAverageIndex
			} // weather type index; i.e., sw / tmin / tmax / prec
