...
//		now prepare the dependent magic numbers...
		...
		// !!! since we are doing daily weather, we need to know what the first fake planting year is....
...
//		weatherExecutionCommand[6] = pathToDSSATDirectory + magicWeatherStationNameToUse + ".WTH"; // this is the CLI file
		...
		} else if (cropFertilizerSchemeToUse.equalsIgnoreCase(this.potatoes2String)) {
...
//			throw new Exception();
		...
		if (irrigationSchemeToUse.equalsIgnoreCase(this.maizeString)) {
			...
			irrigationScheme = new IrriSNone();
		} else if (cropFertilizerSchemeToUse.equalsIgnoreCase(this.threeSplitWithFloweringString)) {
...
//		irrigationScheme
	...
	//		int emergenceToUse = -3;
		...
		private void grabUnifiedNewManyResultsCopied(int nYears, long lineIndex, int plantingWindowIndex) throws InterruptedException, Exception {
			// declarations
			int plantingDate = -1;
			int startingDate = -2;
			int daysSincePlantingForEvent = -3;
			...
			for (int fakeYearIndex = 0; fakeYearIndex < nYears; fakeYearIndex++) {
				...
				try {
							...
							if (everythingIsValid[extraIndex]) {
								try {
									...
									if (stringToParse.indexOf("-") != stringToParse.lastIndexOf("-") ||
										...
										System.out.println("  two numbers likely smooshed together: [" + stringToParse + "]");
					...
					if (splitLine.length == magicNumberOfOutputsExpected) {
						...
						// let's try to go until we fail.... meaning that we just catch exceptions above...
						System.out.println("REAL: got " + splitLine.length + " rather than expected " + magicNumberOfOutputsExpected + 
...
*/
				...
				try {
					...
					if (maturityToUse > 0 && plantingDate > 0) {
						...
						if (daysSincePlantingForMaturity > 0) {
							...
							System.out.print("grab real: pd = " + plantingDate + " ; mat = " + maturityToUse + " ; diff = " + daysSincePlantingForMaturity);
					...
					for (int extraIndex = 0; extraIndex < extraNames.length; extraIndex++) {
						if (everythingIsValid[extraIndex]) { extraSummaryAccumulators[extraIndex].useDoubleValue(extractedValues[extraIndex]); }
					}
		...
		private void messygrabUnifiedNewManyResults(int nYears, long lineIndex, int plantingWindowIndex) throws InterruptedException, Exception {
		// declarations
		int plantingDate = -1;
		int startingDate = -2;
		int daysSincePlantingForEvent = -3;
		...
		// declarations with initializations
		int nLinesToRead = nYears + magicDSSATSummaryLineIndexToRead;
		String[] candidateSummaryContents = null; // new String[nLinesToRead];
		String stringToParse = null;
		...
		for (int fakeYearIndex = 0; fakeYearIndex < nYears; fakeYearIndex++) {
			...
			try {
				...
				// also, i want to see if the maturity length is like way crazy. if so, again: write down zero
				if (maturityToUse == 0 || maturityToUse > happyMaturityThresholdToDoRealRuns) {
					yieldToUse = 0;
				}
...
//				for (int extraIndex = 0; extraIndex < extraNames.length; extraIndex++) {
				// this should really go until we find all the ones we know are there rather than all that could be there...
//				System.out.println("splitLine.length = " + splitLine.length + "; - offset (" + magicOffsetForExtraNamesOutputIndex + ") = " +
//						(splitLine.length - magicOffsetForExtraNamesOutputIndex)
//				);
