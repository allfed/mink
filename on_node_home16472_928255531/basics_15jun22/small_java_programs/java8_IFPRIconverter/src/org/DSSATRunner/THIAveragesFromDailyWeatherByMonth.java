...
public class THIAveragesFromDailyWeatherByMonth {

    // the plan here is to try to read a pile of daily weather files
    // for each one, we will compute the "THI" for the day and decide how bad it is based on some arbitrary thresholds
    // then, we will count up how many of each category of badness there are within the years represented by the weather file
    //
    // the results will go out as a text file with the latitude and longitude and then a bunch of counts
    //
	...
	// set a default value, but allow the dawn humidity assumption to be adjusted...
	double approximateRelativeHumidityAtDawn   = 95.0;
	if (commandLineOptions.length == 3) {
		approximateRelativeHumidityAtDawn   =    Double.parseDouble(commandLineOptions[2]);
	}
...
//	final int rainEnd = 29;
	...
	final int nDecimalsToKeep = 2;

	final double lowestReasonableTemperatureForFiguringTHIinF = 58.0; // weird thing happen below 58F, because it is designed for use above there; so we are going to censor from there...
	...
	double tminHereInF = -5.5;
	double approximateTHItoday = -5.3;
	double approximateTHIAtDawn = -5.3;
	...
	DescriptiveStatisticsUtility[] highTHI = new DescriptiveStatisticsUtility[nMonths];
	DescriptiveStatisticsUtility[] lowTHI = new DescriptiveStatisticsUtility[nMonths];

	// initialize the stat things
	for (int monthIndex = 0; monthIndex < nMonths; monthIndex++) {
	    highTHI[monthIndex] = new DescriptiveStatisticsUtility(true);
	    lowTHI[monthIndex] = new DescriptiveStatisticsUtility(true);
	}
	...
	if (outputFileObject.exists()) {
	    System.out.println("overwriting intended output [" + outputFileObject.getCanonicalPath() + "]");
	}
	...
	// ok, now we have a problem in that we need to have a separate output file for each year, i think. so then we have to know what years we have
	// to work with and initialize those files....

	// determine the years in the first weather file and assume they all are the same
	System.out.println("Assuming that all weather files in the source directory have the same day/year coverage.");
	...
	for (int lineIndex = magicFirstLineToWorkOn; lineIndex < originalAsArray.length; lineIndex++) {
		...
		particularOutputFilename = destinationFilename + DSSATHelperMethods.padWithZeros(yearInFile, 2) + ".txt";
		...
		particularProvenanceFilename = destinationFilename + DSSATHelperMethods.padWithZeros(yearInFile, 2) + ".provenance.txt";

		FunTricks.appendLineToTextFile("sourceDirectory = [" + sourceDirectory + "]", particularProvenanceFilename, false); // make a new file
		FunTricks.appendLineToTextFile("destinationFilename = [" + destinationFilename + "]", particularProvenanceFilename, true); // append from there...
		FunTricks.appendLineToTextFile("approximateRelativeHumidityAtDawn = [" + approximateRelativeHumidityAtDawn + "]", particularProvenanceFilename, true); // append from there...
	...
	for (int fileIndex = 0; fileIndex < inputFiles.length; fileIndex++) {
	    ...
	    // the plan is to read through all the days and look at the high and low temperatures for the day and work out an approximate
	    // relative humidity and temperature-heat-index (THI)
	    //
	    // the worst THI will usually happen around the high of the day, so we need the relative humidity the.
	    // that will be approximated by the dew point which will be approximated by the previous day's low.
	    //
	    // the best THI will usually happen around the low of the day
	    //
	    ...
	    // determine latitude and longitude from the file name since we might be dealing with jawoo's POWER wth files that he
	    ...
	    String strippingOffSuffix = simpleInputFile.substring(0, simpleInputFile.lastIndexOf(".")); // getting rid of .WTH or .csv or whatever
	    String[] underscoreSplit = strippingOffSuffix.split("_"); // separating the various elements so we can find the coordinates
		...
		originalDate = Integer.parseInt(originalAsArray[lineIndex].substring(0, magicLengthOfDate).trim());
		yearInFile = originalDate / 1000;
		dayInFile = originalDate - 1000*yearInFile;
...
//		    particularOutputFilename = destinationFilename + DSSATHelperMethods.padWithZeros(yearInFile, 2) + ".txt"; // this is the original ?wrong way: i am writing it out under the wrong year number...
		    // and also i was forgetting to finish out the last year under its own name after the loop is done...
		    
		    particularOutputFilename = destinationFilename + DSSATHelperMethods.padWithZeros(previousYearInFile, 2) + ".txt";
		    
		    outputLine = latitude + "," + longitude; // + "," + notInAnyBin;
		    
		    // output the THI_min by month
		    for (int monthIndex = 0; monthIndex < nMonths; monthIndex++) {
			// use a NaN since GRASS can interpret that (unlike DSSAT, hence my onlySomeDecimalPlaces will return -0.01 for weird stuff)
			if (lowTHI[monthIndex].getN() == 0) {
			    outputLine += ",NaN";
			} else {
			    outputLine += "," + FunTricks.onlySomeDecimalPlaces(lowTHI[monthIndex].getMean(),nDecimalsToKeep);
			}
		    }
		    
		    // output the THI_max by month
		    for (int monthIndex = 0; monthIndex < nMonths; monthIndex++) {

			if (highTHI[monthIndex].getN() == 0) {
			    outputLine += ",NaN";
			} else {
			    outputLine += "," + FunTricks.onlySomeDecimalPlaces(highTHI[monthIndex].getMean(),nDecimalsToKeep);
			}
		    }
		    ...
		    for (int monthIndex = 0; monthIndex < nMonths; monthIndex++) {
			highTHI[monthIndex].reset();
			lowTHI[monthIndex].reset();
		    }

//		    System.out.println("   done outputting to year: " + particularOutputFilename);
		...
		tminHereInF = tminHere * 9/5 + 32;

		if (tmaxHereInF >= lowestReasonableTemperatureForFiguringTHIinF) {
		    ...
		    approximateTHItoday = tmaxHereInF - (0.55 - 0.55 * approximateRelativeHumidity / 100) * (tmaxHereInF - 58);
		} else {
		    // we will just keep the temperature with no adjustment
		    approximateTHItoday = tmaxHereInF;
		}

		
		// but, for the THI_min, we really want the daybreak THI and our (super simplistic) assumption is that
		// it is roughly 100% humidity at dawn. not always true, but probably good enough when it is warm and not completely dry
		// maybe we'll hedge and say 95%
		// this is specified by the default above and/or via commandline option
		
		if (tminHereInF >= lowestReasonableTemperatureForFiguringTHIinF) {
		    approximateTHIAtDawn = tminHereInF - (0.55 - 0.55 * approximateRelativeHumidityAtDawn / 100) * (tminHereInF - 58);
		} else {
		    approximateTHIAtDawn = tminHereInF;
		}
		
//		System.out.println("THI = " + approximateTHItoday);

		// ok, so what month are we in?
		thisMonthIndex = DSSATHelperMethods.monthIndexFromDayNumber(dayInFile);
		
		lowTHI[thisMonthIndex].useDoubleValue(approximateTHIAtDawn);
		highTHI[thisMonthIndex].useDoubleValue(approximateTHItoday);
		
		// since it is about to be "tomorrow", we need to copy the year and temperature into holding spots...
		yesterdaysTmin = tminHere;
		previousYearInFile = yearInFile;

	    } // for lineIndex

	    // now, we have to take care of the final year in the original text file...
	    // that means, the year number is the CURRENT year number, not the previous one...
	    particularOutputFilename = destinationFilename + DSSATHelperMethods.padWithZeros(yearInFile, 2) + ".txt";
	    ...
	    // debugging...
//	    System.out.println("finished with: " + particularOutputFilename);
	    
	    
	    
	    // ok, now we can check on the status


	    if (fileIndex % (inputFiles.length / 400 + 1) == 0) {
		double timeMinutes = ((System.currentTimeMillis() - startTime)/1000.0/60);

		System.out.println((fileIndex + 1) + "/" + inputFiles.length + " = " +
			FunTricks.fitInNCharacters((100 * (fileIndex + 1.0)/inputFiles.length),5) + "\t" + 
			((System.currentTimeMillis() - startTime)/1000) + "s\t" +
			timeMinutes + "m\t" +
			(timeMinutes * ((inputFiles.length * 1.0) / (fileIndex + 1)) - timeMinutes) + "m remaining"
			);
