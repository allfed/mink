    ...
    /////////////////////
  	...
  	////////////////////////////////////////
		...
		if (nitrogenFertilizerAmount < 20) {
			...
			daysAfterPlantingForApplication[0] = 1;
			             rateAppliedKgPerHa[0] = 20;

			// apply rest halfway through dumping period
			daysAfterPlantingForApplication[1] = (int) ( (dapFertilizerRegimeBegins+dapFertilizerRegimeFinishes)/(nApplicationsAfterInitial+1) );
      ...
      // apply the rest roughly evenly through the fertilizer dumping period in 3 steps
             			nApplicationsAfterInitial = 3;
      afterInitialRate = (int)((nitrogenFertilizerAmount - rateAppliedKgPerHa[0])/nApplicationsAfterInitial);
      afterInitialInterval = (int)Math.max((dapFertilizerRegimeFinishes - dapFertilizerRegimeBegins)/nApplicationsAfterInitial,0.0);

      // all but the last one, so we can make the last one match up nicely
      totalAppliedSoFar = rateAppliedKgPerHa[0];
      for (int intervalIndex = 0; intervalIndex < nApplicationsAfterInitial - 1; intervalIndex++) {
        daysAfterPlantingForApplication[intervalIndex + 1] =
        	dapFertilizerRegimeBegins + intervalIndex * afterInitialInterval;
        rateAppliedKgPerHa[intervalIndex + 1] = afterInitialRate;
        totalAppliedSoFar += rateAppliedKgPerHa[intervalIndex + 1];
      }
      daysAfterPlantingForApplication[(int)nApplicationsAfterInitial] =
      	dapFertilizerRegimeBegins + (int)nApplicationsAfterInitial * afterInitialInterval;
      rateAppliedKgPerHa[(int)nApplicationsAfterInitial] = (int)nitrogenFertilizerAmount - totalAppliedSoFar;
