...
public class JustBuildInitializationBlocks {
	...
	public static void main(String commandLineOptions[]) throws Exception {
		...
		if (initializationDayCode.length() > appropriateDayCodeLength) {
			initializationDayCode = initializationDayCode.substring(initializationDayCode.length() - appropriateDayCodeLength);
		}
		
		SoilProfile soilProfiles = new SoilProfile(soilTypeString.substring(0, 2) + ".SOL"); 

    double
		totalInitialNitrogenKgPerHa 									= Double.parseDouble(initializationValues[readIndex++].split(":")[1].trim()),
		rootWeight 																		= Double.parseDouble(initializationValues[readIndex++].split(":")[1].trim()),
		surfaceResidueWeight 													= Double.parseDouble(initializationValues[readIndex++].split(":")[1].trim()),

    fractionBetweenLowerLimitAndDrainedUpperLimit = Double.parseDouble(initializationValues[readIndex++].split(":")[1].trim()),
		depthForNitrogen 															= Double.parseDouble(initializationValues[readIndex++].split(":")[1].trim()),
		residueNitrogenPercent 												= Double.parseDouble(initializationValues[readIndex++].split(":")[1].trim()),
		incorporationRate 														= Double.parseDouble(initializationValues[readIndex++].split(":")[1].trim()),
		incorporationDepth 														= Double.parseDouble(initializationValues[readIndex++].split(":")[1].trim());

    String clayLoamSandStableCarbonRatesFilename = initializationValues[readIndex++].split(":")[1].trim();
		...
		String initializationBlock = soilProfiles.makeInitializationAndSoilAnalysisBlock(
				soilTypeString,
