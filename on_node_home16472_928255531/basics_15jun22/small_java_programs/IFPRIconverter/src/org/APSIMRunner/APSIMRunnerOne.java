...
public class APSIMRunnerOne {
	...
	private final static String weatherFilePlaceholder  = "WEATHER_FILE_HERE";
	private final static String startDatePlaceholder    = "START_SIMULATION_DATE";
	private final static String endDatePlaceholder      = "END_SIMULATION_DATE";
	private final static String plantingDatePlaceholder = "PLANTING_DAY";
	private final static String soilInfoPlaceholder     = "SOIL_AND_SURFACE_HERE";
	private final static String outputTitlePlaceholder  = "NAME_OF_THING";
	private final static String officialCropPlaceholder = "OFFICIAL_CROP_NAME";
	private final static String outputFilePlaceholder   = "OUTPUT_NAME";
...
//	private final static String modelLogFile 				= "/dev/null";
	...
	private final static String APSIMWeatherFile = "w_from_dssat.met";

	
	private final static String DSSATClimateInput  = "ZZZZ";
	private final static String standaloneWeatherOutput = DSSATClimateInput + ".WTG"; // "Weather.OUT";
	...
	private final static File weatherOutputFileAsFile = new File(standaloneWeatherOutput);
	...
	private final static int happyMaturityIndex = 13;
	private final static int realMaturityIndex  = 21;
	...
	String standaloneCommandTemp = null;

	private String[] oryzaExecutionCommand = null;
	private String[] dssatExecutionCommand = null;
	private String[] standaloneWeatherCommand = new String[6];
	...
	private String officialCropName = null;
	...
	public APSIMRunnerOne() throws FileNotFoundException {
...
//		oryzaExecutionCommand = splitInput;
	...
	public void setStandaloneExecutionCommand(
			String binaryName,
		...
		// assign it to the real variable
		standaloneWeatherCommand[0] = binaryName;
		standaloneWeatherCommand[1] = startDateCode;
		standaloneWeatherCommand[2] = endDateCode;
		standaloneWeatherCommand[3] = randomSeed;
		standaloneWeatherCommand[4] = generatorCode;
		standaloneWeatherCommand[5] = cliFile;
	}
	...
	public void readInitializationFile(String filename) throws IOException, Exception {
		...
		standaloneCommandTemp = initFileContents[storageIndex++];
...
//		fakePlantingYear    = Integer.parseInt(initFileContents[storageIndex++]);
		...
		officialCropName         = initFileContents[storageIndex++];
...
//		controlFileContents += "FILEI1 = '" + varietyFile + "'" + newLineToUse;
	...
	public void prepareHappyRunUnique(int plantingDay) throws FileNotFoundException {
		// prepare the control file
		// Beware the MAGIC NUMBER!!! almost everything. some of these should eventually get split out so
		// as to be able to be controlled and/or not be so magic.
...
//		controlFileContents += "FILEI1 = '" + varietyFile + "'" + newLineToUse;
	...
	public void prepareRealRun(int plantingDay, double fertilizerAmount) throws FileNotFoundException, Exception {
...
//			57., 40.
			...
			if (fertilizerAmount < 60) {
				realRerunContents += "FERTIL = " + magicFirstDayForFertilizer + "," + fertilizerAmount + newLineToUse;
			} else {
  ...
  public void prepareRealRunUnique(int plantingDay, double fertilizerAmount) throws FileNotFoundException, Exception {
...
//					summaryAccumulatorsHappy[outputIndex].useDoubleValue(Double.parseDouble(lineAsArray[outputIndex]));
	...
	public void readResultsFileSpecifyLength(boolean realFlag, int nRepetitions) throws IOException, Exception {
//	String[] lineAsArray = null;
	String[] candidateSummaryContents = FunTricks.readTextFileToArray(rawOutputFile);
...
//				summaryAccumulatorsHappy[outputIndex].useDoubleValue(Double.parseDouble(lineAsArray[outputIndex]));
	...
	public void buildAPSIMWeatherFromDSSATSpecifySeed(MultiFormatMatrix dataTable,
		...
		///////////////////////////////////////////////////////////////////////////
		// now we will try to do this using the standalone weather generator.... //
		///////////////////////////////////////////////////////////////////////////
		...
		// note: we will be ignoring the CO2 numbers from the weather generator since i think we can/are specifying that
		// elsewhere in the APSIM definitions...
...
//		this.convertDSSATWeatherToORYZA(dataTable, geogTable, lineIndex);
		this.convertDSSATWeatherToAPSIM(dataTable, geogTable, lineIndex);
	...
	public void convertDSSATWeatherToAPSIM(MultiFormatMatrix dataTable, MultiFormatMatrix geogTable, long lineIndex)
		...
		String[] weatherFileArray = FunTricks.readTextFileToArray(standaloneWeatherOutput);
		
		
		// make the header line for the oryza style...
		// long, lat, elevation, angstrom A, angstrom B (zeros in our case)
		double latitude  = geogTable.getValue(lineIndex,2); // Beware the MAGIC NUMBER!!! column for latitude

		
		apsimStyle  = "! random weather data generated via DSSAT's weather generators..." + newLineToUse;
		apsimStyle += "Latitude = " + latitude + newLineToUse; // put in the latitude
		apsimStyle += "co2 = " + co2ppm + newLineToUse; // put in the co2

		double tav = dataTable.getValue(lineIndex,67); // Beware the MAGIC NUMBER!!! column for annual average temperature
		double amp = dataTable.getValue(lineIndex,68); // Beware the MAGIC NUMBER!!! column for annual temperature amplitude
		
		apsimStyle += "tav = " + tav + " (oC)     ! annual average ambient temperature" + newLineToUse;
		apsimStyle += "amp = " + amp + " (oC)     ! annual amplitude in mean monthly temperature" + newLineToUse;

		// the header lines
		apsimStyle += "year day radn     mint maxt rain" + newLineToUse;
		apsimStyle += "()   ()  (MJ/m^2) (oC) (oC) (mm)" + newLineToUse;
		
		// now process the daily weather....
		...
		FunTricks.writeStringToFile(apsimStyle, APSIMWeatherFile);
...
//		provenanceOut.print("dssatBinaryNameTemp:\t" + dssatBinaryNameTemp + "\n");
		...
		provenanceOut.print("templateFile:\t" + templateFile + "\n");
		...
		provenanceOut.print("APSIMWeatherName =\t\t\t"            + APSIMWeatherFile + "\n");
		provenanceOut.print("DSSATWeatherOutput =\t\t\t"            + standaloneWeatherOutput + "\n");
	...
	public void runIt() throws IOException, Exception {
		...
		double rootWeight = -10;
		double surfaceResidueWeight = -20;

		String soilSettingsString = null;
...
//		long rootWeightCol           = 72;
		long initialNitrogenColumn = 70;
		long initialRootWeightColumn = 71;
		long surfaceResidueWeightCol = 72;
...
//			long initialRootWeightColumn = 71;
			...
			// Beware the MAGIC NUMBER!!! column number for initial root weight
			totalInitialNitrogenKgPerHa = dataMatrix.getValue(dataLineIndex,initialNitrogenColumn);
			rootWeight                  = dataMatrix.getValue(dataLineIndex,initialRootWeightColumn);
			surfaceResidueWeight        = dataMatrix.getValue(dataLineIndex,surfaceResidueWeightCol);

			soilSettingsString = this.testProfileObject.convertProfileToAPSIM(
					officialCropName,
					soilTypeString,
			...
			for (int plantingWindowIndex = 0; plantingWindowIndex < nPlantingWindowsPerMonth; plantingWindowIndex++) {
				...
				for (int happyPlantSeedIndex = 0; happyPlantSeedIndex < this.nHappyRepetitions; happyPlantSeedIndex++) {
					...
					weatherTimer.tic();
					this.buildAPSIMWeatherFromDSSATSpecifySeed(dataMatrix, geogMatrix, dataLineIndex, randomSeedActualValue);
...
//				System.out.println(summaryAccumulatorsHappy[this.commercialWeightIndex].getAllPretty());
				...
				if (happyYieldMeanThisWindowOnly >= happyYieldThresholdToDoRealRuns) {
					...
					for (int realSeedIndex = 0; realSeedIndex < this.nRealRepetitions; realSeedIndex++) {
						...
						weatherTimer.tic();
						buildAPSIMWeatherFromDSSATSpecifySeed(dataMatrix, geogMatrix, dataLineIndex, randomSeedActualValue);
