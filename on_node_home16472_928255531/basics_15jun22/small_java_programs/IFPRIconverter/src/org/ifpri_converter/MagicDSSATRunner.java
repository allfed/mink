   ...
   2 017.9  27.7  09.6 001.0 000.0
	...
	public static void main(String commandLineOptions[]) throws Exception {
		...
		if (commandLineOptions.length == 0) {
			System.out.println("Usage: org.ifpri_converter.MagicDSSATRunner GIS_table_base_name template_X_file yield_output_base_name ALL_flag settings_csv\n" +
					"\n" +
...
//		RandomAccessFile weatherStationFileWriter = new RandomAccessFile(weatherStationFileObject,"rwd"); // trying the extra "d" for the fun of it
...
//					System.out.println("D #" + nLines + ":" + (endNanos - startNanos)/1000000.0 + " ; ");
...
//							System.out.println("substring = [" + goodSummaryLine.substring(magicHarvestedWeightAtHarvestStartIndex, magicHarvestedWeightAtHarvestEndIndex) + "]");
							...
							System.out.println("Failed to read summary properly on try #" + nParsingTries + ", trying again [number format exception ]; line " + nLines + ", window = " + plantingWindowIndex + ", seed = " + randomSeedIndex);
							nfe.printStackTrace();
						} catch (NullPointerException npe) {
							// do nothing so that it will return and try again							
							System.out.println("Failed to read summary properly on try #" + nParsingTries + ", trying again [null pointer exception  ]; line " + nLines + ", window = " + plantingWindowIndex + ", seed = " + randomSeedIndex);
							npe.printStackTrace();
...
//							System.out.println("Failed to read summary properly on try #" + nParsingTries + ", trying again [file not found exception] + (line " + nLines + ", window = " + plantingWindowIndex + ", seed = " + randomSeedIndex);
							if (nParsingTries > maxParsingTries) {
								System.out.println("re-running DSSAT after " + nParsingTries + "; line " + nLines + ", window = " + plantingWindowIndex + ", seed = " + randomSeedIndex);
								Runtime.getRuntime().exec(dssatExecutionCommand , null , pathToDSSATDirectoryAsFile);
								nParsingTries = 0;
...
//								System.out.print("[" + nParsingTries + "]");
							...
							System.out.println("Failed to read summary properly on try #" + nParsingTries + ", re-running DSSAT [string index out of bounds exception]; line " + nLines + ", window = " + plantingWindowIndex + ", seed = " + randomSeedIndex);
							Thread.sleep(sleeptimeWaitForFileMillis);
							Runtime.getRuntime().exec(dssatExecutionCommand , null , pathToDSSATDirectoryAsFile);
						}
		...
		System.out.println(" total time in DSSAT = " + totalDSSATTimeNanos / 1000000000.0 / 60 + "min ; per run average = " 
				+ totalDSSATTimeNanos / nLines / nRandomSeedsToUse / nPlantingWindowsPerMonth / 1000000.0 + "ms");
