...
//	private final static String modelLogFile 				= "/dev/null";
	...
	private final static int nOutputVariables = 13;
...
*/
		...
		summaryAccumulatorsHappy = new DescriptiveStatisticsUtility[nOutputVariables];
		summaryAccumulatorsReal  = new DescriptiveStatisticsUtility[nOutputVariables];
		for (int outputIndex = 0; outputIndex < nOutputVariables; outputIndex++) {
...
//		mark that we're good to go...
	...
	public void readResultsFile(boolean realFlag) throws IOException, Exception {
		...
		for (int lineIndex = 1; lineIndex < candidateSummaryContents.length; lineIndex++) {
			...
			if (realFlag == this.REAL_RUN) {
				for (int outputIndex = 1; outputIndex < nOutputVariables; outputIndex++) {
	...
	public void buildORYZAWeatherFromDSSAT(MultiFormatMatrix dataTable,
			...
			throws InterruptedException, FileNotFoundException, IOException, Exception {

		TimerUtility thingee = new TimerUtility();
		DescriptiveStatisticsUtility thingeeDSSATStats = new DescriptiveStatisticsUtility(true);
		DescriptiveStatisticsUtility thingeeWritingStats = new DescriptiveStatisticsUtility(true);
		...
		// execute DSSAT
//		System.out.println("\t\t    ==> attempting the weather/DSSAT run...");
		thingee.tic();
		...
		thingeeDSSATStats.useDoubleValue(thingee.tocSeconds());
		
		// interpret the output
		thingee.tic();
		this.convertDSSATWeatherToORYZA(dataTable, geogTable, lineIndex);
		thingeeWritingStats.useDoubleValue(thingee.tocSeconds());
		
		System.out.println("        -- DSSAT took " + thingeeDSSATStats.getTotalDouble() + "; writing " + thingeeWritingStats.getTotalDouble());
//		System.out.println(" upon completion of DSSAT run, we find " + FunTricks.nLinesInTextFile(DSSATWeatherOutput) + " lines in the weather file...");
	}
...
//			int nitrogenLevel = nonClimateInfo[4];
			...
			for (int plantingWindowIndex = 0; plantingWindowIndex < nPlantingWindowsPerMonth; plantingWindowIndex++) {
				...
				// run the HAPPY case
				System.out.println("==> attempting the happy run... (window " + plantingWindowIndex + ")");
				...
				if (happyYieldMeanThisWindowOnly >= happyYieldThresholdToDoRealRuns) {
					...
					if (!haveBuiltRealWeather) {
						...
						System.out.println("  .. just built real weather for line " + dataLineIndex + " window " + plantingWindowIndex + " ..");
					} else {
						System.out.println("  .. skipping   real weather for line " + dataLineIndex + " window " + plantingWindowIndex + " ..");
					}


					// do the real runs
					prepareRealRun(magicPlantingDay);
					System.out.println("==> attempting the real run...");
					rawOutputFileAsFile.delete(); // clear out any old results file...
				...
				System.out.println("==== REAL plant results ====");
				System.out.println("commercial weight...");
