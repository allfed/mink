...
public class SimpleSender {
  ...
  public SimpleSender(String ComputerName, int portNumber, long SleepTime, long BadConnectionRetries) {
  ...
  public SimpleSender(String ComputerName, int portNumber, long SleepTime, long BadConnectionRetries, String label) {
    this.portNumber           = portNumber;
    this.ComputerName         = ComputerName;
    this.SleepTime            = SleepTime;
    this.BadConnectionRetries = BadConnectionRetries;
  ...
  public void establishConnection() throws Exception {
  	...
  	if (weHaveAGoodConnection) {
  		System.out.println("  --[" + this.toString() + "] already has a good connection; doing nothing--");
  		return;
  	}
    ...
    for (long badRetry = 0; badRetry < BadConnectionRetries; badRetry++) {
      try {
        ...
        System.out.println("  --[" + this.toString() + "] failed to connect to [" + ComputerName + "] on port [" + portNumber + "] current retry = " + badRetry + "--");
  ...
  public void sendObject(Object X) throws Exception {
    ...
    if (verboseStatus) {
    	System.out.println(     "[" + this + "/" + label +"] sent [" + X + "] as #" + nObjectsSent + " at " + new Date());
