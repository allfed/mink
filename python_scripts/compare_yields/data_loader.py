...
def load_data(
    ...
    ) = get_codes(crop_value, water_values)
    ...
    if water_key == "overall":
        if (
            ...
            world = load_faostat_data(
                world, git_root, faostat_code, settings["faostat_data_location"]
            )
        ...
        if (
            ...
            or comparisons_to_run["plot_SPAM_country_map"]
            or comparisons_to_run["plot_hist_side_by_side"]
            or comparisons_to_run[
                "plot_by_country_scatter"
            ]  # need the area in this case
        ):
            world = load_by_country_csv(
                world,
                git_root,
...
def get_average_of_rasters_over_time(
    ...
    SPAM_production_ascii_file = os.path.join(
        f"{git_root}wth_historical", f"{crop_key}_production.asc"
    )
    try:
        historical_data = import_ascii(SPAM_production_ascii_file, "production")
    ...
    for cat_or_cntrl in cat_or_cntrl_list:
        ...
        for year in years:
            ...
            try:
                model_results_by_year = import_ascii(ascii_loc, "production")
            except FileNotFoundError:
            ...
            # combined_data = remove_rows_with_nan(
    ...
    # this is the smae function commented out, but with yields per hectare, not overall production
    # def get_average_of_rasters_over_time(
    #     years,
    #     git_root,
    #     crop_key,
    ...
    #         f"{git_root}wth_historical", f"{crop_key}_production.asc"
    #     )
    #     historical_data = import_ascii(SPAM_yields_ascii_file, "yields")
    ...
    #                 each_year_base_filename + "_y" + str(year) + ".asc",
    #             )
    #             model_results_by_year = import_ascii(ascii_loc, "yields")
...
def load_by_country_csv(
    ...
    production_area_by_country_csv = os.path.join(
        git_root, csv_folder, country_csv_filename
    )
    ...
    world_no_zero_area_or_non_existent_countries = world.dropna(
        subset=[data_category + "_average_yield"]
    )
