...
public class ParseOverview {
    ...
    static private int countRuns(String overviewFileName, String runSeparator) throws IOException {
...
//		int magicLengthOfGrowthStageNameAndTimeSpan = 30;
    ...
    static private DescriptiveStatisticsUtility[][] originalExtractStressIndicesWpsWgNpsNg(
...
//	int nGroupsFound = 0;
	...
	DescriptiveStatisticsUtility[][] stressAccumulators = new DescriptiveStatisticsUtility[nGrowthStages][4];
	...
	// ok, repeat a lot of the logic from the growth stage counter, just this time we need to read the values
...
//	int nitrogenSynthStartIndex = -2;
	...
	while (keepLookingForRuns) {
	    ...
	    // a cheater way of trying to bail out: short circuit the while loop to get it back to its natural check
...
System.out.println("overviewFileArray[" + lineIndex + "] = [" + overviewFileArray[lineIndex] + "]");
		    ...
		    // character of the line is almost always a space...
//		    System.out.println("candidate string = [" + overviewFileArray[lineIndex] + "]");
		    int finalSpaceIndex = overviewFileArray[lineIndex].substring(0, magicLengthOfGrowthStageNameAndTimeSpan).lastIndexOf(magicSpace);
		    // grab everything before that
		    // trim it up
		    // convert spaces and hyphens to underscores
		    // Beware the MAGIC NUMBER!!! the prefix for the growth stage names so that we can find them easily in lists
		    // of variable names... (i.e., for importing the maps into GRASS)
		    String paddedGrowthStageNameFromOverviewDotOut = magicExtraPrefixForGrowthStages + "_" 
			    + overviewFileArray[lineIndex].substring(0, finalSpaceIndex).trim().replaceAll("-", "_").replaceAll(" ","_").replaceAll("\\.","_");
...
//	System.out.println("file: " + overviewToTry + " contains " + nRuns + " runs, we think...");
	...
	DescriptiveStatisticsUtility[][] growthStageStressAccumulators = extractStressIndicesWpsWgNpsNg(
		overviewAsArray, growthStageNames, waterSynthStartIndexToUse, nitrogenSynthStartIndexToUse, null
		);
	...
	for (int growthStageIndex = 0; growthStageIndex < growthStageNames.length; growthStageIndex++) {
	    System.out.println("[" + growthStageIndex + "] = [" + growthStageNames[growthStageIndex] + "]");
	}
	...
	for (int growthStageIndex = 0; growthStageIndex < growthStageStressAccumulators.length; growthStageIndex++) {
	    for (int stressIndex = 0; stressIndex < growthStageStressAccumulators[0].length; stressIndex++) {
		System.out.println("growth stage index " + growthStageIndex + " [" + growthStageNames[growthStageIndex] + "][" + stressIndex + "] = " +
