...
public class MagicHCgridMatcherForMappingRawSimulationsOptimizer {
...
//		Date startTime = new Date();
		...
		if (commandLineOptions.length == 0) {
			System.out.println("Usage: org.ifpri_converter.MagicHCgridMatcherForMappingRawSimulationsOptimizer cell_info_file data_file output_file\n" +
					...
					"\n" +
					"The cell_info_file file should be the tab delimited list of pixels of the form: cell_id<tab>soil_number<tab>planting month\n" +
					"with one pixel on each line.\n" +
					"\n" +
					"The data_file should be tab delimited and sorted by cell id number (whichever column that is). The\n" +
					...
					"* CO2 concentration\n" +
					"\n" +
					"The column_designations_csv should be a comma separated list specifying which column INDEX corresponds\n" +
					"to each of the values in the above list (and in that order).\n" +
					"\n" +
					"The values_to_find_csv should be a comma separated list specifying the specific values desired for the\n" +
			...
			"\n" +
			"This particular implementation assumes that there might be multiple cases fitting the bill for each pixel.\n" +
			"The idea is to pull out the best one (highest yield) for mapping.");
			System.exit(1);
		}
		...
		double highestYieldValue = -5;
		double highestYieldLine = -6;
		int nFound = -7;
...
//				System.out.println("...dCN = " + dataCellNumber + "; cNIM = " + cellNumberInMap);
				...
				if (dataLineContents != null) {
					...
					System.out.println("    == hit end of data file, setting dataCellNumber to Integer.MAX_VALUE (" + dataCellNumber + ")");
				}
			}
			...
			if ((dataCellNumber > cellNumberInMap) && !cellNotInDataTable) {
				// we have overshot. that is, there is no data for the cell the map wants
				// to match to. we will record a "missing data" value.
//				System.out.println("....dCN = " + dataCellNumber + "; cNIM = " + cellNumberInMap + "(overshot second search)");
...
//				if (dataCellNumber != cellNumberInMap) {
//					System.out.println("   we have a problem at dCN = " + dataCellNumber + " ; cNIM = " + cellNumberInMap);
//				}
...
////////////////////
				...
				highestYieldValue = Double.NEGATIVE_INFINITY;
				highestYieldLine = -1; // Beware the MAGIC NUMBER!!! initializing with a negative number
				nFound = 0;
				while (dataCellNumber == cellNumberInMap) {
					// step through the data file until we run out of lines for the pixel we're looking at....
					
					// check whether this is a "worthy" line, and update the best yield...
					...
					if (everythingMatches) {
						if (dataYield > highestYieldValue) {
							highestYieldValue = dataYield;
							highestYieldLine = currentDataLineNumber;
							nFound++;
				...
				} // end while cell == data cell
				
				// now decide what to write....
				// finish out the nFound lines for debugging
				System.out.println();
				if (nFound == 0) {
					// bummer, write the bad values
					...
					// yippee! we managed to find something. so we should write down the good values.
					dataOut.print(cellNumberInMap + magicDelimiter);
					dataOut.print(soilNumberInMap + magicDelimiter);
					dataOut.print(plantingMonthInMap + magicDelimiter);
					dataOut.print(dataClimateCode + magicDelimiter);
					dataOut.print(dataWaterSource + magicDelimiter);
					dataOut.print(highestYieldValue + magicDelimiter);
//					dataOut.print(dataYield + magicDelimiter);
					dataOut.print(dataNitrogen + magicDelimiter);
					dataOut.print(dataCo2 + magicDelimiter);
//					dataOut.print(currentDataLineNumber + "\n");
					dataOut.print(highestYieldLine + "\n");
				}
//////////////////////
...
//							co2ToFind           == dataCo2
//					);
//					// if dataCellNumber > cellNumberInMap; i.e., we have overshot when looking within this pixel code
//
...
//		}
		...
		outInfoWriterObject.print((nCellLinesProcessed * magicNumberOfOutputColumns) + "\t = Total Number of Elements\n");
