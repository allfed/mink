...
//	private boolean useFloating = false;
	...
	public void useDoublePair(double sortingValue, double[] friendValues) throws Exception {
			...
			for (int friendColIndex = 0; friendColIndex < nFriendCols; friendColIndex++) {
				...
				doubleFriendSquaredTotal[friendColIndex] += friendValues[friendColIndex] * friendValues[friendColIndex];
...
//					doubleArrayRaw[rawIndex][1] = doubleArrayRawCopy[rawIndex][1];
	...
	public double[][] sortDatasetWithCumulativeFraction() throws Exception {
		...
		double[][] sortedWithCumulativeFraction = new double[nNumbers][nFriendCols+2]; // sorted + friends + cumulative fraction
...
//			System.out.println("sI = " + quantileStartIndex + "; eI = " + quantileEndIndex + "; qmIl = " + quantileMedianIndexLo + "; qmIH = " + quantileMedianIndexHi);
			
			sortedMedianValue = (sortedVersion[quantileMedianIndexLo][0] + sortedVersion[quantileMedianIndexHi][0]) / 2; // Beware the MAGIC NUMBER!!! the sorting values are in column 0
			...
			for (int friendCol = 0; friendCol < nFriendCols; friendCol++) {
				...
				for (int rowIndex = 0; rowIndex < nInThisQuantile; rowIndex++) {
					friendValuesToSort[rowIndex][0] = sortedVersion[quantileStartIndex + rowIndex][1 + friendCol]; // Beware the MAGIC NUMBER!!! the friends are in column 2
				}
				...
				friendMedianValue = (friendValuesAfterSorting[quantileMedianIndexLo - quantileStartIndex][0] +
						friendValuesAfterSorting[quantileMedianIndexHi - quantileStartIndex][0]) / 2; // Beware the MAGIC NUMBER!!! there is only a single column here
...
//		return new double[] {doubleSortingTotal / nNumbers , doubleFriendTotal / nNumbers};
	...
	public double[] getStds() {
			...
			for (int friendIndex = 0; friendIndex < nFriendCols; friendIndex++) {
				allStds[1+friendIndex] = Math.sqrt(doubleFriendSquaredTotal[friendIndex] / nNumbers - means[1+friendIndex] *  means[1+friendIndex]);
			}
//				return new double[] {doubleSortingMin , doubleFriendMin};
			return allStds;
...
//		return ("min = " + getMinsAsDouble()[0] + "\n" + getMinsAsDouble()[1] + " max = " + getMaxesAsDouble()[0] + "," + getMaxesAsDouble()[1] +
//				" mean = " + getMeans()[0] + "," + getMeans()[1] + " std = " + getStds()[0] + "," + getStds()[1] + " n = " + nNumbers);
	}
